...
var metricsHost = os.Getenv("Astral_METRICS_HOST")
...
var staticCDNEndpoint = func() string {
	if v := strings.TrimSpace(os.Getenv("Astral_STATIC_CDN_ENDPOINT")); v != "" {
		return v
	}
	return "https://Astralstatic.com"
}()

const cacheControlImmutable = "public, max-age=31536000, immutable"
const cacheControlShort = "public, max-age=3600"
const cacheControlWeb = "public, max-age=86400"
...
func (s *Server) initAssetsProxy() {
	...
	if err != nil {
		s.errorLog.Printf("Invalid Astral_STATIC_CDN_ENDPOINT %q: %v", staticCDNEndpoint, err)
		return
	}
	...
	proxy.ModifyResponse = func(resp *http.Response) error {
		if strings.HasPrefix(resp.Request.URL.Path, "/assets/") {
			resp.Header.Set("Cache-Control", cacheControlImmutable)
		}
		return nil
	}
...
func (s *Server) buildCSP(nonce string) string {
	cspHosts := map[string][]string{
		...
		"CONNECT": {
			...
			"https://*.hcaptcha.com",
			"https://challenges.cloudflare.com",
			"https://*.Astral.workers.dev",
			"https://Astralusercontent.com",
			"https://Astralstatic.com",
			"https://sentry.web.Astral.app",
			"https://sentry.web.canary.Astral.app",
			"https://Astral.media",
			"ipc:",
			"http://ipc.localhost",
...
func (s *Server) handleStaticAsset(w http.ResponseWriter, r *http.Request, filename, contentType string) {
	...
	w.Header().Set("Cache-Control", staticAssetCacheControl(filename))
	if _, err := w.Write(data); err != nil {
		s.errorLog.Printf("Failed to write response for %s: %v", filename, err)
	}
}

func staticAssetCacheControl(filename string) string {
	switch filename {
	case "sw.js", "version.json":
...
func webAssetContentType(filename string) string {
...
func (s *Server) handleWebAsset(w http.ResponseWriter, r *http.Request) {
	relativePath := strings.TrimPrefix(r.URL.Path, "/web/")
	...
	data, err := staticFS.ReadFile("static/web/" + relativePath)
	if err != nil {
		http.NotFound(w, r)
		return
	}

	w.Header().Set("Content-Type", webAssetContentType(relativePath))
	w.Header().Set("Cache-Control", cacheControlWeb)
	if _, err := w.Write(data); err != nil {
		s.errorLog.Printf("Failed to write web asset %s: %v", relativePath, err)
	}
}
...
func isSourceMapRequest(requestPath string) bool {
	lowerPath := strings.ToLower(requestPath)
	return lowerPath == "/sw.js.map" || (strings.HasPrefix(lowerPath, "/assets/") && strings.HasSuffix(lowerPath, ".map"))
}
...
func sendMetric(path string, body map[string]any) {
	...
	go func() {
		...
		req.Header.Set("User-Agent", "Astral-proxy/metrics")
...
func (s *Server) dispatch(w http.ResponseWriter, r *http.Request) {
	switch {
	case isSourceMapRequest(r.URL.Path):
		http.NotFound(w, r)
		return
...
func (s *Server) handleAppleAppSiteAssociation(w http.ResponseWriter) {
  ...
  "webcredentials": {
    "apps": [
      "3G5837T29K.app.Astral",
      "3G5837T29K.app.Astral.canary"
    ]
  }
}`
