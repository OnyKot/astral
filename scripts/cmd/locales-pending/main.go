...
func main() {
	...
	model := flag.String("model", "", "Model override (defaults to OPENROUTER_MODEL or built-in default)")
	...
	if err := runTranslation(absLocalesDir, *chunkSize, *apiEndpoint, selectedModel, *apiTimeout, *concurrency, *dryRun, *singleLocale, *logRequests, *slowAfter, *qcOnly, qcPasses); err != nil {
		fmt.Printf("\nTranslation failed: %v\n", err)
		os.Exit(1)
	}
}

func runTranslation(localesDir string, chunkSize int, apiEndpoint string, model string, apiTimeout time.Duration, concurrency int, dryRun bool, singleLocale string, logRequests bool, slowAfter time.Duration, qcOnly bool, qcPasses int) error {
	...
	fmt.Printf("╔════════════════════════════════════════════════════════════════════╗\n")
	if qcOnly {
		fmt.Printf("║  Astral Locale QC (Quality Control)                                ║\n")
	} else {
		fmt.Printf("║  Astral Locale Translation                                         ║\n")
	}
	fmt.Printf("╠════════════════════════════════════════════════════════════════════╣\n")
	...
	for _, locale := range targetLocales {
		...
		go func(locale string) {
			defer wg.Done()
			processLocale(locale, reference.Entries, chunkSize, apiEndpoint, model, httpClient, tracker, semaphore, localesDir, logRequests, apiTimeout, qcOnly, qcPasses)
		}(locale)
	}
...
func processLocale(locale string, entries []POEntry, chunkSize int, apiEndpoint string, model string, client *http.Client, tracker *progressTracker, sem chan struct{}, localesDir string, logRequests bool, apiTimeout time.Duration, qcOnly bool, qcPasses int) {
	lp := tracker.locales[locale]
		...
		for qcPass := 1; qcPass <= qcPasses; qcPass++ {
			...
			for i, c := range chunks {
				...
				go func(chunkIndex int, chunkData chunk, passNum int) {
					...
					improvedTranslations, err := callTranslationAPI(client, apiEndpoint, model, prompt, meta, tracker, logRequests, apiTimeout)
					if err != nil {
	...
	for i, c := range chunks {
		...
		go func(chunkIndex int, chunkData chunk) {
			...
			startTime := time.Now()
			translations, err := callTranslationAPI(client, apiEndpoint, model, prompt, meta, tracker, logRequests, apiTimeout)
			if err != nil {
...
func callTranslationAPI(client *http.Client, endpoint, model, prompt string, meta requestMeta, tracker *progressTracker, logRequests bool, apiTimeout time.Duration) (map[string]string, error) {
	var lastErr error

	for attempt := 1; attempt <= maxRetries; attempt++ {
		result, err := doTranslationRequest(client, endpoint, model, prompt, meta, tracker, logRequests, apiTimeout, attempt)
		if err == nil {
			return result, nil
		}
		lastErr = err
...
func doTranslationRequest(client *http.Client, endpoint, model, prompt string, meta requestMeta, tracker *progressTracker, logRequests bool, apiTimeout time.Duration, attempt int) (map[string]string, error) {
...
func buildChunkPrompt(localeName, localeCode string, c chunk, chunkNumber, totalChunks int) string {
	...
	sb.WriteString("- Brand names (Astral, iOS, Android, etc.) stay as-is.\n")
	sb.WriteString("- Product tier names (Astral Plutonium, etc.) stay as-is.\n\n")
...
func buildQCPrompt(localeName, localeCode string, c chunk, currentTranslations map[string]string, chunkNumber, totalChunks, qcPassNum, totalQCPasses int) string {
	...
	sb.WriteString("- Brand names: Astral, iOS, Android, etc.\n")
	sb.WriteString("- Product tier names: Astral Plutonium, etc.\n")
