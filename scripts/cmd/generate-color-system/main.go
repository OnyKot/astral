 ...
 */`
	...
	// Theme classes use :root.theme-* (specificity 0,2,0) rather than bare
	// .theme-* (0,1,0). This is required so they win the cascade against
	// :root declarations in other CSS files (e.g. global.css) that have the
	// same specificity and would otherwise take over via source-order
	// tiebreaker. Without this, switching the theme class on <html> has no
	// visible effect on tokens that global.css happens to redefine.
	blocks := []string{
		renderBlock(":root", rootTokens, cfg.Families),
		renderBlock(":root.theme-light", lightTokens, cfg.Families),
		renderBlock(":root.theme-coal", coalTokens, cfg.Families),
	}
