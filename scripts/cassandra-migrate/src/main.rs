...
#[command(about = "Forward-only Cassandra migration tool for Astral", long_about = Some("A simple, forward-only migration tool for Cassandra.\nMigrations are stored in Astral_devops/cassandra/migrations.\nMigration metadata is stored in the 'Astral' keyspace."))]
struct Cli {
    #[command(subcommand)]
    command: Commands,
