...
fn infer_metric_type(metric: &str) -> &'static str {
    ...
    let is_worker_gauge_metric = metric.starts_with("worker.queue.")
        || metric.starts_with("worker.redis_queue.")
        || metric.starts_with("worker.concurrency.")
        || metric.starts_with("worker.cron.")
        || metric.starts_with("worker.job.avg_wait_time_ms")
        || metric.starts_with("db.pool.")
        || metric == "redis.connection.status";

    if metric.starts_with("gateway.") || is_worker_gauge_metric || is_percentile_metric {
...
pub struct AggregateParams {
    ...
    #[serde(rename = "type", alias = "metric_type")]
...
pub async fn query_aggregate(
    ...
    if metric_type == METRIC_TYPE_GAUGE {
        let summaries = match state
            .storage
            .query_latest_gauges(&params.metric, params.group_by.as_deref())
            .await
        {
            Ok(s) => s,
            Err(e) => {
                error!("Aggregate gauge query failed for {}: {}", params.metric, e);
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(serde_json::json!({"error": format!("Query failed: {}", e)})),
                )
        ...
        if params.group_by.is_some() {
            let mut breakdown: Vec<TopEntry> = summaries
                .into_iter()
            ...
            let total = breakdown.iter().map(|entry| entry.value).sum();
        ...
        let total = summaries.iter().map(|summary| summary.value).sum();
        return Json(AggregateResponse {
            metric: params.metric,
            total,
            breakdown: None,
