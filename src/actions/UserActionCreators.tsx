...
interface AstralTagAvailabilityResponse {
	taken: boolean;
}
...
export const checkAstralTagAvailability = async ({
	username,
	discriminator,
}: {
	username: string;
	...
	try {
		logger.debug(`Checking availability for AstralTag ${username}#${discriminator}`);
		const response = await http.get<AstralTagAvailabilityResponse>({
			url: Endpoints.USER_CHECK_TAG,
			query: {username, discriminator},
		});
		return response.body.taken;
	} catch (error) {
		logger.error('Failed to check AstralTag availability:', error);
		throw error;
	}
};
