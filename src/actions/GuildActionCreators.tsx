...
export const moveChannel = async (guildId: string, operation: ChannelMoveOperation): Promise<void> => {
	try {
		...
		logger.warn(
			`Primary move request failed for channel ${operation.channelId} in guild ${guildId}. Retrying with sibling-only payload...`,
			error,
		);
		try {
			await http.patch({
			...
			logger.debug(`Moved channel ${operation.channelId} in guild ${guildId} via fallback payload`);
		} catch (fallbackError) {
			logger.warn(
				`Sibling-only move failed for channel ${operation.channelId} in guild ${guildId}. Retrying with position-only payload...`,
				fallbackError,
			);
			try {
				await http.patch({
				...
				logger.debug(`Moved channel ${operation.channelId} in guild ${guildId} via position-only fallback`);
			} catch (finalError) {
				logger.error(`Failed to move channel ${operation.channelId} in guild ${guildId}:`, finalError);
				throw finalError;
			}
		}
	}
...
export const setRoleOrder = async (guildId: string, orderedRoleIds: Array<string>): Promise<void> => {
	...
	await setRoleOrderPositions(guildId, payload);
};
...
export const setRoleHoistOrder = async (guildId: string, orderedRoleIds: Array<string>): Promise<void> => {
	...
	await setRoleHoistOrderPositions(guildId, payload);
};

export const setRoleOrderPositions = async (
	guildId: string,
...
export const setRoleHoistOrderPositions = async (
	guildId: string,
	payload: Array<{id: string; hoist_position: number}>,
): Promise<void> => {
	if (payload.length === 0) return;
...
export const toggleDiscoveryVisibility = async (guildId: string, enabled: boolean): Promise<Guild> => {
	try {
		const response = await http.patch<Guild>(Endpoints.GUILD_DISCOVERY_VISIBILITY(guildId), {enabled});
		const guild = response.body;
		logger.debug(`${enabled ? 'Enabled' : 'Disabled'} discovery visibility for guild ${guildId}`);
		return guild;
	} catch (error) {
		logger.error(`Failed to ${enabled ? 'enable' : 'disable'} discovery visibility for guild ${guildId}:`, error);
		throw error;
	}
};
...
export const fetchDiscoveryApplication = async (guildId: string): Promise<DiscoveryApplication | null> => {
	try {
		const response = await http.get<DiscoveryApplication | null>({
			url: Endpoints.GUILD_DISCOVERY_APPLICATION(guildId),
		});
		return response.body ?? null;
	} catch (error) {
		logger.error(`Failed to fetch discovery application for guild ${guildId}:`, error);
		throw error;
	}
};

export const submitDiscoveryApplication = async (
	guildId: string,
	body: {category: string; description: string; tags: Array<string>},
): Promise<DiscoveryApplication> => {
	try {
		const response = await http.put<DiscoveryApplication>({
			url: Endpoints.GUILD_DISCOVERY_APPLICATION(guildId),
			body,
		});
		logger.debug(`Submitted discovery application for guild ${guildId}`);
		return response.body;
	} catch (error) {
		logger.error(`Failed to submit discovery application for guild ${guildId}:`, error);
		throw error;
	}
};

export const withdrawDiscoveryApplication = async (guildId: string): Promise<void> => {
	try {
		await http.delete({url: Endpoints.GUILD_DISCOVERY_APPLICATION(guildId)});
