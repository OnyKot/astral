...
const withPlatformHeader = (headers?: Record<string, string>): Record<string, string> => ({
	'X-Astral-Platform': getPlatformHeaderValue(),
	...(headers ?? {}),
});
...
export const login = async ({
	...
	try {
		...
		// Development mode: auto-add bypass captcha token if not provided
		// Backend's TestCaptchaService checks for X-Captcha-Token: "test" to bypass captcha
		const effectiveCaptchaToken = captchaToken ?? (import.meta.env.DEV ? 'test' : undefined);
		if (effectiveCaptchaToken) {
			headers['X-Captcha-Token'] = effectiveCaptchaToken;
			headers['X-Captcha-Type'] = captchaType || 'turnstile';
		}
		const body: {
			email: string;
			password: string;
...
export const register = async (data: RegisterData): Promise<TokenResponse> => {
	try {
		...
		if (error instanceof HttpError) {
			const errorBody = (error.body ?? {}) as {code?: string; message?: string};
...
export const completeLogin = async ({
	...
	if (userId && token) {
		await AccountManager.switchToNewAccount(userId, token, userData, skipRedirect);
	} else {
		startSession(token, {startGateway: true});
	}
};

export const sendRegistrationPhoneVerification = async (phone: string): Promise<void> => {
	try {
		await http.post({
			url: Endpoints.AUTH_PHONE_SEND_VERIFICATION,
			body: {phone},
			skipAuth: true,
			headers: withPlatformHeader(),
		});
		logger.debug('Registration phone verification code sent');
	} catch (error) {
		logger.error('Failed to send registration phone verification code', error);
		throw error;
	}
};

export const verifyRegistrationPhone = async (phone: string, code: string): Promise<PhoneTokenResponse> => {
	try {
		const response = await http.post<PhoneTokenResponse>({
			url: Endpoints.AUTH_PHONE_VERIFY,
			body: {phone, code},
			skipAuth: true,
			headers: withPlatformHeader(),
		});
		logger.debug('Registration phone verification code accepted');
		return response.body;
	} catch (error) {
		logger.error('Failed to verify registration phone code', error);
		throw error;
	}
};
