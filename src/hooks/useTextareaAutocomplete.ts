			...
			case 'emoji': {
				...
				if (!matchedText || matchedText.trim() === '') {
					...
					const filteredEmojis = permissionFilteredEmojis.filter((emoji) => {
						const isCustom = emoji.guildId != null || emoji.id != null;
	...
	const handleSelect = React.useCallback(
		(option: AutocompleteOption) => {
			...
			if (isMentionMember(option)) {
				...
				const isCustomEmoji = option.emoji.guildId != null || option.emoji.id != null;
				if (!isCustomEmoji) {
					const unicodeText = option.emoji.surrogates ?? '';
					if (!unicodeText) {
						return;
					}

					const replacementText = `${guildBeforeMatch}${unicodeText}`;
					const changeStart = beforeMatch.length;
					const changeEnd = matchEnd;
					segmentManagerRef.current.updateSegmentsForTextChange(changeStart, changeEnd, replacementText.length);

					const finalText = `${beforeMatch}${replacementText}${afterMatch}`;
					const newCursorPosition = beforeMatch.length + replacementText.length;
				...
				displayText = getEmojiDisplayText(option.emoji);
			...
			const newCursorPosition = beforeMatch.length + guildBeforeMatch.length + displayText.length;
