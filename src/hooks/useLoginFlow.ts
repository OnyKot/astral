...
export function useLoginFormController({
	...
	const handleDesktopPasskeyHandoff = useCallback(async () => {
		...
		}, targetWebAppUrl, form.getValue('email') || undefined);
	}, [customApiEndpoint, form, onLoginSuccess, redirectPath]);

	const handlePasskeyLogin = useCallback(async () => {
		setIsPasskeyLoading(true);
		try {
			if (shouldUseLegacyPasskeyBridge()) {
				await handleDesktopPasskeyHandoff();
				return;
			}
			...
			if ((isDesktop() || isNativeMobile()) && !userCancelled) {
				handleDesktopPasskeyHandoff();
			}
		} finally {
			setIsPasskeyLoading(false);
		}
	}, [form, inviteCode, onLoginSuccess, redirectPath, handleDesktopPasskeyHandoff]);
