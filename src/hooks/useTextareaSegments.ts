...
export function useTextareaSegments(): UseTextareaSegmentsReturn {
	...
	const displayToActualSubstring = React.useCallback((displayText: string, start: number, end: number): string => {
		return segmentManagerRef.current.displayToActualSubstring(displayText, start, end);
	}, []);
