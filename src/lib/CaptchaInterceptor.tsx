...
class CaptchaInterceptorStore {
	...
	private isCaptchaError(body: unknown): boolean {
		// Handle plain text "403" response from backend
		if (body === '403' || body === '403 Forbidden') {
			return true;
		}
	...
	private intercept(
		...
		if ((response.status === 400 || response.status === 403) && this.isCaptchaError(response.body)) {
			const i18n = this.i18n!;
			let errorMessage: string;

			// Handle plain text "403" response
			if (typeof response.body === 'string' && (response.body === '403' || response.body === '403 Forbidden')) {
				errorMessage = i18n._(msg`Captcha verification required. Please complete the captcha to continue.`);
			} else {
				const errorBody = response.body as {message?: string};
				errorMessage = errorBody?.message || i18n._(msg`Captcha verification failed. Please try again.`);
			}
