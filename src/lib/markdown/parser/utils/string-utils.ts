...
import {APP_PROTOCOL_PREFIX, LEGACY_APP_PROTOCOL_PREFIX, hasAppProtocolPrefix} from '~/utils/appProtocol';
...
export function startsWithUrl(text: string): boolean {
	...
	if (hasAppProtocolPrefix(text)) {
		const prefixEnd = text.startsWith(APP_PROTOCOL_PREFIX) ? APP_PROTOCOL_PREFIX.length : LEGACY_APP_PROTOCOL_PREFIX.length;
		return !text.substring(0, prefixEnd).includes('"') && !text.substring(0, prefixEnd).includes("'");
	}
