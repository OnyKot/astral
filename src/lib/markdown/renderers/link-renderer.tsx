...
import {APP_PROTOCOL_PREFIX, LEGACY_APP_PROTOCOL_PREFIX} from '~/utils/appProtocol';
...
export const LinkRenderer = observer(function LinkRenderer({
	...
	const userProfileTarget = parseUserProfileLink(url);
	...
	if (shouldShowAccessDeniedModal) {
		...
		isInternal = true;
	} else if (url === `${APP_PROTOCOL_PREFIX}dev` || url === `${LEGACY_APP_PROTOCOL_PREFIX}dev`) {
		handleClick = (e) => {
			e.preventDefault();
		...
		try {
			...
			} else if (userProfileTarget) {
				handleClick = (e) => {
					e.preventDefault();
					const channelGuildId = userProfileTarget.channelId
						? (ChannelStore.getChannel(userProfileTarget.channelId)?.guildId ?? undefined)
						: undefined;
					UserProfileActionCreators.openUserProfile(userProfileTarget.userId, channelGuildId);
				};
				isInternal = true;
			} else if (themeCode) {
				handleClick = (e) => {
