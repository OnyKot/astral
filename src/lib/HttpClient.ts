...
const SUDO_MODE_HEADER = 'x-Astral-sudo-mode-jwt';
...
class HttpClient {
	...
	private normalizeLoopbackOrigin(url: string): string {
		...
		try {
			...
			if (
				this.isLoopbackHost(target.hostname) &&
				this.isLoopbackHost(current.hostname) &&
	...
	private resolveRequestUrl(
		...
		if (path.startsWith('//') || path.includes('://')) {
			return this.normalizeLoopbackOrigin(path);
		}
		...
		return this.normalizeLoopbackOrigin(url);
	}
		...
		const initiator =
			typeof window !== 'undefined'
				...
				: Config.PUBLIC_PROJECT_ENV === 'canary'
					? 'https://astraof.com'
					: 'https://astraof.com';
		requestHeaders.set('X-Astral-Proxy-Initiator', initiator);
