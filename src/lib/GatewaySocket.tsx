...
import type {GatewayMusicActivityPayload} from '~/lib/musicActivity';
import {ExponentialBackoff} from '~/lib/ExponentialBackoff';
import {type CompressionType, GatewayDecompressor, getPreferredCompression} from '~/lib/GatewayCompression';
...
export interface GatewayPresence {
	...
	music_activity?: GatewayMusicActivityPayload | null;
}
...
export class GatewaySocket extends EventEmitter<GatewaySocketEvents> {
	...
	updatePresence(
		...
		musicActivity?: GatewayMusicActivityPayload | null,
	): void {
		...
		this.sendPayload({
			...
			d: {
				...
				...(musicActivity !== undefined && {music_activity: musicActivity}),
			},
		});
	}
	...
	private openSocket(): void {
		...
		try {
			...
			// Use getPreferredCompression() as the default (not a hardcoded
			// 'zstd-stream') so we honour the libfluxcore-known-broken flag.
			// If libfluxcore failed to init in a prior session, this returns
			// 'none' and we never enter the failing decompression path.
			const compression: CompressionType = this.options.compression ?? getPreferredCompression();
			if (compression !== 'none') {
	...
	private buildGatewayUrl(): string {
		...
		// Same default as openSocket — if libfluxcore is flagged broken this
		// resolves to 'none' so the server won't ship us zstd frames.
		const compression: CompressionType = this.options.compression ?? getPreferredCompression();
