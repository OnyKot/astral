...
async function safeUnpublishTrack(participant: LocalParticipant, track: LocalAudioTrack): Promise<void> {
	try {
		...
		// This can happen when permission updates race with reconnect/disconnect internals.
		...
		try {
			switch (source) {
				case 'audio': {
					...
					if (tracks.length > 0) {
						await Promise.allSettled(tracks.map((track) => safeUnpublishTrack(localParticipant, track)));
					}
					break;
				}
				case 'video':
