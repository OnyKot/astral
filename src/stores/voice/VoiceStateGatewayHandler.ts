...
export class VoiceStateGatewayHandler {
	...
	handleGatewayVoiceStateUpdate(guildId: string | null, voiceState: VoiceState): void {
		...
		if (!connectionId) {
			if (!channelId) {
				const removedCount = this.removeAllUserVoiceStates(userId, key);
				logger.debug('[VoiceStateGatewayHandler] User left voice channel without connection_id', {
					guildId: key,
					userId,
					removedCount,
				});
		...
		runInAction(() => {
			...
			this.removeOtherUserConnectionsFromDraft(
				userId,
				key,
				connectionId,
				newVoiceStates,
				newUserVoiceStates,
				newConnectionVoiceStates,
			);
	...
	private removeAllUserVoiceStates(userId: string, guildId: string): number {
		let removedCount = 0;
		...
		runInAction(() => {
			...
			for (const [connectionId, voiceState] of Object.entries(this.getConnectionVoiceStates())) {
				if (voiceState.user_id !== userId || voiceState.guild_id !== guildId) {
					continue;
				}
	...
	private removeOtherUserConnectionsFromDraft(
		userId: string,
		guildId: string,
		keepConnectionId: string,
		newVoiceStates: Record<string, Record<string, Record<string, VoiceState>>>,
		newUserVoiceStates: Record<string, Record<string, VoiceState>>,
		newConnectionVoiceStates: Record<string, VoiceState>,
	): number {
		let removedCount = 0;

		for (const [connectionId, voiceState] of Object.entries(this.getConnectionVoiceStates())) {
			if (connectionId === keepConnectionId || voiceState.user_id !== userId || voiceState.guild_id !== guildId) {
				continue;
			}
			...
			if (voiceState.channel_id) {
				const guildStates = {...(newVoiceStates[guildId] || {})};
				const channelStates = {...(guildStates[voiceState.channel_id] || {})};
				...
				if (Object.keys(guildStates).length === 0) {
					delete newVoiceStates[guildId];
				} else {
					newVoiceStates[guildId] = guildStates;
				}
			}
		...
		const userStates = {...(newUserVoiceStates[userId] || {})};
		if (userStates[guildId]?.connection_id !== keepConnectionId) {
			delete userStates[guildId];
		...
		if (removedCount > 0) {
			logger.debug('[VoiceStateGatewayHandler] Removed duplicate voice connections for user', {
				guildId,
				userId,
				keepConnectionId,
				removedCount,
