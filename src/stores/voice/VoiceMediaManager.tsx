...
import type {LocalAudioTrack, LocalParticipant, Room, ScreenShareCaptureOptions, TrackPublishOptions} from 'livekit-client';
...
function isTrackDetachRaceError(error: unknown): boolean {
	if (!(error instanceof Error)) return false;
	const message = error.message.toLowerCase();
	return error.name === 'InvalidAccessError' || message.includes('sender was not created by this peer connection');
}

async function safeUnpublishTrack(participant: LocalParticipant, track: LocalAudioTrack): Promise<void> {
	try {
		await participant.unpublishTrack(track);
	} catch (error) {
		// LiveKit can race during reconnect/teardown and throw on removeTrack for stale senders.
		if (isTrackDetachRaceError(error)) {
			logger.debug('[safeUnpublishTrack] Ignoring stale sender detach race');
			return;
		}
		throw error;
	}
...
class VoiceMediaManager {
	...
	async ensureMicrophone(room: Room, channelId: string): Promise<void> {
		...
		if (selfMute || selfDeaf || pushToTalkGateMuted) {
			logger.debug('[ensureMicrophone] Skipping: microphone should stay muted', {selfMute, selfDeaf, pushToTalkGateMuted});
	...
	async enableMicrophone(room: Room, channelId: string): Promise<void> {
		...
		try {
			if (isDesktop() || isNativeMobile()) {
				const nativeResult = await ensureNativePermission('microphone');
				if (isNativePermissionDenied(nativeResult)) {
			...
			const currentDeviceState = VoiceDevicePermissionStore.getState();
			const shouldRequestDevicePermissions =
				currentDeviceState.permissionStatus !== 'granted' || currentDeviceState.inputDevices.length === 0;
			await VoiceDevicePermissionStore.ensureDevices({
				requestPermissions: shouldRequestDevicePermissions,
				permissionTargets: ['microphone'],
			}).catch(() => {});
	...
	async disableMicrophone(room: Room): Promise<void> {
		...
		try {
			...
			if (audioPublications.length > 0) {
				...
				await Promise.allSettled(tracks.map((track) => safeUnpublishTrack(participant, track)));
	...
	async setCameraEnabled(enabled: boolean, options?: SetCameraEnabledOptions): Promise<void> {
		...
		try {
			...
			const videoResolution = getCameraCaptureResolution(resolution, frameRate);
			const {publishOptions: cameraPublishOptions, ...cameraOptions} = restOptions;
			const publishOptions = {
				...getCameraPublishOptions(resolution),
				...cameraPublishOptions,
			};

			await participant.setCameraEnabled(enabled, {resolution: videoResolution, ...cameraOptions}, publishOptions);
	...
	async applyLiveMicrophoneSettings(room: Room | null): Promise<void> {
		if (!room?.localParticipant || !room.localParticipant.isMicrophoneEnabled) {
			return;
		}

		const requestedInputDeviceId = VoiceSettingsStore.getInputDeviceId() || 'default';
		const availableInputDevices = VoiceDevicePermissionStore.getState().inputDevices;
