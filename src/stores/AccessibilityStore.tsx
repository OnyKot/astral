...
export const THEME_GRADIENT_STYLES = [
	'default',
	'legacy',
	'aurora',
	'sunset',
...
const isThemeGradientStyle = (value: unknown): value is ThemeGradientStyle =>
	typeof value === 'string' && (THEME_GRADIENT_STYLES as readonly string[]).includes(value);
...
class AccessibilityStore {
	...
	buttonMotionStyle: ButtonMotionStyle = 'default';
	...
	updateSettings(data: Readonly<Partial<AccessibilitySettings>>): void {
		...
		if (validated.customThemeGradientStart !== undefined)
			this.customThemeGradientStart = validated.customThemeGradientStart;
		if (validated.customThemeGradientMiddle !== undefined)
			this.customThemeGradientMiddle = validated.customThemeGradientMiddle;
		if (validated.customThemeGradientEnd !== undefined) this.customThemeGradientEnd = validated.customThemeGradientEnd;
		if (validated.customThemeGradientAngle !== undefined)
		...
		if (validated.openRouterApiKey !== undefined) this.openRouterApiKey = validated.openRouterApiKey;
		if (validated.openRouterModel !== undefined) this.openRouterModel = validated.openRouterModel;
	}
	...
	private validateSettings(data: Readonly<Partial<AccessibilitySettings>>): Partial<AccessibilitySettings> {
		return {
			...
			themeGradientStyle: isThemeGradientStyle(data.themeGradientStyle)
				...
				: this.themeGradientStyle,
			buttonMotionStyle: isButtonMotionStyle(data.buttonMotionStyle)
				? data.buttonMotionStyle
				: this.buttonMotionStyle,
			customThemeGradientStart: isHexColor(data.customThemeGradientStart)
				? data.customThemeGradientStart.toUpperCase()
				: this.customThemeGradientStart,
			customThemeGradientMiddle: isHexColor(data.customThemeGradientMiddle)
				? data.customThemeGradientMiddle.toUpperCase()
				: this.customThemeGradientMiddle,
			customThemeGradientEnd: isHexColor(data.customThemeGradientEnd)
				? data.customThemeGradientEnd.toUpperCase()
				: this.customThemeGradientEnd,
			customThemeGradientAngle: Math.max(
				0,
				Math.min(360, Math.round(data.customThemeGradientAngle ?? this.customThemeGradientAngle)),
			),
			customThemeGradientGlow: Math.max(
				0,
			...
			openRouterApiKey:
				typeof data.openRouterApiKey === 'string' ? data.openRouterApiKey.slice(0, 512) : this.openRouterApiKey,
			openRouterModel:
				typeof data.openRouterModel === 'string' && data.openRouterModel.trim().length > 0
					? data.openRouterModel.trim().slice(0, 120)
