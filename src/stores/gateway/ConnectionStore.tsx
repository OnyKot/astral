...
import {FAVORITES_GUILD_ID, GatewayCloseCodes, GatewayIdentifyFlags} from '~/Constants';
	...
	private createGatewaySocket(token: string, properties: GatewaySocketProperties, generation: number): GatewaySocket {
		...
		const socket = new GatewaySocket(
			gatewayUrl,
			{
				...
				presence: {
					...
					music_activity: presence.music_activity,
				},
				compression,
				identifyFlags,
				initialGuildId,
		...
		socket.on(
			...
			action(({code}) => {
				...
				this.handleConnectionClosed(code);
			}),
		);
