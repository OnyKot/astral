...
class EmojiPickerStore {
	...
	getQuickReactionEmojis(allEmojis: ReadonlyArray<Emoji>, count: number): Array<Emoji> {
		...
		const result: Array<Emoji> = [];
		const seenKeys = new Set<string>();
		const appendIfMissing = (emoji: Emoji | undefined) => {
			if (!emoji) return;
		...
		// Prioritize explicit favorites for quick reactions.
		for (const favoriteEmoji of this.getFavoriteEmojis(allEmojis)) {
			appendIfMissing(favoriteEmoji);
			if (result.length >= count) {
				return result.slice(0, count);
			}
		}

		for (const frecentEmoji of this.getFrecentEmojis(allEmojis, count)) {
			appendIfMissing(frecentEmoji);
			if (result.length >= count) {
				return result.slice(0, count);
			}
		}

		for (const defaultEmoji of DEFAULT_QUICK_EMOJIS) {
			const found = allEmojis.find((emoji) => emoji.uniqueName === defaultEmoji.uniqueName);
			appendIfMissing(found);
			if (result.length >= count) {
				break;
			}
		}
