...
import {musicActivityToKey, type GatewayMusicActivityPayload, toGatewayMusicActivity} from '~/lib/musicActivity';
...
class LocalPresenceStore {
	...
	getPresence(): Presence {
		return {
			...
			music_activity: toGatewayMusicActivity(MusicPresenceStore.currentActivity),
		};
	}

	get presenceFingerprint(): string {
		return `${this.status}|${customStatusToKey(this.customStatus)}|music:${musicActivityToKey(MusicPresenceStore.currentActivity)}|afk:${this.afk ? '1' : '0'}`;
	}

	get musicActivity() {
		return MusicPresenceStore.currentActivity;
	}
