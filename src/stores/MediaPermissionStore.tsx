...
import {checkNativePermission, isNativePermissionDenied} from '~/utils/NativePermissions';
	...
	private async tryInitializeNativePermissions(): Promise<boolean> {
		...
		runInAction(() => {
			if (micState !== 'unsupported') {
				this.microphoneExplicitlyDenied = isNativePermissionDenied(micState);
				this.microphonePermissionState = micState === 'granted' ? 'granted' : 'denied';
			}
			if (cameraState !== 'unsupported') {
				this.cameraExplicitlyDenied = isNativePermissionDenied(cameraState);
				this.cameraPermissionState = cameraState === 'granted' ? 'granted' : 'denied';
			}
			if (screenState !== 'unsupported') {
				this.screenRecordingExplicitlyDenied = isNativePermissionDenied(screenState);
