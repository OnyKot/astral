...
class FeatureFlagStore {
	...
	isFeatureEnabled(flag: FeatureFlag, guildId?: string): boolean {
		...
		if (!guildId) {
			return this.getGuildSet(flag).has(FEATURE_FLAG_GLOBAL_GUILD_ID);
		}

		const guildSet = this.getGuildSet(flag);
		return guildSet.has(FEATURE_FLAG_GLOBAL_GUILD_ID) || guildSet.has(guildId);
	}
	...
	hasAccessToAnyEnabledGuild(flag: FeatureFlag, guildIds: Array<string>): boolean {
		const guildSet = this.getGuildSet(flag);
		if (guildSet.has(FEATURE_FLAG_GLOBAL_GUILD_ID)) {
			return true;
		}
