...
class UserSettingsStore {
	...
	updateUserSettings(userSettings: unknown, options: {hydrate?: boolean} = {}): void {
		...
		/*
		 * Locale catalogs are now loaded as dynamic chunks. Fire-and-
		 * forget so the rest of settings hydration doesn't block on a
		 * network round-trip; once the chunk lands i18n re-renders
		 * the tree via mobx and the new strings appear in place. We
		 * optimistically assume the requested locale is supported and
		 * set it immediately so the UI label matches the user choice.
		 *
		 * Defer the post-load mobx update to a microtask so we never
		 * mutate state during a parent component's render — even if
		 * the dynamic-import promise resolves synchronously from the
		 * webpack cache, the .then() body has to wait one tick. Without
		 * this React logged "Cannot update a component while rendering
		 * a different component" on cached locale loads.
		 */
		this.locale = localeToLoad;
		void loadLocaleCatalog(localeToLoad).then((normalizedLocale) => {
			queueMicrotask(() => {
				/*
				 * Wrap the deferred locale update in runInAction —
				 * queueMicrotask runs outside of any mobx action frame,
				 * and with strict-mode enabled (enforceActions: "always")
				 * direct observable mutations throw a warning. This is
				 * specifically the async resolution of the catalog load.
				 */
				runInAction(() => {
		...
		DeveloperModeStore.setServerEnabled(camelCaseSettings.developerMode);
		this.developerMode = DeveloperModeStore.isDeveloper ? camelCaseSettings.developerMode : false;
	...
	async saveSettings(settings: Partial<UserSettings>): Promise<void> {
		...
		if ('developerMode' in sanitizedSettings && !DeveloperModeStore.isDeveloper) {
			sanitizedSettings.developerMode = false;
		}
		const mergedSnapshot = {
			...previousSnapshot,
			...sanitizedSettings,
			developerMode:
				'developerMode' in sanitizedSettings && !DeveloperModeStore.isDeveloper
					? false
					: (sanitizedSettings.developerMode ?? previousSnapshot.developerMode),
		};
