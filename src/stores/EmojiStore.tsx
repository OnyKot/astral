...
class EmojiStore {
	skinTone = '';
	hiddenEmojiKeyList: Array<string> = [];
	hiddenEmojiKeys = new Set<string>();
	hiddenEmojiRevision = 0;
	...
	private async initPersistence(): Promise<void> {
		await makePersistent(this, 'EmojiStore', ['skinTone', 'hiddenEmojiKeyList']);
		this.syncHiddenEmojiKeysFromList();
	...
	private getEmojiKey(emoji: Emoji): string {
		if (emoji.id) {
			return `guild:${emoji.id}`;
		}
		return `unicode:${emoji.uniqueName || emoji.name}`;
	}

	private isEmojiHidden(emoji: Emoji): boolean {
		return this.hiddenEmojiKeys.has(this.getEmojiKey(emoji));
	}

	private syncHiddenEmojiKeysFromList(): void {
		const dedupedKeys = Array.from(new Set([...this.hiddenEmojiKeys, ...this.hiddenEmojiKeyList].filter(Boolean)));
		this.hiddenEmojiKeyList = dedupedKeys;
		this.hiddenEmojiKeys = new Set(dedupedKeys);
		this.hiddenEmojiRevision += 1;
	}

	private filterHiddenEmojis(emojis: ReadonlyArray<Emoji>): ReadonlyArray<Emoji> {
		if (this.hiddenEmojiKeys.size === 0) {
			return emojis;
		}
		return emojis.filter((emoji) => !this.isEmojiHidden(emoji));
	}

	markEmojiAsUnavailable(emoji: Emoji): void {
		const key = this.getEmojiKey(emoji);
		if (this.hiddenEmojiKeys.has(key)) {
			return;
		}

		this.hiddenEmojiKeys.add(key);
		this.hiddenEmojiKeyList = Array.from(this.hiddenEmojiKeys);
		this.hiddenEmojiRevision += 1;
	}
	...
	filterExternal(
		...
		const visibleResults = this.filterHiddenEmojis(results);
		const filtered = filterEmojisForAutocomplete(i18n, visibleResults, channel);

		return count > 0 ? filtered.slice(0, count) : filtered;
	}

	getAllEmojis(channel: ChannelRecord | null): ReadonlyArray<Emoji> {
		return this.filterHiddenEmojis(this.getDisambiguatedEmojiContext(channel?.guildId).getDisambiguatedEmoji());
	}
