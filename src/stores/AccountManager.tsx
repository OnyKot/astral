...
class AccountManager {
	...
	private shouldManagePushSubscriptions(): boolean {
		return PushSubscriptionService.shouldManagePushSubscriptions();
	}
	...
	async switchToAccount(userId: string): Promise<void> {
		const nextAccount = this.accounts.get(userId);
		...
		if (SessionManager.token) {
			void AndroidBiometricStore.syncSession({
				token: SessionManager.token,
				userId,
				email: nextAccount?.userData?.email ?? null,
			});
		}
	...
	async logout(): Promise<void> {
		await AndroidBiometricStore.clearSession();
