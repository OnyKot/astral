...
import {ChannelTypes, ME, normalizeStatus, OFFLINE_STATUS_TYPES, RelationshipTypes, StatusTypes} from '~/Constants';
import {type CustomStatus, fromGatewayCustomStatus, type GatewayCustomStatusPayload} from '~/lib/customStatus';
import {fromGatewayMusicActivity, musicActivityToKey, type GatewayMusicActivityPayload, type MusicActivity} from '~/lib/musicActivity';
...
export interface Presence {
	...
	readonly music_activity?: GatewayMusicActivityPayload | null;
}
...
class PresenceStore {
	...
	constructor() {
		...
		reaction(
			() => ({
				status: LocalPresenceStore.status,
				customStatus: LocalPresenceStore.customStatus,
				musicKey: musicActivityToKey(LocalPresenceStore.musicActivity),
			}),
	...
	getMusicActivity(userId: string): MusicActivity | null {
		return this.musicActivities.get(userId) ?? null;
	}
	...
	handleConnectionOpen(user: UserPrivate, guilds: Array<GuildReadyData>, presences?: ReadonlyArray<Presence>): void {
		...
		this.musicActivities.clear();
		this.bumpPresenceVersion();

		this.statuses.set(user.id, localStatus);
		this.customStatuses.set(user.id, localCustomStatus);
		this.musicActivities.set(user.id, LocalPresenceStore.musicActivity);
	...
	handlePresenceUpdate(presence: Presence): void {
		const {guild_id: guildIdRaw, user, status, afk, mobile, custom_status: customStatusPayload, music_activity: musicActivityPayload} = presence;
		const normalizedStatus = normalizeStatus(status);
		const userId = user.id;
		const customStatus = fromGatewayCustomStatus(customStatusPayload);
		const musicActivity = fromGatewayMusicActivity(musicActivityPayload);
		const isOfflineLike = OFFLINE_STATUS_TYPES.has(normalizedStatus);
		...
		if (isOfflineLike) {
			existing.guildIds.delete(guildId);
		} else {
	...
	private handleReadyPresence(presence: Presence, initialGuildIds?: Set<string>, hasMeContext = false): void {
		const {user, status, afk, mobile, custom_status: customStatusPayload, music_activity: musicActivityPayload} = presence;
		const normalizedStatus = normalizeStatus(status);
		const customStatus = fromGatewayCustomStatus(customStatusPayload);
		const musicActivity = fromGatewayMusicActivity(musicActivityPayload);
		const userId = user.id;
	...
	private syncLocalPresence(): void {
		...
		const localStatus = LocalPresenceStore.getStatus();
		const localCustomStatus = LocalPresenceStore.customStatus;
		const localMusicActivity = LocalPresenceStore.musicActivity;
		const oldStatus = this.statuses.get(userId);
		const oldMusicActivity = this.musicActivities.get(userId) ?? null;
		...
		this.musicActivities.set(userId, localMusicActivity);
		changed = changed || musicActivityToKey(oldMusicActivity) !== musicActivityToKey(localMusicActivity);
		changed = true;
	...
	private evictPresence(userId: string): void {
		...
		this.musicActivities.delete(userId);
		this.bumpPresenceVersion();
