...
type UserAreaPopoutVariant = 'default' | 'voice-compact';
...
export const UserAreaPopout = observer(({variant = 'default'}: {variant?: UserAreaPopoutVariant}) => {
	...
	const isVoiceCompact = variant === 'voice-compact';

	return (
		<FocusRingScope containerRef={popoutContainerRef}>
	<div ref={popoutContainerRef} className={clsx(styles.container, isVoiceCompact && styles.containerVoiceCompact)}>
		<ProfileCardLayout
			borderColor={isVoiceCompact ? 'var(--guild-shell-stroke)' : borderColor}
			accentEffectPreset={isVoiceCompact ? 'none' : accentEffectPreset}
			className={clsx(styles.profileCardShell, isVoiceCompact && styles.profileCardVoiceCompact)}
		>
			{isVoiceCompact ? (
				<>
					<ProfileCardContent isWebhook={false}>
						<div className={styles.voiceCompactHeader}>
							<StatusAwareAvatar user={currentUser} size={40} />
							<div className={styles.voiceCompactHeaderText}>
								<div className={styles.voiceCompactName}>{displayName}</div>
								<div className={styles.voiceCompactTag}>{currentUser.tag}</div>
							</div>
						</div>
