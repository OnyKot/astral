...
export const UserProfilePopout: React.FC<UserProfilePopoutProps> = observer(
	({popoutKey, user, isWebhook, guildId, isPreview, variant = 'default'}) => {
		const {t} = useLingui();
			...
			<FocusRingScope containerRef={popoutContainerRef}>
				<div ref={popoutContainerRef}>
					<ProfileCardLayout
						...
						hoverRef={hoverRef}
						className={isMemberListVariant ? styles.memberListProfileCard : undefined}
					>
						<ProfileCardBanner
							...
							headerHeight={isMemberListVariant ? 128 : undefined}
						/>
