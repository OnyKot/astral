...
import {PHONE_VERIFICATION_DISMISS_KEY, PHONE_VERIFICATION_MODAL_KEY, PHONE_VERIFICATION_SNOOZE_MS} from '../layout/app-layout/types';
...
const getDescriptionDescriptor = (mode: VerificationMode, reverify = false): MessageDescriptor => {
	switch (mode) {
		case 'email':
			return reverify
				? msg`We've detected suspicious activity on your account. Please reverify your email address to continue using Astral.`
				: msg`We've detected suspicious activity on your account. Please verify your email address to continue using Astral.`;
		case 'phone':
			return reverify
				? msg`We've detected suspicious activity on your account. Please reverify your phone number to continue using Astral.`
				: msg`We've detected suspicious activity on your account. Please verify your phone number to continue using Astral.`;
		case 'email_or_phone':
			return reverify
				? msg`We've detected suspicious activity on your account. Please reverify your email address or phone number to continue using Astral.`
				: msg`We've detected suspicious activity on your account. Please verify your email address or phone number to continue using Astral.`;
	}
};

const RequiredActionModal: React.FC<{mock?: boolean; modeOverride?: VerificationMode}> = observer(
	({mock = false, modeOverride}) => {
	const {t} = useLingui();
	...
	}, [mock, modeOverride, user?.requiredActions, DeveloperOptionsStore.mockRequiredActionsMode]);
	...
	const canDeferPhoneVerification = !mock && modeOverride === 'phone';
	...
	useEffect(() => {
		...
		phoneForm.reset();
		codeForm.reset();
	}, [mock, modeOverride, user?.requiredActions]);
	...
	const handleDeferPhoneVerification = useCallback(() => {
		AppStorage.setItem(PHONE_VERIFICATION_DISMISS_KEY, String(Date.now() + PHONE_VERIFICATION_SNOOZE_MS));
		ModalActionCreators.popWithKey(PHONE_VERIFICATION_MODAL_KEY);
	}, []);
	...
	return (
		...
		>
			...
			{canDeferPhoneVerification && (
				<Modal.Footer>
					<Button variant="secondary" onClick={handleDeferPhoneVerification}>
