...
import {ArrowClockwiseIcon, CameraIcon, ImageIcon} from '@phosphor-icons/react';
...
const MEDIAPIPE_TASKS_VISION_WASM_BASE = `https://astraof.com/libs/mediapipe/tasks-vision/0.10.14/wasm`;
const MEDIAPIPE_SEGMENTER_MODEL_PATH =
	'https://astraof.com/libs/mediapipe/image_segmenter/selfie_segmenter/float16/latest/selfie_segmenter.tflite';
...
const CameraPreviewModalContent = observer((props: CameraPreviewModalProps) => {
	...
	const initializeCamera = useCallback(async () => {
		...
		const shouldApplyBackgroundEffects = !isMobile;
		...
		try {
			...
			const configuredResolutionPreset = CAMERA_RESOLUTION_PRESETS[voiceSettings.cameraResolution];
			const resolutionPreset =
				isMobile && voiceSettings.cameraResolution === 'high'
					? CAMERA_RESOLUTION_PRESETS.medium
					: configuredResolutionPreset;
			...
			try {
				...
				// Attached stream can still render after a rejected eager play call in mobile WebViews.
			}
			...
			if (negotiatedResolution && !isApplyingFixRef.current && !isMobile) {
			...
			try {
				if (shouldApplyBackgroundEffects && isBlur) {
				...
				} else if (shouldApplyBackgroundEffects && !isNone) {
			...
			if (isMountedRef.current) {
				...
				if (!isMobile && needsResolutionFixRef.current && !isApplyingFixRef.current) {
	...
	const handleRetryCameraPreview = useCallback(() => {
		mediaDeviceCache.invalidate('video');
	...
	useEffect(() => {
		...
		void VoiceDevicePermissionStore.ensureDevices({
			requestPermissions: true,
			permissionTargets: ['camera'],
		}).catch(() => {});
		initializeCamera();
	...
	return (
		<Modal.Root size="medium">
							...
							<div className={styles.errorOverlay}>
								<div className={styles.errorText}>
									...
									<div className={styles.errorActions}>
										<Button variant="secondary" small={true} onClick={handleRetryCameraPreview}>
											<Trans>Retry preview</Trans>
										</Button>
									</div>
								</div>
							</div>
			...
			<Modal.Footer>
				<Button variant="secondary" onClick={handleRetryCameraPreview} leftIcon={<ArrowClockwiseIcon size={16} />}>
