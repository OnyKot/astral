...
import styles from '~/components/modals/AstralTagChangeModal.module.css';
...
export const AstralTagChangeModal = observer(() => {
	const {t} = useLingui();
	...
	const onSubmit = React.useCallback(
		async (data: FormInputs) => {
			...
			if (!hasPremium && !skipAvailabilityCheckRef.current && !confirmedRerollRef.current) {
				const tagTaken = await UserActionCreators.checkAstralTagAvailability({
					username: usernameValue,
					discriminator: normalizedDiscriminator,
				});

				if (tagTaken && !isSameTag) {
					const AstralTag = `${usernameValue}#${normalizedDiscriminator}`;

					ModalActionCreators.push(
						modal(() => (
							<ConfirmModal
								title={t`AstralTag already taken`}
								description={
									<div className={styles.confirmDescription}>
										<p>
											<Trans>
												The AstralTag <strong>{AstralTag}</strong> is already taken. Continuing will reroll your
			...
			ToastActionCreators.createToast({type: 'success', children: t`AstralTag updated`});
		},
	...
	return (
		<Modal.Root size="small" centered initialFocusRef={usernameRef}>
			<Form form={form} onSubmit={handleSubmit} aria-label={t`Change AstralTag form`}>
				<Modal.Header title={t`Change your AstralTag`} />
				<Modal.Content className={confirmStyles.content}>
					<p className={clsx(styles.description, confirmStyles.descriptionText)}>
						{hasPremium ? (
							<Trans>
								...
								case-insensitive. You can pick your own numeric tag if it's available.
							</Trans>
						) : (
							<Trans>
								Usernames can only contain letters (a-z, A-Z), numbers (0-9), and underscores. Usernames are
					...
					<div className={styles.AstralTagContainer}>
						<span className={styles.AstralTagLabel}>{t`AstralTag`}</span>
						<div className={styles.AstralTagInputRow}>
						...
						{!hasPremium && (
							<PlutoniumUpsell className={styles.premiumUpsell}>
								<Trans>Customize your numeric tag or keep it when changing your username</Trans>
							</PlutoniumUpsell>
						)}
					</div>
				</Modal.Content>
