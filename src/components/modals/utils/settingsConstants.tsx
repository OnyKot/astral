...
export function getCategoryLabel(category: UserSettingsTabCategories): React.ReactElement {
	switch (category) {
		case 'user_settings':
			return <Trans>Account & Profile</Trans>;
		case 'app_settings':
			return <Trans>Frequently Used</Trans>;
		case 'developer':
			return <Trans>Developer Tools</Trans>;
		case 'staff_only':
			return <Trans>Internal Tools</Trans>;
	}
}
...
const ALL_TABS_DESCRIPTORS: Array<SettingsTabDescriptor> = [
	{
		...
		label: msg`Voice & Video Settings`,
		icon: MicrophoneIcon,
	},
	{
		type: 'appearance',
	...
	{
		/*
		 * Applications & Bots is the user-facing developer entry point —
		 * "User Settings → Applications" is what every doc / quickstart
		 * tells bot authors to open. It used to live under the hidden
		 * `developer` category alongside the staff-only debug tools,
		 * which is why users couldn't find it. Promoted to the regular
		 * user_settings group so claimed accounts see it inline; the
		 * tab itself still gracefully disables the create button for
		 * unclaimed accounts via `UserStore.currentUser?.isClaimed()`.
		 */
		type: 'applications',
		category: 'user_settings',
...
const HIDDEN_TAB_CATEGORIES = new Set<UserSettingsTabCategories>(['developer', 'staff_only']);

export const getSettingsTabs = (t: (msg: MessageDescriptor) => string): Array<SettingsTab> => {
	const allTabs = ALL_TABS_DESCRIPTORS.filter((tab) => !HIDDEN_TAB_CATEGORIES.has(tab.category)).map((tab) => ({
		...tab,
		label: t(tab.label),
	}));
...
const SETTINGS_SECTIONS_MAP_DESCRIPTORS: Partial<Record<UserSettingsTabType, Array<SettingsSectionConfigDescriptor>>> =
	{
		...
		notifications: [
			...
			{id: 'android', label: msg`Android`, isAdvanced: false},
		],
		music_connections: [
			{id: 'services', label: msg`Connected Services`, isAdvanced: false},
			{id: 'sharing', label: msg`Sharing`, isAdvanced: false},
			{id: 'preview', label: msg`Preview`, isAdvanced: false},
		],
		developer_options: [
...
function shouldIncludeSettingsSection(tabType: UserSettingsTabType, sectionId: string): boolean {
	if (tabType === 'notifications' && sectionId === 'android') {
...
export function getSectionsForTab(
	...
	return sections
		.filter((section) => shouldIncludeSettingsSection(tabType, section.id))
		.map((section) => ({
			...section,
...
export function getSectionIdsForTab(tabType: UserSettingsTabType): Array<string> {
	...
	return sections.filter((section) => shouldIncludeSettingsSection(tabType, section.id)).map((section) => section.id);
}
