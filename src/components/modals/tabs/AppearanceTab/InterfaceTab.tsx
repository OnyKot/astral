...
import {getDefaultAvatarURLByIndex} from '~/utils/AvatarUtils';
import {AppZoomLevelTabContent, getAppZoomLevelDescription, shouldShowAppZoomLevel} from './ScalingTab';
						...
						<AvatarStack size={12} maxVisible={5} className={styles.typingAvatars}>
							{[1, 2, 3].map((index) => (
								<MockAvatar
									key={index}
									size={12}
									userTag={t`User ${index}`}
									avatarUrl={getDefaultAvatarURLByIndex(index)}
								/>
							))}
						</AvatarStack>
					)}
...
export const InterfaceTabContent: React.FC = observer(() => {
	...
	const showDockPanelSetting = !MobileLayoutStore.isMobileLayout();
	...
	return (
		<>
			{showZoomLevel ? (
				<SettingsTabSection title={t`App zoom level`} description={getAppZoomLevelDescription(t)}>
