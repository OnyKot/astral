...
const LanguageTab = observer(() => {
	...
	const renderLanguageContent = (option: RadioOption<string>, checked: boolean) => {
		...
		const FlagIcon = ({emoji, label}: {emoji: string; label: string}) => {
			const [imageFailed, setImageFailed] = React.useState(false);
			const emojiUrl = EmojiUtils.getEmojiURL(emoji);
			if (!emojiUrl || imageFailed) {
				return (
					<span className={styles.flagEmoji} role="img" aria-label={label}>
						{emoji}
					</span>
				);
			}
			...
			<div className={styles.notice}>
				<p className={styles.noticeText}>
					<Trans>
						...
						help us localize Astral into your language! To do so, send an email to{' '}
						<a href="mailto:i18n@astraof.com" className={styles.link}>
							i18n@astraof.com
						</a>{' '}
