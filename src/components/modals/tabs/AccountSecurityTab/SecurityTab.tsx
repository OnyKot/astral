...
import {shouldUseLegacyPasskeyBridge} from '~/utils/LegacyPasskeyBridge';
...
export const SecurityTabContent: React.FC<SecurityTabProps> = observer(
	({
		...
		const passkeysManagedOnLegacyBridge = shouldUseLegacyPasskeyBridge();
		const biometricStore = AndroidBiometricStore;
		const biometricAvailable = biometricStore.supported && biometricStore.available;
		const biometricToggleDisabled =
			biometricStore.loading || !AuthenticationStore.authToken || !user.id || (!biometricAvailable && !biometricStore.enabled);

		const handleBiometricToggle = async (enabled: boolean) => {
			if (!AuthenticationStore.authToken || !user.id) {
				return;
			}

			if (enabled) {
				await biometricStore.enableCurrentSession({
					token: AuthenticationStore.authToken,
					userId: user.id,
		...
		const handleRefreshBiometricSession = async () => {
			if (!AuthenticationStore.authToken || !user.id) {
				return;
			}

			await biometricStore.syncSession({
				token: AuthenticationStore.authToken,
				userId: user.id,
				email: user.email ?? null,
			});
		...
		return (
			<>
				...
				{biometricStore.supported && (
					<SettingsTabSection
						title={<Trans>Biometric Sign-In</Trans>}
						description={
							biometricStore.enabled && biometricStore.hasStoredSession ? (
								<Trans>Unlock the saved Astral session on this phone with your fingerprint or face.</Trans>
							) : biometricStore.enabled ? (
								<Trans>Biometric sign-in is enabled. Your next successful sign-in will save a fresh session.</Trans>
							) : biometricAvailable ? (
								<Trans>Use your fingerprint or face to unlock the last saved Astral session on this phone.</Trans>
							) : (
								<Trans>Set up fingerprint or face unlock in Android to use biometric sign-in.</Trans>
							)
						}
					>
						<div className={styles.row}>
							<div className={styles.rowContent}>
								<div className={styles.label}>
									<Trans>Enable biometric sign-in</Trans>
								</div>
								<div className={styles.description}>
									{biometricStore.hasStoredSession && biometricStore.email ? (
										<Trans>Saved account: {biometricStore.email}</Trans>
									) : biometricStore.hasStoredSession ? (
										<Trans>The current account already has a saved session on this phone.</Trans>
									) : biometricStore.available ? (
										<Trans>The current session will be encrypted on this device and unlocked only after biometric confirmation.</Trans>
									) : (
										<Trans>Android biometrics are not available or not enrolled on this device yet.</Trans>
									)}
								</div>
							</div>
							<Switch
								...
								disabled={biometricToggleDisabled}
								ariaLabel={t`Enable biometric sign-in`}
								compact
							/>
						</div>
						...
						{biometricStore.enabled && (
							<div className={styles.divider}>
								<div className={styles.row}>
									<div className={styles.rowContent}>
										<div className={styles.label}>
											<Trans>Saved session</Trans>
										</div>
										<div className={styles.description}>
											{biometricStore.hasStoredSession ? (
												<Trans>Refresh the encrypted session after security changes or account updates.</Trans>
											) : (
												<Trans>No session is saved right now. Save the current session on this phone.</Trans>
											)}
										</div>
									</div>
									<div className={styles.passkeyActions}>
										<Button variant="secondary" small={true} onClick={handleRefreshBiometricSession}>
											{biometricStore.hasStoredSession ? <Trans>Refresh Session</Trans> : <Trans>Save Session</Trans>}
										</Button>
										{biometricStore.hasStoredSession && (
											<Button variant="danger-secondary" small={true} onClick={() => biometricStore.clearSession()}>
				...
				<SettingsTabSection
					...
					description={
						passkeysManagedOnLegacyBridge ? (
							<Trans>
								Existing passkeys still run through the legacy bridge while passkey sign-in finishes moving to
								astraof.com.
							</Trans>
						) : (
							<Trans>Use passkeys for passwordless sign-in and two-factor authentication</Trans>
						)
					}
				>
					<div className={styles.row}>
						...
						{passkeysManagedOnLegacyBridge ? (
							<Tooltip
								text={t`Passkey setup temporarily stays on the legacy bridge while the domain migration finishes.`}
							>
								<div>
