...
const AuthSession: React.FC<AuthSessionProps> = observer(
	({authSession, isCurrent = false, isSelected, onSelect, index, selectionMode}) => {
		...
		const platformLabel =
			authSession.clientPlatform === 'Astral Desktop' ? t`Astral Desktop` : authSession.clientPlatform;
...
const DevicesTab: React.FC = observer(() => {
	...
	const inactiveDevices = authSessions
		.slice(10)
		.filter((authSession) => authSession.id !== authSessionIdHash);
	...
	const handleSelectInactive = () => {
		toggleSelectionMode(true);
		setSelectedDevices(new Set(inactiveDevices.map((device) => device.id)));
		setLastToggledIndex(-1);
	};
	...
	return (
		<SettingsTabContainer>
			...
			<SettingsTabContent>
				{authSessions.length > 10 && (
					<div className={styles.sessionNotice}>
						<WarningCircleIcon className={styles.sessionNoticeIcon} weight="fill" />
						<div className={styles.sessionNoticeBody}>
							<div className={styles.sessionNoticeTitle}>
								<Trans>Too many sessions</Trans>
							</div>
							<div className={styles.sessionNoticeText}>
								<Trans>
									You have more than 10 active sessions. Close the ones you no longer use to keep account access tidy.
								</Trans>
							</div>
						</div>
						{inactiveDevices.length > 0 && (
							<Button variant="secondary" onClick={handleSelectInactive}>
								<Trans>Select inactive</Trans>
							</Button>
						)}
					</div>
				)}
