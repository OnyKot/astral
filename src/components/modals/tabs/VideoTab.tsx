	...
	({voiceSettings, hasPremium, autoRequestPermission = true}) => {
		...
		const screenshareResolutionOptions: ReadonlyArray<RadioOption<'low' | 'medium' | 'high' | 'ultra' | '4k'>> = [
			...
			{value: 'medium', name: t`Medium (720p)`, desc: t`Balanced quality and bandwidth`},
			{
				value: 'high',
				name: t`High (1080p)`,
				desc: t`Best quality for most users`,
			},
			{
				value: 'ultra',
				name: t`Ultra (1440p)`,
						...
						<div className={styles.deviceNoticeText}>
							...
							<p className={styles.deviceNoticeDescription}>
								{permissionStatus === 'denied' ? (
									isNativeAndroid ? (
										<Trans>
											Allow Astral to use your camera in Android app settings so preview, effects, and device
											selection work reliably.
										</Trans>
									) : (
										<Trans>
										Allow Astral to access your camera in System Settings / Privacy &amp; Security / Camera to preview
										and select devices.
									</Trans>
									)
								) : (
									<Trans>Astral needs access to your camera before we can list it here.</Trans>
								)}
							</p>
						</div>
						<Button
							...
							onClick={() => {
								if (permissionStatus === 'denied' && isNativeAndroid) {
									void openNativePermissionSettings('camera');
									return;
								}
					...
					<div className={styles.premiumCard}>
						<div className={styles.premiumHeader}>
							...
							<span className={styles.premiumTitle}>
								<Trans>Unlock Ultra Screen Sharing with Plutonium</Trans>
							</span>
						</div>
						<p className={styles.premiumDescription}>
							<Trans>
								Get even higher quality screen sharing with Ultra (1440p) and 4K (2160p) resolutions.
							</Trans>
						</p>
