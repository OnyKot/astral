...
const MyProfileTabComponent = observer(function MyProfileTabComponent({
	...
	const [profileAccentEffectPreset, setProfileAccentEffectPreset] = React.useState<ProfileAccentEffectPreset>(
		DEFAULT_PROFILE_ACCENT_EFFECT_PRESET,
	);
	...
	const handleProfileTypeChange = React.useCallback(
		(nextGuildId: string | null) => {
			if (nextGuildId === selectedGuildId) {
				return;
			}
			...
			ModalActionCreators.push(
				modal(() => (
					<ConfirmModal
						...
						description={t`Switching profile type will discard your unsaved profile edits.`}
						primaryText={t`Discard and switch`}
		...
		[hasUnsavedChanges, selectedGuildId, t],
	);
	...
	const handleAccentEffectPresetChange = React.useCallback(
		(preset: ProfileAccentEffectPreset) => {
			setProfileAccentEffectPreset(preset);
		},
		[],
	);
	...
	const shouldUseDesktopAccentBioSplit = !mobileLayout.enabled && !isPerGuildProfile;
	const accentBlockClassName = isPerGuildProfile && !hasPremium ? styles.opacityHalf : '';

	const accentColorPickerContent = (
		<AccentColorPicker
			value={form.watch('accent_color') ?? 0}
			hasPremium={hasPremium}
			selectedAccentEffectPreset={profileAccentEffectPreset}
			onAccentEffectPresetChange={handleAccentEffectPresetChange}
			showPremiumAccentEffects={!isPerGuildProfile}
			onChange={(value: number) => form.setValue('accent_color', value === 0 ? null : value, {shouldDirty: true})}
						...
						{isLoadingProfile ? (
							...
							<>
								<div className={styles.contentLayout}>
									...
									{!shouldUseDesktopAccentBioSplit && (
										<>
											<div className={accentBlockClassName}>{accentColorPickerContent}</div>
											<div className={accentBlockClassName}>{bioEditorContent}</div>
										</>
									)}
									</div>
									...
									<div className={styles.previewColumn}>
										<ProfilePreview
											...
											previewAccentEffectPreset={!isPerGuildProfile ? profileAccentEffectPreset : undefined}
								...
								{shouldUseDesktopAccentBioSplit && (
									<div className={styles.profileEnhancementsRow}>
										<div className={styles.profileEnhancementItem}>
											<div className={accentBlockClassName}>{accentColorPickerContent}</div>
										</div>
										<div className={styles.profileEnhancementItem}>
											<div className={accentBlockClassName}>{bioEditorContent}</div>
