...
const GuildRolesTab: React.FC<{guildId: string}> = observer(({guildId}) => {
	...
	const handleSave = React.useCallback(async () => {
		...
		try {
			if (pendingRoleOrder) {
				const filteredIds = pendingRoleOrder.filter((id) => id !== guild.id);
				const rolePositionPayload = filteredIds
					.map((id, index) => {
						const role = guild.roles[id];
						if (!role || role.isEveryone || isRoleLocked(role)) return null;
						const nextPosition = filteredIds.length - index;
						if (role.position === nextPosition) return null;
						return {id, position: nextPosition};
					})
					.filter((entry): entry is {id: string; position: number} => entry !== null);

				if (rolePositionPayload.length > 0) {
					await GuildActionCreators.setRoleOrderPositions(guild.id, rolePositionPayload);
				}
			}

			if (pendingHoistOrder) {
				const filteredIds = pendingHoistOrder.filter((id) => id !== guild.id);
				const hoistPositionPayload = filteredIds
					.map((id, index) => {
						const role = guild.roles[id];
						if (!role || role.isEveryone || isRoleLocked(role)) return null;
						const nextHoistPosition = filteredIds.length - index;
						if (role.effectiveHoistPosition === nextHoistPosition) return null;
						return {id, hoist_position: nextHoistPosition};
					})
					.filter((entry): entry is {id: string; hoist_position: number} => entry !== null);

				if (hoistPositionPayload.length > 0) {
					await GuildActionCreators.setRoleHoistOrderPositions(guild.id, hoistPositionPayload);
				}
			}
