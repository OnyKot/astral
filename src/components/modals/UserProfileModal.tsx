...
const UserInfo: React.FC<UserInfoProps> = observer(
	({user, profile, guildId, warningIndicator, isCurrentUser, onEditCustomStatus, accentEffectPreset}) => {
		...
		return (
			<div className={userProfileModalStyles.userInfo}>
				<div className={clsx(userProfileModalStyles.userInfoHeader, userProfileModalStyles.userInfoHeaderDesktop)} data-accent-effect={accentEffectPreset}>
...
function getMediaPreviewFromAttachment(attachment: MessageAttachment): ProfileMediaPreviewItem | null {
	...
	const isVideoAttachment = contentType.startsWith('video/');
	const isImageAttachment = contentType.startsWith('image/');

	if (!isVoiceAttachment && !isVideoAttachment && !isImageAttachment) {
		return null;
	}
	...
	return {
		...
		info,
		icon: isVoiceAttachment ? 'voice' : isVideoAttachment ? 'video' : 'image',
	};
}
...
const ProfileContent: React.FC<ProfileContentProps> = observer(({profile, user, userNote, autoFocusNote, noteRef}) => {
	...
	const directDmChannel = ChannelStore.dmChannels.find(
		(channel) =>
			channel.type === ChannelTypes.DM &&
			channel.recipientIds.includes(user.id) &&
			!channel.recipientIds.includes(normalizedCurrentUserId),
	);
	...
	for (let index = directDmMessages.length - 1; index >= 0; index -= 1) {
		...
		if (message.author.id !== normalizedCurrentUserId) {
			continue;
		}
		...
		for (const attachment of message.attachments) {
			...
			mediaPreviewItems.push({
				...preview,
				key: `${message.id}-${preview.key}`,
			});

			if (mediaPreviewItems.length >= 16) {
				break;
			}
		}
				...
				<div className={userProfileModalStyles.mediaPreviewSection}>
					...
					<div className={userProfileModalStyles.mediaPreviewList}>
						{mediaPreviewItems.map((item) => (
							<div key={item.key} className={userProfileModalStyles.mediaPreviewItem}>
								<div className={userProfileModalStyles.mediaPreviewThumb}>
									{item.icon === 'voice' ? (
										<WaveformIcon weight="fill" className={userProfileModalStyles.mediaPreviewIcon} />
									) : item.icon === 'video' ? (
										<PlayCircleIcon weight="fill" className={userProfileModalStyles.mediaPreviewIcon} />
									) : (
										<ImageSquareIcon weight="fill" className={userProfileModalStyles.mediaPreviewIcon} />
									)}
								</div>
								<div className={userProfileModalStyles.mediaPreviewMeta}>
									<span className={userProfileModalStyles.mediaPreviewName}>{item.name}</span>
									<span className={userProfileModalStyles.mediaPreviewInfo}>{item.info}</span>
								</div>
							</div>
						))}
						{mediaPreviewItems.length === 0 && (
							<div className={userProfileModalStyles.mediaPreviewEmpty}>
								<Trans>No media sent in this direct message yet.</Trans>
							</div>
						)}
					</div>
				</div>
...
const ProfileModalContent: React.FC<ProfileModalContentProps> = observer(
	({profile, user, userNote, autoFocusNote, noteRef, renderActionButtons, previewOverrides, warningIndicator, accentEffectPreset}) => {
		const {t} = useLingui();
		const effectiveProfile = profile?.getEffectiveProfile() ?? null;
			...
			if (isCurrentUser && disableEditProfile) {
				return (
					<div className={userProfileModalStyles.actionButtons}>
						...
						<Tooltip text={t`Copy User ID`} maxWidth="xl">
							<div>
								<Button
									small={true}
									square={true}
									variant="secondary"
									icon={<CopySimpleIcon className={userProfileModalStyles.buttonIcon} />}
									onClick={handleQuickCopy}
								/>
							</div>
						</Tooltip>
						<Tooltip text={t`Copy Profile Link`} maxWidth="xl">
							<div>
								<Button
									small={true}
									square={true}
