...
import {filterSettingsTabsByAccess} from './utils/settingsTabFilters';
...
export const UserSettingsModal: React.FC<UserSettingsModalProps> = observer(
	({initialTab, initialSubtab, initialGuildId}) => {
		...
		const groupedSettingsTabs = React.useMemo(() => {
			const tabsToGroup = filterSettingsTabsByAccess(settingsTabs, {
				isDeveloper,
				hasExpressionPackAccess,
			});
		...
		React.useEffect(() => {
			const hasSelectedTabAccess = Object.values(groupedSettingsTabs).some((tabs) => tabs.some((tab) => tab.type === selectedTab));
			if (!hasSelectedTabAccess) {
				setSelectedTab('my_profile');
			}
		}, [groupedSettingsTabs, selectedTab]);
		...
		return (
			<SettingsContentKeyProvider>
				<Modal.Root size="fullscreen" onClose={handleClose} disableMobileSwipeDismiss>
