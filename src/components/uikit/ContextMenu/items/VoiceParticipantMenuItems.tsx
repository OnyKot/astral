...
interface ParticipantStreamVolumeSliderProps {
	userId: string;
}

export const ParticipantStreamVolumeSlider: React.FC<ParticipantStreamVolumeSliderProps> = observer(({userId}) => {
	const {t} = useLingui();
	const participantStreamVolume = ParticipantVolumeStore.getStreamVolume(userId);
	const handleChange = React.useCallback(
		(value: number) => {
			ParticipantVolumeStore.setStreamVolume(userId, value);
			MediaEngineStore.applyLocalAudioPreferencesForUser(userId);
		},
		[userId],
	);
