...
import GuildDockStore, {MAX_PINNED_GUILDS_IN_DOCK} from '~/stores/GuildDockStore';
...
export const PinCommunityInDockMenuItem: React.FC<GuildMenuItemProps> = observer(({guild, onClose}) => {
	const {t} = useLingui();
	const isPinned = GuildDockStore.isPinned(guild.id);
	const pinnedCount = GuildDockStore.getPinnedCount();
	const isMaxReached = pinnedCount >= MAX_PINNED_GUILDS_IN_DOCK;
	const disablePinAction = !isPinned && isMaxReached;

	const handleTogglePinned = React.useCallback(() => {
		const result = GuildDockStore.togglePinned(guild.id);
		if (!result.ok && result.reason === 'max-reached') {
			ToastActionCreators.error(
				t`You can pin up to ${MAX_PINNED_GUILDS_IN_DOCK} communities in the dock.`,
			);
			return;
		}
	...
	return (
		<MenuItem
			icon={<PinIcon />}
			hint={isPinned ? t`Pinned` : t`${pinnedCount}/${MAX_PINNED_GUILDS_IN_DOCK}`}
			disabled={disablePinAction}
			onClick={handleTogglePinned}
		>
			{isPinned ? t`Unpin from Dock` : t`Pin in Dock`}
		</MenuItem>
	);
});
