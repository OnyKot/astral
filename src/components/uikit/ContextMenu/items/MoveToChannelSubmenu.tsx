	...
	({userId, guildId, connectionId, connectionIds, onClose, label}) => {
		...
		const handleMoveToChannel = React.useCallback(
			async (channelId: string) => {
				...
				const channelName = targetChannel?.name ?? 'channel';
				...
				try {
					...
					ToastActionCreators.createToast({
						type: 'success',
						children: <Trans>Moved to {channelName}</Trans>,
					});
				} catch (error) {
					...
					ToastActionCreators.createToast({
						type: 'error',
						children: <Trans>Failed to move to {channelName}</Trans>,
					});
				}
			},
			[channels, guildId, userId, connectionId, connectionIds, onClose, isSelf],
		);
