...
type CopyMessageQuoteMenuItemProps = MessageMenuItemProps & {
	onCopyMessageAsQuote: () => void;
};

export const CopyMessageQuoteMenuItem: React.FC<CopyMessageQuoteMenuItemProps> = observer(
	({onCopyMessageAsQuote, onClose}) => {
		const {t} = useLingui();
		const handleCopyQuote = React.useCallback(() => {
			onCopyMessageAsQuote();
			onClose();
		}, [onCopyMessageAsQuote, onClose]);

		return (
			<MenuItem icon={<CopyTextIcon />} onClick={handleCopyQuote} shortcut="q">
				{t`Copy as Quote`}
			</MenuItem>
		);
	},
