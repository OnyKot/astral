...
export const GroupDMContextMenu: React.FC<GroupDMContextMenuProps> = observer(({channel, onClose}) => {
	...
	const handleToggleDockPin = React.useCallback(() => {
		...
		if (!result.ok && result.reason === 'max-reached') {
			ToastActionCreators.createToast({
				type: 'error',
				children: t`You can pin up to ${MAX_PINNED_DMS_IN_DOCK} chats in the dock`,
			});
			return;
		}
		...
		ToastActionCreators.createToast({
			type: 'success',
			children: result.pinned ? t`Pinned in dock` : t`Removed from dock`,
		});
	}, [channel.id, onClose, t]);
		...
		<>
			...
			<MenuGroup>
				...
				<MenuItem icon={<PushPinIcon />} onClick={handleToggleDockPin}>
					{GuildDockStore.isDmPinned(channel.id) ? t`Unpin from Dock` : t`Pin in Dock`}
				</MenuItem>
			</MenuGroup>
