...
import GuildDockStore, {MAX_PINNED_DMS_IN_DOCK} from '~/stores/GuildDockStore';
...
export const DMContextMenu: React.FC<DMContextMenuProps> = observer(({channel, recipient, onClose}) => {
	...
	const handleToggleDockPin = React.useCallback(() => {
		onClose();
		const result = GuildDockStore.togglePinnedDmChannel(channel.id);
		if (!result.ok && result.reason === 'max-reached') {
			ToastActionCreators.createToast({
				type: 'error',
				children: t(i18n)`You can pin up to ${MAX_PINNED_DMS_IN_DOCK} chats in the dock`,
			});
			return;
		}
		...
		ToastActionCreators.createToast({
			type: 'success',
			children: result.pinned ? t(i18n)`Pinned in dock` : t(i18n)`Removed from dock`,
		});
	}, [channel.id, i18n, onClose]);
	...
	return (
		<>
			...
			<MenuGroup>
				...
				<MenuItem icon={<PushPinIcon />} onClick={handleToggleDockPin}>
					{GuildDockStore.isDmPinned(channel.id) ? t(i18n)`Unpin from Dock` : t(i18n)`Pin in Dock`}
				</MenuItem>
			</MenuGroup>
