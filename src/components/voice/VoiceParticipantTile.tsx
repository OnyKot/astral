...
function getTileVideoElement(element: HTMLElement | null): WebKitFullscreenVideoElement | null {
	return (element?.querySelector('video') as WebKitFullscreenVideoElement | null) ?? null;
}
	...
	const fetchPreview = useCallback(async () => {
		...
		const p = (async () => {
			...
			try {
				...
				/*
				 * Stream preview is best-effort: the backend returns 404
				 * for streams that haven't generated a snapshot yet,
				 * which is the steady state for short-lived screen
				 * shares. Logging at console.error spammed the console
				 * on every hover. Quietly return null and rely on the
				 * "No preview yet" placeholder text in the popover.
				 */
				return null;
			} finally {
				setLoading(false);
...
/*
 * The outer wrapper does NOT access any MobX observables — it only calls
 * LiveKit hooks (useEffectiveTrackRef, useParticipantTile). Wrapping it
 * with observer() was a no-op at best and actively harmful at worst:
 * MobX subscribes to every observable that any transitive call touches,
 * which meant every tile re-rendered on ANY voice state change, not
 * just its own. The inner component keeps observer() because it reads
 * from MediaEngineStore / UserStore / CallMediaPrefsStore.
 *
 * React.memo prevents re-renders from parent (VoiceGridLayout) when
 * the tile's own props haven't changed — e.g. when another participant
...
const VoiceParticipantTileInner = observer(function VoiceParticipantTileInner({
	...
	const isOwnScreenSharePreviewPaused =
		isOwnScreenShare && Boolean(callId) && LocalScreenSharePreviewStore.isPreviewPaused(callId);
	...
	const hasVideo = useMemo(() => {
		...
		return Boolean(pub?.track) && !pub?.isMuted && !videoLocallyDisabled && !isOwnScreenSharePreviewPaused;
	}, [trackRef, videoLocallyDisabled, isOwnScreenSharePreviewPaused]);
	...
	const showFullscreenQuickButton = (!isMobileExperience && !isScreenShare) || (isMobileExperience && isScreenShare);
	const shouldPauseOwnScreenSharePreview = isOwnScreenShare && (isOwnScreenSharePreviewPaused || !isWindowFocused);
	...
	const toggleTileFullscreen = useCallback(
		(event?: React.SyntheticEvent) => {
			...
			const video = getTileVideoElement(element);
	...
	}, [identity, isScreenShare, onClick, toggleTileFullscreen]);
	...
	const handleTileFullscreenQuickActionClick = useCallback(
		(event: React.MouseEvent<HTMLButtonElement>) => {
			toggleTileFullscreen(event);
		},
		[toggleTileFullscreen],
	);
	...
	useEffect(() => {
		const onFullscreenChange = () => {
			const video = getTileVideoElement(tileRef.current);
			setIsTileFullscreen(document.fullscreenElement === tileRef.current || Boolean(video?.webkitDisplayingFullscreen));
		};
		...
		<>
			<FocusRing offset={-2}>
				<LongPressable
					...
					onDoubleClick={toggleTileFullscreen}
					role="button"
					tabIndex={0}
				>
					{mediaNode}

					{showFullscreenQuickButton &&
						(isMobileExperience ? (
							<FocusRing offset={-2}>
								<button
									type="button"
									className={clsx(
										styles.tileFullscreenQuickButton,
										isScreenShare && styles.tileFullscreenQuickButtonMobileScreenShare,
										isTileFullscreen && styles.tileFullscreenQuickButtonActive,
									)}
									onClick={handleTileFullscreenQuickActionClick}
									aria-label={isTileFullscreen ? t`Exit Fullscreen` : t`Fullscreen`}
								>
									{isTileFullscreen ? (
										<XIcon weight="bold" className={styles.menuButtonIcon} />
									) : (
										<ArrowsOutIcon weight="bold" className={styles.menuButtonIcon} />
									)}
								</button>
							</FocusRing>
						) : (
							<Tooltip text={isTileFullscreen ? t`Exit Fullscreen` : t`Fullscreen`} position="top">
								<FocusRing offset={-2}>
									<button
										type="button"
										className={clsx(
							...
							{!shouldPauseOwnScreenSharePreview ? (
								...
								<div className={styles.selfStreamPreviewPaused}>
									...
									<span className={styles.pausedText}>
										{isOwnScreenSharePreviewPaused ? t`Preview paused locally` : t`Preview paused to save resources`}
									</span>
					...
					<div className={voiceCallStyles.lkParticipantMetadata}>
						...
						<Tooltip
							text={isCurrentUser ? t`Options` : isScreenShare ? t`Stream volume & options` : t`Volume & options`}
							position="top"
						>
							<FocusRing offset={-2}>
								<div
									...
									tabIndex={0}
									aria-label={
										isCurrentUser ? t`Options` : isScreenShare ? t`Stream volume and options` : t`Volume and options`
									}
