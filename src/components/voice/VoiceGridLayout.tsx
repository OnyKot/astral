	...
	return (
		...
		>
			...
			>
				{tracks.map((trackRef) => {
					/*
					 * Use participant.sid (LiveKit session ID, stable per-connection)
					 * as the primary key component. Falls back to identity for edge
					 * cases where sid isn't available yet (pre-join). This prevents
					 * React from unmounting + remounting the entire tile DOM tree
					 * (including the <video> element and its LiveKit subscription)
					 * when participants shift positions in the grid.
					 */
					const participantKey = trackRef.participant.sid || trackRef.participant.identity;
					const key = isTrackReference(trackRef)
						? `${participantKey}-${trackRef.source}`
						: `placeholder-${participantKey}`;
