...
const InviteLoginPageContainer = observer(() => {
	...
	const params = useMemo(() => new URLSearchParams(location.search), [location.search]);
	const rawRedirect = params.get('redirect_to');
	const inviteLandingPath = `${Routes.inviteRegister(code)}${rawRedirect ? `?redirect_to=${encodeURIComponent(rawRedirect)}` : ''}`;
	const registerPath = `${Routes.REGISTER}?redirect_to=${encodeURIComponent(inviteLandingPath)}`;
	const loginPath = `${Routes.LOGIN}?redirect_to=${encodeURIComponent(inviteLandingPath)}`;

	useAstralDocumentTitle(t`Accept Invite`);
		...
		return (
			<div className={sharedStyles.container}>
				...
				<div className={sharedStyles.disabledActions}>
					<AuthRouterLink to={registerPath} className={sharedStyles.disabledActionLink}>
						<Button fitContainer>
							<Trans>Create Account</Trans>
						</Button>
					</AuthRouterLink>
					<AuthRouterLink to={loginPath} className={sharedStyles.disabledActionLink}>
