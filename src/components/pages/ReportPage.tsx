...
export const ReportPage = observer(() => {
	...
	useAstralDocumentTitle(t`Report Illegal Content`);
		...
		(
			...
			if (error && typeof error === 'object' && 'body' in error && (error as {body?: unknown}).body) {
				...
				const pathMap: Record<string, keyof FormValues> = {
					...
					reporter_Astral_tag: 'reporterAstralTag',
	...
	const handleSubmit = React.useCallback(async () => {
		...
		const reporterAstralTag = state.formValues.reporterAstralTag.trim();
		...
		if (reporterAstralTag) payload.reporter_Astral_tag = reporterAstralTag;
		...
		switch (state.selectedType) {
			...
			case 'user': {
				...
				if (!userId && !userTag) {
					dispatch({
						type: 'SET_ERROR',
						message: t`Provide either a user ID or a AstralTag for the person you are reporting.`,
					});
					return;
				}
