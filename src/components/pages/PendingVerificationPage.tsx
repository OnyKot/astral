...
const PendingVerificationPageContent = observer(function PendingVerificationPageContent() {
	...
	const verificationEmail = new URLSearchParams(location.search).get('email');
	const token = AuthenticationStore.token;
	const socket = ConnectionStore.socket;
	const currentUser = UserStore.currentUser;

	useAstralDocumentTitle(t`Pending Verification`);
		...
		<>
			<div className={styles.header}>
				...
				{verificationEmail ? (
					<p className={styles.description}>
						<Trans>
							We sent a confirmation message to {verificationEmail}. Open that inbox to finish verification while your
