...
export const EmojiPickerInspector = observer(({hoveredEmoji}: EmojiPickerInspectorProps) => {
	...
	const getEmojiForDisplay = (
		...
		/*
		 * Mirror EmojiRenderer's bypass of the sprite sheet for unicode
		 * emoji: production sheet asset 404s on the inspector preview
		 * just like in the picker grid. Use the per-emoji Twemoji URL
		 * instead so the inspector tile actually paints.
		 */
		if (emoji.surrogates) {
