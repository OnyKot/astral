...
export const ChannelChatLayout = observer(({channel, messages, textarea}: ChannelChatLayoutProps) => {
	...
	const isExpressionPickerOpenForChannel =
		MobileLayoutStore.enabled && ExpressionPickerStore.isOpen && ExpressionPickerStore.channelId === channel.id;
	const isDesktopPrivateChannel = !MobileLayoutStore.enabled && channel.isPrivate();

	return (
		<div
			className={clsx(
				styles.container,
				isExpressionPickerOpenForChannel && styles.containerWithRaisedComposer,
