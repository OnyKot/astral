...
export const AddFriendForm: React.FC<AddFriendFormProps> = observer(({onSuccess}) => {
	...
	const getErrorMessage = () => {
		switch (errorCode) {
			...
			case APIErrorCodes.NO_USERS_WITH_ASTRALTAG_EXIST:
				return t`No user found with that AstralTag.`;
			...
			case APIErrorCodes.DISCRIMINATOR_REQUIRED:
				return t`Please enter a valid AstralTag (Username#3).`;
			default:
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (
			...
			Number.isNaN(discriminatorNumber) ||
			discriminatorNumber < 1 ||
			discriminatorNumber > 9999
		) {
			setResultStatus('error');
			setErrorCode(APIErrorCodes.NO_USERS_WITH_ASTRALTAG_EXIST);
			return;
		}
		...
		<form onSubmit={handleSubmit} className={styles.form}>
			<div className={clsx(styles.container, !isMobile && styles.containerDesktop)}>
				<Input
					...
					disabled={isLoading}
					aria-label={t`Friend's AstralTag`}
