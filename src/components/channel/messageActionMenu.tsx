...
import {ArrowsClockwiseIcon, ShareNetworkIcon} from '@phosphor-icons/react';
...
export const useMessageActionMenuData = (
	...
	/*
	 * `createMessageActionHandlers` calls `useLingui()` internally,
	 * which means it's a hook. Calling it inside `React.useMemo` is a
	 * "Hook called inside a built-in Hook" rules-of-hooks violation
	 * that React flags at runtime. Move the call up to the top level
	 * of the function — the handlers don't need memoizing because
	 * they only close over `message` and `onClose` which are already
	 * stable across renders for the same logical message.
	 */
		...
		if (message.state === MessageStates.SENT) {
			...
			if (typeof navigator !== 'undefined' && typeof navigator.share === 'function') {
				utilityActions.push({
					...
					label: t`Share`,
					onClick: handlers.handleShareMessage,
				});
			}
			...
			utilityActions.push({
				icon: <CopyTextIcon size={20} />,
				label: t`Copy as Quote`,
				onClick: handlers.handleCopyMessageAsQuote,
			});
