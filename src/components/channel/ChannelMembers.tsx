...
import {CommunityMembersCard} from '~/components/channel/CommunityMembersCard';
...
const ESTIMATED_MEMBER_ITEM_HEIGHT_PX = 34;
const INITIAL_MEMBER_RANGE: [number, number] = [0, 220];
const SCROLL_BUFFER = 220;
const SUBSCRIBE_RANGE_STEP = 120;
...
const GroupDMMemberListGroup = observer(({group, channelId, ownerId}: GroupDMMemberListGroupProps) => (
	<div className={styles.groupContainer}>
		<div className={styles.groupHeader}>{group.displayName} - {group.count}</div>
...
const LazyMemberListGroup = observer(({guild, group, channelId, members}: LazyMemberListGroupProps) => {
	...
	return (
		<div className={styles.groupContainer}>
			<div className={styles.groupHeader}>{groupName} - {group.count}</div>
			<div className={styles.membersList}>
...
const LazyMemberList = observer(({guild, channel}: LazyMemberListProps) => {
	const subscribedEndRef = React.useRef(INITIAL_MEMBER_RANGE[1]);
	...
	React.useEffect(() => {
		subscribedEndRef.current = INITIAL_MEMBER_RANGE[1];
		subscribe([INITIAL_MEMBER_RANGE]);
	...
	const handleScroll = React.useCallback(
		(event: React.UIEvent<HTMLDivElement>) => {
			...
			const endIndex = Math.ceil((scrollTop + clientHeight) / ESTIMATED_MEMBER_ITEM_HEIGHT_PX) + SCROLL_BUFFER;
			const normalizedEnd = Math.ceil(Math.max(endIndex, INITIAL_MEMBER_RANGE[1]) / SUBSCRIBE_RANGE_STEP) * SUBSCRIBE_RANGE_STEP;

			if (normalizedEnd <= subscribedEndRef.current) {
				return;
			}

			subscribedEndRef.current = normalizedEnd;
			subscribe([[0, normalizedEnd]]);
		},
		[subscribe],
	);
	...
	if (isLoading) {
		return (
			<MemberListContainer channelId={channel.id}>
				<CommunityMembersCard guild={guild} />
				<div className={styles.groupContainer}>
...
export const ChannelMembers = observer(({guild = null, channel}: ChannelMembersProps) => {
	...
	if (isGroupDM) {
		...
		return (
			<OutlineFrame hideTopBorder className={styles.memberFrame}>
				<MemberListContainer channelId={channel.id}>
	...
	return (
		<OutlineFrame hideTopBorder sides={frameSides} className={styles.memberFrame}>
