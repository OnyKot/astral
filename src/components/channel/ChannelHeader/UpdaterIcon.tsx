...
export const UpdaterIcon = observer(() => {
	...
	const hasActionableNativeUpdate = (Platform.isElectron && store.nativeUpdateReady) || store.androidUpdateAvailable;
	...
	const tooltip = React.useMemo(() => {
		...
		if (store.androidUpdateAvailable) {
			return version ? t`Click to download Android update (${version})` : t`Click to download Android update`;
		}
	...
	}, [hasActionableNativeUpdate, store.androidUpdateAvailable, store.displayVersion]);
