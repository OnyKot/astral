...
export const FriendsList: React.FC<FriendsListProps> = observer(({showOnlineOnly, openProfile, searchQuery, onAddFriend}) => {
	const {t} = useLingui();
	...
	if (friendIds.length === 0) {
		return (
			<EmptyStateView
				...
				subtitle={t`Add friends by username to start chatting and hop into voice together.`}
				action={onAddFriend ? {label: t`Add Friend`, onClick: onAddFriend} : undefined}
			/>
		);
	}
