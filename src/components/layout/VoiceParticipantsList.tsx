...
export const VoiceParticipantsList = observer(({guild, channel}: {guild: GuildRecord; channel: ChannelRecord}) => {
	...
	const grouped = React.useMemo(() => {
		...
		/*
		 * Sort order: current user first, then streamers, then stable
		 * alphabetical by userId. Deliberately NOT sorting by anySpeaking
		 * — that changes 2-3×/sec during active conversation and causes
		 * the list to constantly shuffle, which is visually jarring and
		 * triggers cascading re-renders of every child component.
		 */
