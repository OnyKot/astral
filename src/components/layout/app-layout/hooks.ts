...
import {
	...
	type NagbarState,
	NagbarType,
	PHONE_VERIFICATION_DISMISS_KEY,
	PHONE_VERIFICATION_GRACE_START_KEY,
	UPDATE_DISMISS_KEY,
} from './types';
...
export const useNagbarConditions = (): NagbarConditions => {
	...
	const canShowPhoneVerification = (() => {
		if (!user || user.bot || user.system) return false;
		if (user.phone) {
			AppStorage.removeItem(PHONE_VERIFICATION_GRACE_START_KEY);
			AppStorage.removeItem(PHONE_VERIFICATION_DISMISS_KEY);
			return false;
		}
		return false;
	})();
	...
	const canShowGuildMembershipCta = false;
...
export const useActiveNagbars = (conditions: NagbarConditions): Array<NagbarState> => {
	return React.useMemo(() => {
		...
		const nagbars: Array<NagbarState> = [
			...
			{
				type: NagbarType.PHONE_VERIFICATION,
				priority: 2,
				visible: conditions.canShowPhoneVerification,
			},
			{
