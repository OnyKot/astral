...
interface DMListItemProps {
	...
	voiceCallSpeaking?: boolean;
	isDocked?: boolean;
}

export const DMListItem = observer(
	({channel, isSelected, className, voiceCallActive = false, voiceCallSpeaking = false, isDocked = false}: DMListItemProps) => {
	...
	const isIndicatorVisible = hasUnreadMessages || isSelected || isHovering;

	return (
		<>
			<Tooltip position={isDocked ? 'top' : 'right'} size="large" text={tooltipText}>
				<LongPressable
				...
				>
					<FocusRing offset={-2} focusTarget={buttonRef} ringTarget={iconRef}>
						...
						>
							<AnimatePresence>
								{isIndicatorVisible && (
									<div className={clsx(guildStyles.guildIndicator, isDocked && guildStyles.guildIndicatorDocked)}>
										<motion.span
											className={clsx(guildStyles.guildIndicatorBar, isDocked && guildStyles.guildIndicatorBarDocked)}
											initial={false}
											animate={
												isDocked
											...
											exit={isDocked ? {opacity: 0, scale: 0.75, y: 2} : {opacity: 0, scale: 0, height: 0}}
											transition={
												isDocked
							...
							<div className={styles.relative}>
								...
								{voiceCallActive && voiceCallSpeaking && (
									<>
										<span className={styles.voiceSpeakingRingOuter} />
										<span className={styles.voiceSpeakingRingInner} />
									</>
								)}
