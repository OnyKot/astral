...
import {AstralSymbol} from '~/components/icons/AstralSymbol';
...
interface AstralButtonProps {
	isDocked?: boolean;
}

export const AstralButton = observer(({isDocked = false}: AstralButtonProps) => {
	const {t} = useLingui();
			...
			<FocusRing offset={-2} focusTarget={buttonRef} ringTarget={iconRef}>
				...
				>
					...
					<div className={styles.relative}>
						<motion.div
							ref={iconRef}
							className={clsx(styles.AstralButtonIcon, isSelected && styles.AstralButtonIconSelected)}
							animate={{borderRadius: isActive ? '30%' : '50%'}}
							initial={false}
							transition={{duration: 0.07, ease: 'easeOut'}}
							whileHover={{borderRadius: '30%'}}
						>
							<AstralSymbol className={styles.AstralSymbolIcon} />
						</motion.div>
