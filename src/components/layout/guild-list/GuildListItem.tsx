...
export const GuildListItem = observer(
	({guild, isSortingList = false, isSelected, guildIndex, selectedGuildIndex, isDocked = false}: GuildListItemProps) => {
		const {t} = useLingui();
		...
		const handleSelect = () => {
			if (isDragging || isSortingList) return;
						...
						!isSortingList && (
							<div className={styles.guildTooltipContainer}>
								...
								{guild.features.has(GuildFeatures.UNAVAILABLE_FOR_EVERYONE_BUT_STAFF) && (
									<span className={styles.guildTooltipError}>Only accessible to Astral staff</span>
								)}
