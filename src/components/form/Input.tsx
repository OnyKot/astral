...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
	(
		...
		/*
		 * Auto-generate an id when the consumer didn't pass one so the
		 * field's <label htmlFor> always points at a real input. Without
		 * this, clicking the visible label was a no-op for any caller
		 * that forgot to thread `id` through — i.e. most of them.
		 */
		const generatedId = React.useId();
		const resolvedId = (props.id as string | undefined) ?? generatedId;
		...
		return (
			<FieldSet error={error} footer={footer} label={label} labelRight={labelRight} htmlFor={resolvedId}>
				{focusDecoratedInput}
			</FieldSet>
		);
	},
