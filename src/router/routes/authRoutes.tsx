...
/*
 * All auth-flow pages are lazy-loaded so they don't ship with the
 * main bundle. They're tiny by themselves but they pull in form
 * controllers, IP-auth widgets, MFA UI, biometric stores, etc., and
 * an authenticated user never visits them after the first load.
 * `React.lazy` + the now-enabled rspack dynamicImport produces one
 * chunk per page that the browser fetches on demand.
 */
const AuthorizeIPPage = React.lazy(() => import('~/components/pages/AuthorizeIPPage'));
const EmailRevertPage = React.lazy(() => import('~/components/pages/EmailRevertPage'));
const ForgotPasswordPage = React.lazy(() => import('~/components/pages/ForgotPasswordPage'));
const GiftLoginPage = React.lazy(() => import('~/components/pages/GiftLoginPage'));
const GiftRegisterPage = React.lazy(() => import('~/components/pages/GiftRegisterPage'));
const InviteLoginPage = React.lazy(() => import('~/components/pages/InviteLoginPage'));
const InviteRegisterPage = React.lazy(() => import('~/components/pages/InviteRegisterPage'));
const LoginPage = React.lazy(() => import('~/components/pages/LoginPage'));
const OAuthAuthorizePage = React.lazy(() => import('~/components/pages/OAuthAuthorizePage'));
const PendingVerificationPage = React.lazy(() => import('~/components/pages/PendingVerificationPage'));
const RegisterPage = React.lazy(() => import('~/components/pages/RegisterPage'));
const ReportPage = React.lazy(() =>
	import('~/components/pages/ReportPage').then((m) => ({default: m.ReportPage})),
);
const ResetPasswordPage = React.lazy(() => import('~/components/pages/ResetPasswordPage'));
const SpotifyConnectCallbackPage = React.lazy(() => import('~/components/pages/SpotifyConnectCallbackPage'));
const ThemeLoginPage = React.lazy(() => import('~/components/pages/ThemeLoginPage'));
const ThemeRegisterPage = React.lazy(() => import('~/components/pages/ThemeRegisterPage'));
...
const spotifyConnectCallbackRoute = createRoute({
	getParentRoute: () => authLayoutRoute,
	id: 'spotifyConnectCallback',
	path: Routes.SPOTIFY_CONNECT_CALLBACK,
...
const giftsRegisterRoute = createRoute({
	getParentRoute: () => authLayoutRoute,
	id: 'giftsRegister',
	path: '/gifts/:code',
	onEnter: whenAuthenticated((ctx) => {
...
const giftsLoginRoute = createRoute({
	getParentRoute: () => authLayoutRoute,
	id: 'giftsLogin',
	path: '/gifts/:code/login',
	onEnter: whenAuthenticated((ctx) => {
		const code = ctx.params.code;
		if (code) {
			GiftActionCreators.openAcceptModal(code);
...
export const authRouteTree = authLayoutRoute.addChildren([
	...
	...(RuntimeConfigStore.isSelfHosted()
		? []
		: [giftRegisterRoute, giftLoginRoute, giftsRegisterRoute, giftsLoginRoute]),
]);
