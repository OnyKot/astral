...
export const useGuildOverviewData = (guildId: string) => {
	...
	const defaultValues = React.useMemo(() => {
		...
		return {
			...
			discovery_visible: !guild.features.has(GuildFeatures.DISCOVERY_DISABLED),
	...
	const onSubmit = React.useCallback(
		async (data: FormInputs, formInstance: UseFormReturn<FormInputs>) => {
			...
			const discoveryVisible = !guild.features.has(GuildFeatures.DISCOVERY_DISABLED);
			if (data.discovery_visible !== discoveryVisible) {
				await GuildActionCreators.toggleDiscoveryVisibility(guild.id, data.discovery_visible);
			}
