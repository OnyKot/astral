...
const DB_NAME = 'AstralCustomSounds';
const DB_VERSION = 2;
...
export const clearCustomSounds = async (): Promise<void> => {
	const db = await openDB();
	return new Promise((resolve, reject) => {
		const transaction = db.transaction([STORE_NAME], 'readwrite');
		const store = transaction.objectStore(STORE_NAME);
		...
		request.onerror = () => {
			reject(new Error('Failed to clear custom sounds'));
		};
	});
};
...
export const clearAudioCustomizationCache = async (): Promise<void> => {
	await Promise.all([clearCustomSounds(), deleteEntranceSound()]);
};
