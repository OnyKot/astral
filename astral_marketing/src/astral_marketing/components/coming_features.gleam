              ...
              coming_features
                |> list.map(fn(feature) {
                  html.span(
                    [
                      attribute.class(
                        "feature-pill inline-block rounded-xl sm:rounded-2xl bg-[#18181b] px-4 py-2.5 sm:px-6 sm:py-3.5 md:px-7 md:py-4.5 text-sm sm:text-base md:text-lg font-semibold text-zinc-100 shadow-md border border-zinc-700 whitespace-nowrap",
                      ),
                    ],
                    [html.text(feature)],
                  )
