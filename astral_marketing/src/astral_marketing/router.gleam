...
import astral_marketing/badge_proxy
import astral_marketing/help_center
import astral_marketing/locale
import astral_marketing/pages/ai_agent_page
import astral_marketing/pages/careers_page
import astral_marketing/pages/company_page
import astral_marketing/pages/download_page
import astral_marketing/pages/guidelines_page
import astral_marketing/pages/help_article_page
import astral_marketing/pages/help_category_page
import astral_marketing/pages/help_center_page
import astral_marketing/pages/help_search_page
import astral_marketing/pages/home_page
import astral_marketing/pages/migration_page
import astral_marketing/pages/not_found_page
import astral_marketing/pages/partners_page
import astral_marketing/pages/philosophy_page
...
import astral_marketing/sitemap
...
pub fn handle_request(req: Request, ctx: Context) -> Response {
  case wisp.path_segments(req) {
    [] ->
      case is_legacy_domain(req, ctx) {
        True -> migration_page.render(req, ctx)
        False -> home_page.render(req, ctx)
      }
    ["_locale"] -> handle_locale_change(req, ctx)
    ["moved"] -> migration_page.render(req, ctx)
    ...
    ["favicon.ico"] -> wisp.redirect(ctx.app_endpoint <> "/web/favicon.ico")
    ["favicon-16x16.png"] ->
      wisp.redirect(ctx.app_endpoint <> "/web/favicon-16x16.png")
    ["favicon-32x32.png"] ->
      wisp.redirect(ctx.app_endpoint <> "/web/favicon-32x32.png")
    ["apple-touch-icon.png"] ->
      wisp.redirect(ctx.app_endpoint <> "/web/apple-touch-icon.png")
    ...
    ["status"] -> status_page.render(req, ctx)
    ...
    ["docs"] -> wisp.redirect(web.public_docs_url())
    ["docs", "bots"] -> wisp.redirect(web.public_bot_docs_url())
    ["ai-agent"] -> ai_agent_page.render(req, ctx)
...
fn handle_security_txt(ctx: Context) -> Response {
  ...
  let body_lines =
    [
      ...
      "Contact: mailto:security@astraof.com",
      "Expires: " <> expires,
...
fn is_legacy_domain(req: Request, ctx: Context) -> Bool {
  let host = case request.get_header(req, "x-forwarded-host") {
    Ok(value) -> first_host_value(value)
    Error(_) ->
      case request.get_header(req, "host") {
        Ok(value) -> value
        Error(_) -> ctx.base_url
      }
  }

  let host = string.lowercase(host)
  let base_url = string.lowercase(ctx.base_url)

  string.contains(host, "asrtal.ru") || string.contains(base_url, "asrtal.ru")
}

fn first_host_value(header: String) -> String {
