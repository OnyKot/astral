...
pub fn load_config() -> Result(Config, String) {
  ...
  use api_endpoint_raw <- result.try(required_env("ASTRAL_API_PUBLIC_ENDPOINT"))
  use api_host <- result.try(required_env("ASTRAL_API_HOST"))
  use app_endpoint_raw <- result.try(required_env("ASTRAL_APP_ENDPOINT"))
  use cdn_endpoint_raw <- result.try(required_env("ASTRAL_CDN_ENDPOINT"))
  use marketing_endpoint_raw <- result.try(required_env(
    "ASTRAL_MARKETING_ENDPOINT",
  ))
  use base_path_raw <- result.try(required_env("ASTRAL_PATH_MARKETING"))
  use port <- result.try(required_int_env("ASTRAL_MARKETING_PORT"))
