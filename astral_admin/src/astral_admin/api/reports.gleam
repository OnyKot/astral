...
import astral_admin/api/messages.{type Message, Message, MessageAttachment}
...
pub fn resolve_report(
  ...
  let fields = case public_comment {
    option.Some(comment) -> {
      let trimmed_comment = string.trim(comment)
      case string.is_empty(trimmed_comment) {
        True -> fields
        False -> [#("public_comment", json.string(trimmed_comment)), ..fields]
      }
    }
    option.None -> fields
  }
