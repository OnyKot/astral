...
pub fn send_passkey_migration_email(
  ctx: web.Context,
  session: web.Session,
  user_id: String,
) -> Result(Nil, ApiError) {
  admin_post_simple(ctx, session, "/admin/users/send-passkey-migration-email", [
    #("user_id", json.string(user_id)),
  ])
}

pub fn reset_passkeys(
  ctx: web.Context,
  session: web.Session,
  user_id: String,
) -> Result(Nil, ApiError) {
  admin_post_simple(ctx, session, "/admin/users/reset-passkeys", [
    #("user_id", json.string(user_id)),
  ])
}
...
pub fn search_users(
  ...
  has_premium: Option(Bool),
  is_bot: Option(Bool),
  limit: Int,
  offset: Int,
) -> Result(SearchUsersResponse, ApiError) {
  let url = ctx.api_endpoint <> "/admin/users/search"
  let query_field = case query {
    option.Some(raw_query) ->
      case string.trim(raw_query) {
        "" -> []
        normalized_query -> [#("query", json.string(normalized_query))]
      }
    option.None -> []
  }
  let body =
    json.object(
      query_field
      |> list.append(case email_verified {
        option.Some(value) -> [#("email_verified", json.bool(value))]
        option.None -> []
      })
      |> list.append(case has_premium {
        option.Some(value) -> [#("has_premium", json.bool(value))]
        option.None -> []
      })
      |> list.append(case is_bot {
        option.Some(value) -> [#("is_bot", json.bool(value))]
        option.None -> []
      })
      |> list.append([
