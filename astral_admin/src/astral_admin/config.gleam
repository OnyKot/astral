...
pub fn load_config() -> Result(Config, String) {
  use api_endpoint_raw <- result.try(
    case optional_env("ASTRAL_ADMIN_API_ENDPOINT") {
      option.Some(endpoint) -> Ok(endpoint)
      option.None ->
        case optional_env("ASTRAL_API_CLIENT_ENDPOINT") {
          option.Some(endpoint) -> Ok(endpoint)
          option.None -> required_env("ASTRAL_API_PUBLIC_ENDPOINT")
        }
    },
  )
  use media_endpoint_raw <- result.try(required_env("ASTRAL_MEDIA_ENDPOINT"))
  use cdn_endpoint_raw <- result.try(required_env("ASTRAL_CDN_ENDPOINT"))
  use admin_endpoint_raw <- result.try(required_env("ASTRAL_ADMIN_ENDPOINT"))
  use web_app_endpoint_raw <- result.try(required_env("ASTRAL_APP_ENDPOINT"))
  ...
  use base_path_raw <- result.try(required_env("ASTRAL_PATH_ADMIN"))
  use port <- result.try(required_int_env("ASTRAL_ADMIN_PORT"))
  ...
  let metrics_endpoint = case optional_env("ASTRAL_METRICS_HOST") {
