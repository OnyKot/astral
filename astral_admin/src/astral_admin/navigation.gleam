...
pub fn sections() -> List(NavSection) {
  [
    NavSection("Поиск", [
      NavItem("Поиск пользователей", "/users", "users", [
        constants.acl_user_lookup,
      ]),
      NavItem("Поиск серверов", "/guilds", "guilds", [
        constants.acl_guild_lookup,
      ]),
    ]),
    NavSection("Модерация", [
      NavItem("Жалобы", "/reports", "reports", [constants.acl_report_view]),
      NavItem("Проверки", "/pending-verifications", "pending-verifications", [
        constants.acl_pending_verification_view,
      ]),
      NavItem("Discovery", "/discovery-applications", "discovery-applications", [
        constants.acl_guild_discovery_review,
      ]),
      NavItem("Массовые операции", "/bulk-actions", "bulk-actions", [
    ...
    NavSection("Контент", [
      ...
      NavItem("Пурж ассетов", "/asset-purge", "asset-purge", [
        constants.acl_asset_purge,
      ]),
    ]),
    NavSection("Наблюдение", [
      NavItem("Сводка", "/metrics", "metrics", [constants.acl_metrics_view]),
      NavItem("API и сообщения", "/messages-metrics", "messages-metrics", [
        constants.acl_metrics_view,
      ]),
      NavItem("ИИ", "/ai", "ai", [constants.acl_metrics_view]),
    ]),
    NavSection("Операции", [
      ...
      NavItem("Очереди", "/jobs", "jobs", [constants.acl_metrics_view]),
      NavItem("Хранилище", "/storage", "storage", [constants.acl_metrics_view]),
      NavItem("База и Redis", "/database", "database", [
        constants.acl_metrics_view,
      ]),
      NavItem("Деплой / CI", "/ops", "ops", [constants.acl_metrics_view]),
      NavItem("Аудит", "/audit-logs", "audit-logs", [
        constants.acl_audit_log_view,
      ]),
    ]),
    NavSection("Платформа", [
      NavItem("Индексы", "/search-index", "search-index", [
        constants.acl_guild_lookup,
      ]),
    ...
    NavSection("Настройки", [
      NavItem("Конфиг", "/instance-config", "instance-config", [
        constants.acl_instance_config_view,
        constants.acl_instance_config_update,
      ]),
      NavItem("Флаги", "/feature-flags", "feature-flags", [
        constants.acl_feature_flag_view,
        constants.acl_feature_flag_manage,
      ]),
    ]),
    NavSection("Коды", [
      NavItem("Бета-коды", "/beta-codes", "beta-codes", [
