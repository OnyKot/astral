...
import astral_admin/api/system
...
import astral_admin/web.{
  type Context, type Session, action, cache_busted_asset, href,
  prepend_base_path,
}
import gleam/float
import gleam/int
...
pub fn view(
  ...
  let hard_error = case node_stats_result {
    Error(common.Unauthorized) -> option.Some("Unauthorized")
    Error(common.Forbidden(message)) -> option.Some(message)
    Error(common.NotFound) -> option.Some("Not found")
    _ ->
      case guild_stats_result {
        Error(common.Unauthorized) -> option.Some("Unauthorized")
        Error(common.Forbidden(message)) -> option.Some(message)
        Error(common.NotFound) -> option.Some("Not found")
        _ -> option.None
  ...
  let content = case hard_error {
    option.Some(message) -> render_error(ctx, message)
    option.None -> {
      let node_stats = case node_stats_result {
        Ok(stats) -> option.Some(stats)
        _ -> option.None
      }
      let guilds = case guild_stats_result {
        Ok(stats) -> stats.guilds
        _ -> []
      }
      let node_warning = case node_stats_result {
        Error(common.ServerError) ->
          option.Some("Live gateway node stats are temporarily unavailable.")
        Error(common.NetworkError) ->
          option.Some(
            "Could not reach the gateway stats service. Showing cached page shell.",
          )
        _ -> option.None
      }
      let guild_warning = case guild_stats_result {
        Error(common.ServerError) ->
          option.Some("Guild memory leaderboard is temporarily unavailable.")
        Error(common.NetworkError) ->
          option.Some(
            "Could not reach guild memory service. Leaderboard data was not loaded.",
          )
        _ -> option.None
      }
      let warnings = case node_warning, guild_warning {
      ...
      render_success(ctx, admin_acls, node_stats, guilds, result, warnings)
    }
  }
...
fn render_warnings(warnings: List(String)) {
  case warnings {
    [] -> element.none()
    _ ->
      h.div(
        [
          a.class(
            "mb-6 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-amber-900",
          ),
        ],
        [
          h.p([a.class("text-sm font-medium mb-2")], [
            element.text("Some gateway data could not be loaded"),
          ]),
          h.ul(
            [a.class("list-disc ml-5 space-y-1 text-sm text-amber-800")],
            list.map(warnings, fn(message) { h.li([], [element.text(message)]) }),
          ),
        ],
      )
  }
