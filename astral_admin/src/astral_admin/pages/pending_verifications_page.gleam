...
fn selection_toolbar() -> element.Element(a) {
  h.div(
    [
      a.class(
        "mt-4 flex items-center justify-between gap-3 bg-zinc-950/80 border border-zinc-800 rounded-xl px-3 py-2",
      ),
    ...
    [
      h.div([a.class("flex items-center gap-3")], [
        ...
        h.span([a.class("text-sm text-zinc-300")], [
          element.text("Выбрать все видимые"),
        ]),
      ]),
      h.div([a.class("flex items-center gap-3 flex-wrap justify-end")], [
        ...
        h.div([a.class("flex items-center gap-2")], [
          h.button(
            ...
            [element.text("Отклонить выбранные")],
          ),
          h.button(
            [
            ...
            [element.text("Одобрить выбранные")],
          ),
        ]),
      ]),
    ],
  ...
  let content = case result {
    Ok(response) -> {
      ...
      h.div(
        ...
        [
          ui.flex_row_between([
            ui.heading_page("Ожидают проверки"),
            h.div([a.class("flex items-center gap-4")], [
              case total {
                ...
                count ->
                  h.span(
                    [
                      a.class("body-sm text-zinc-400"),
                      a.attribute("data-remaining-total", "true"),
                    ],
                    [element.text(int.to_string(count) <> " осталось")],
                  )
              },
            ]),
          ]),
              ...
              ui.flex_row_between([
                ...
                h.a(
                  [
                    ...
                    a.class("text-sm text-zinc-400 hover:text-zinc-100"),
                  ],
...
fn render_pending_verification_card(
  ...
  h.div(
    [
      a.class(
        "bg-zinc-950/80 border border-zinc-800 rounded-xl shadow-xs p-6 focus:outline-none focus:ring-2 focus:ring-indigo-500/20",
      ),
      a.tabindex(0),
    ...
    [
        ...
        h.div([a.class("flex-1 min-w-0")], [
          h.a(
            [
              ...
              a.class(
                "text-lg text-zinc-100 hover:text-zinc-400 underline decoration-neutral-300 hover:decoration-neutral-500 font-semibold",
              ),
            ],
            [
              element.text(
          ...
          h.div([a.class("text-sm text-zinc-400 mt-1 truncate")], [
            case pv.user.email {
              option.Some(email) -> element.text(email)
              option.None -> element.text("N/A")
            },
          ]),
          h.div([a.class("text-sm text-zinc-500 mt-1")], [
      ...
      h.details(
        ...
        [
          h.summary(
            [
              a.class(
                "cursor-pointer text-sm text-zinc-400 hover:text-zinc-100 list-none flex items-center gap-2",
              ),
            ],
            [
              element.text("Registration metadata"),
              h.span([a.class("text-zinc-500 group-open:hidden")], [
                element.text("▼"),
              ]),
              h.span([a.class("text-zinc-500 hidden group-open:inline")], [
                element.text("▲"),
              ]),
            ],
          ),
          h.div([a.class("mt-3 text-sm text-zinc-400 space-y-1")], [
      ...
      case can_review {
        True ->
          h.div([a.class("pt-4 border-t border-zinc-800")], [
        ...
        False ->
          h.div([a.class("text-sm text-zinc-500 pt-2")], [
...
fn render_registration_metadata(
  ...
  h.div([a.class("flex flex-col gap-0.5 text-xs text-zinc-400")], [
...
(function () {
  ...
  function showToast(message, variant) {
    ...
    box.innerHTML =
      ...
      (variant === 'success' ? 'Сохранено' : 'Ошибка действия') +
      '</div>' +
  ...
  function removeCard(id) {
    ...
    if (grid.querySelectorAll('[data-select-card]').length === 0) {
      grid.innerHTML =
        '<div class=\"col-span-full border border-dashed border-zinc-800 rounded-lg p-8 text-center text-zinc-500\">All registration requests have been processed</div>';
    }
  }
