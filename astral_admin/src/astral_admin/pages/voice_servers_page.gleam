...
fn view_servers(
  ...
  let hard_error = case servers_result {
    Error(common.Unauthorized) -> option.Some(common.Unauthorized)
    Error(common.Forbidden(message)) -> option.Some(common.Forbidden(message))
    Error(common.NotFound) -> option.Some(common.NotFound)
    _ ->
      case region_result {
        Error(common.Unauthorized) -> option.Some(common.Unauthorized)
        Error(common.Forbidden(message)) ->
          option.Some(common.Forbidden(message))
        Error(common.NotFound) -> option.Some(common.NotFound)
        _ -> option.None
      }
  }

  let content = case hard_error {
    option.Some(err) -> errors.error_view(err)
    option.None -> {
      let region_name = case region_result {
        Ok(region_response) ->
          case region_response.region {
            option.Some(r) -> r.name
            option.None -> region_id
          }
        Error(_) -> region_id
      }
      let servers = case servers_result {
        Ok(servers_response) -> servers_response.servers
        Error(_) -> []
      }
      let servers_load_failed = case servers_result {
        Error(common.ServerError) -> True
        Error(common.NetworkError) -> True
        _ -> False
      }
      let warning_message = case servers_result, region_result {
        Error(common.ServerError), _ ->
          option.Some("Failed to load current server list. Please try again.")
        Error(common.NetworkError), _ ->
          option.Some(
            "Could not reach voice server backend. Showing region shell only.",
          )
        _, Error(common.ServerError) ->
          option.Some(
            "Failed to load region details. Showing region id instead.",
          )
        _, Error(common.NetworkError) ->
          option.Some("Could not reach region details endpoint.")
        _, _ -> option.None
      }
      ...
      h.div([a.class("space-y-6")], [
        ui.flex_row_between([
          ...
          h.a(
            [
              a.href("#create"),
              a.class(
                "px-4 py-2 bg-neutral-900 text-white rounded text-sm font-medium hover:bg-neutral-800 transition-colors text-center w-full sm:w-auto",
              ),
            ],
        ...
        render_servers_list(ctx, region_id, servers, servers_load_failed),
  ...
  case load_failed {
    True ->
      h.div(
        [
          a.class(
            "bg-amber-50 border border-amber-200 rounded-xl p-8 text-center",
          ),
        ],
        [
          h.p([a.class("text-amber-800 text-sm font-medium mb-2")], [
            element.text("Unable to load voice servers right now."),
          ]),
          h.a(
            [
              href(ctx, "/voice-servers?region=" <> region_id),
              a.class(
                "inline-block mt-2 px-4 py-2 bg-amber-600 text-white rounded-lg text-sm font-medium hover:bg-amber-700 transition-colors",
              ),
            ],
            [element.text("Retry")],
          ),
...
fn render_server_card(
  ...
  h.div(
    ...
    [
      h.div(
        ...
        [
          h.div([a.class("min-w-0")], [
            h.h3(
              [a.class("text-base font-semibold text-neutral-900 break-all")],
              [
                element.text(server.server_id),
              ],
            ),
            h.p([a.class("text-sm text-neutral-600 mt-1 break-all")], [
      ...
      h.div([a.class("grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4")], [
          ...
          [
            h.button(
              [
                a.type_("submit"),
                a.class(
                  "px-3 py-1.5 w-full bg-red-600 text-white text-sm rounded hover:bg-red-700 transition-colors",
                ),
                a.attribute(
                  "onclick",
      ...
      h.details([a.class("mt-6")], [
        h.summary(
          [
            a.class(
              "cursor-pointer px-4 py-2 bg-blue-50 text-blue-700 rounded hover:bg-blue-100 transition-colors text-sm font-medium w-full sm:w-auto text-center",
            ),
          ],
          [
      ...
      [
        ...
        h.div([a.class("space-y-2")], [
          h.label([a.class("flex items-center gap-2")], [
            h.input(
              [
                [a.type_("checkbox"), a.name("is_active"), a.value("true")],
                case server.is_active {
                  True -> [a.checked(True)]
                  False -> []
                },
