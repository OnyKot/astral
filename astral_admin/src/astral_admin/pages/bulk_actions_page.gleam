...
import astral_admin/api/bulk
import astral_admin/api/common
import astral_admin/components/deletion_days_script
...
pub fn view(
  ...
  let content =
    h.div([a.class("space-y-6")], [
      ...
      case
        acl.has_permission(admin_acls, constants.acl_bulk_send_email_broadcast)
      {
        True -> render_bulk_send_email_broadcast()
        False -> element.none()
      },
...
fn render_result(response: bulk.BulkOperationResponse) {
  let success_count = response.successful_count
  let fail_count = response.failed_count
...
fn render_bulk_send_email_broadcast() {
  let broadcast_subject = "Поддержка Astral теперь в Telegram"
  let broadcast_body =
    "Привет!\n\nУ Astral появился официальный Telegram-бот поддержки: @AstraSuport_bot.\n\nПиши туда по вопросам аккаунта, входа, жалоб, оплат и доступа. Если нужен быстрый контакт с поддержкой, используй именно его.\n\nПароль, коды входа и другие секреты мы не запрашиваем."
  let broadcast_key = "telegram_support_bot_2026_05_11"
  let broadcast_category = "support_notice"
  let broadcast_reason = "Announce official Telegram support bot"

  ui.card(ui.PaddingMedium, [
    ui.heading_card_with_margin("Bulk Email Broadcast"),
    h.p([a.class("text-sm text-neutral-600 mb-4")], [
      element.text(
        "Sends an email to users with verified and non-bounced email addresses.",
      ),
    ]),
    h.form(
      [
        a.method("POST"),
        a.action("?action=bulk-send-email-broadcast"),
        a.class("space-y-4"),
        a.attribute(
          "onsubmit",
          "return confirm('Send this email broadcast to all eligible users?')",
        ),
      ],
      [
        h.div([a.class("space-y-2")], [
          ...
          h.input([
            a.type_("text"),
            a.name("subject"),
            a.value(broadcast_subject),
            a.placeholder("Updated Astral legal documents"),
            a.required(True),
            a.class(
              "w-full px-3 py-2 border border-neutral-300 rounded focus:outline-none focus:ring-2 focus:ring-neutral-900 text-sm",
            ),
        ...
        h.div([a.class("space-y-2")], [
          ...
          h.textarea(
            [
              ...
              a.attribute("rows", "10"),
              a.placeholder(
                "Hello.\n\nWe updated the Astral Terms of Service and Community Guidelines.\n\nhttps://astraof.com/terms\nhttps://astraof.com/guidelines",
              ),
              a.class(
                "w-full px-3 py-2 border border-neutral-300 rounded focus:outline-none focus:ring-2 focus:ring-neutral-900 text-sm",
              ),
        ...
        h.div([a.class("grid grid-cols-1 md:grid-cols-2 gap-4")], [
          h.div([a.class("space-y-2")], [
            h.label(
              [a.class("text-sm font-medium text-neutral-700 mb-2 block")],
              [
                element.text("Broadcast Key"),
              ],
            ),
            h.input([
              a.type_("text"),
              a.name("broadcast_key"),
              a.value(broadcast_key),
              a.placeholder("legal_docs_2026_03_15"),
              a.class(
                "w-full px-3 py-2 border border-neutral-300 rounded focus:outline-none focus:ring-2 focus:ring-neutral-900 text-sm",
              ),
            ]),
          ...
          h.div([a.class("space-y-2")], [
            ...
            h.input([
              a.type_("text"),
              a.name("category"),
              a.value(broadcast_category),
              a.placeholder("legal_notice"),
              a.class(
                "w-full px-3 py-2 border border-neutral-300 rounded focus:outline-none focus:ring-2 focus:ring-neutral-900 text-sm",
              ),
            ]),
        ...
        h.div(
          [
            a.class(
              "space-y-3 rounded border border-neutral-200 bg-neutral-50 p-4",
            ),
          ],
          [
            ...
            ui.custom_checkbox(
              "attach_tracking_headers",
              "true",
              "Attach Astral tracking headers for provider-side filtering",
              True,
              option.None,
            ),
            h.p([a.class("text-xs text-neutral-500")], [
              element.text(
                "Use a broadcast key for every mass mailing so we can later find it in provider logs and internal audits.",
              ),
            ]),
          ],
        ),
        h.div([a.class("space-y-2")], [
          ...
          h.input([
            a.type_("text"),
            a.name("audit_log_reason"),
            a.value(broadcast_reason),
            a.placeholder("Mass user email broadcast"),
            a.class(
              "w-full px-3 py-2 border border-neutral-300 rounded focus:outline-none focus:ring-2 focus:ring-neutral-900 text-sm",
            ),
          ]),
...
pub fn handle_action(
  ...
  case action {
    ...
    option.Some("bulk-send-email-broadcast") -> {
      let subject =
        list.key_find(form_data.values, "subject") |> option.from_result
      let body = list.key_find(form_data.values, "body") |> option.from_result
      let broadcast_key =
        list.key_find(form_data.values, "broadcast_key")
        |> option.from_result
      ...
      let attach_tracking_headers =
        list.key_find(form_data.values, "attach_tracking_headers")
        |> option.from_result
        |> option.is_some
      let audit_log_reason =
        list.key_find(form_data.values, "audit_log_reason")
      ...
      case subject, body {
        option.Some(subject), option.Some(body) -> {
          case
            bulk.bulk_send_email_broadcast(
              ctx,
              session,
              string.trim(subject),
              string.trim(body),
