...
pub fn account_tab(
  ...
  let has_webauthn = list.contains(user.authenticator_types, 2)
  h.div([a.class("space-y-6")], [
    ...
    ui.card(ui.PaddingMedium, [
      ...
      h.div([a.class("flex flex-wrap gap-3")], [
        ...
        case has_webauthn && option.is_some(user.email) {
          True ->
            h.form(
              [
                a.method("POST"),
                a.action("?action=send-passkey-migration-email&tab=account"),
              ],
              [
                h.button(
                  [
                  ...
                  [element.text("Send Passkey Migration Email")],
                ),
              ],
            )
          False -> element.none()
        ...
        case has_webauthn {
          True ->
            h.form(
              [
                a.method("POST"),
                a.action("?action=reset-passkeys&tab=account"),
                a.attribute(
                  "onsubmit",
                  "return confirm('Are you sure you want to remove all passkeys for this user? They will need to sign in again and add a new passkey on astraof.com.')",
                ),
              ],
              [
                h.button(
                  [
                    a.type_("submit"),
                    a.class(
                      "w-full px-4 py-2 bg-red-600 text-white rounded text-sm font-medium hover:bg-red-700 transition-colors text-left",
                    ),
                  ],
                  [element.text("Reset All Passkeys")],
                ),
              ],
            )
          False -> element.none()
