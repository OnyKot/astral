...
import astral_admin/components/icons

import astral_admin/components/ui

import astral_admin/constants

import astral_admin/pages/user_detail/forms
...
fn ai_analysis_widget(user: common.UserLookupResult) {
  let last_active = case user.last_active_at {
    option.Some(c) -> c
    option.None -> ""
  }
  let payload =
  ...
  h.div(
    [
      a.class(
        "rounded-xl border border-indigo-500/30 bg-indigo-950/20 p-4 space-y-3",
      ),
    ],
    [
      h.div([a.class("flex items-center justify-between")], [
        h.div([a.class("flex items-center gap-2")], [
          h.div(
            [
              a.class(
                "w-7 h-7 rounded-lg bg-gradient-to-br from-indigo-500 to-violet-500 flex items-center justify-center text-xs font-bold text-white",
              ),
            ],
            [element.text("AI")],
          ),
          h.span([a.class("font-semibold text-zinc-100")], [
            element.text("ИИ-анализ пользователя"),
          ]),
        ]),
        h.button(
          [
            a.id("ai-analyze-user-btn"),
            a.attribute("data-user-payload", payload),
            a.attribute("type", "button"),
            a.class(
              "px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold transition-colors",
            ),
          ],
          [element.text("Проанализировать")],
        ),
      ]),
      h.div(
        [
          a.id("ai-analyze-user-result"),
          a.class(
            "hidden text-sm text-zinc-200 bg-zinc-950/60 rounded-lg p-3 whitespace-pre-wrap border border-zinc-800",
          ),
        ],
        [],
      ),
      h.script(
        ...
        "
(function(){
  var btn = document.getElementById('ai-analyze-user-btn');
  if (!btn) return;
  var out = document.getElementById('ai-analyze-user-result');
  btn.addEventListener('click', async function(){
    var payload = btn.getAttribute('data-user-payload');
    if (!payload) return;
    out.classList.remove('hidden');
    out.textContent = 'Анализирую с помощью Grok...';
    btn.disabled = true;
    btn.textContent = 'Анализирую...';
    try {
      var basePath = document.documentElement.getAttribute('data-base-path') || '';
      var r = await fetch(basePath + '/api-proxy/ai/analyze-user', {
        method: 'POST',
      ...
      var d = await r.json();
      out.textContent = d.analysis || d.error || 'Нет ответа';
    } catch (e) {
