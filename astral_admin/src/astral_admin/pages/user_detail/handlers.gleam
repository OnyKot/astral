...
pub fn handle_send_passkey_migration_email(
  ctx: Context,
  session: Session,
  user_id: String,
  redirect_url: String,
) -> Response {
  case users.send_passkey_migration_email(ctx, session, user_id) {
    Ok(_) ->
      flash.redirect_with_success(
        ctx,
        redirect_url,
        "Passkey migration email sent successfully",
      )
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        redirect_url,
        "Failed to send passkey migration email",
      )
  }
}

pub fn handle_reset_passkeys(
  ctx: Context,
  session: Session,
  user_id: String,
  redirect_url: String,
) -> Response {
  case users.reset_passkeys(ctx, session, user_id) {
    Ok(_) ->
      flash.redirect_with_success(
        ctx,
        redirect_url,
        "Passkeys reset successfully",
      )
    Error(_) ->
      flash.redirect_with_error(ctx, redirect_url, "Failed to reset passkeys")
  }
}
