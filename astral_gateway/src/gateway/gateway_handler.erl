...
->
    ...
    PresenceUpdate0 = #{status => AdjustedStatus, afk => Afk, mobile => Mobile},
    PresenceUpdate =
        case maps:find(<<"music_activity">>, Data) of
            {ok, MusicActivity} ->
                PresenceUpdate0#{music_activity => MusicActivity};
            error ->
                PresenceUpdate0
        end,

    gen_server:cast(Pid, {presence_update, PresenceUpdate}),
    {ok, State};
