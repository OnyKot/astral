...
get_loaded_modules() ->
    [M || {M, _} <- code:all_loaded(), is_Astral_module(M)].
...
get_changed_modules() ->
    ...
    [M || M <- Modified, is_Astral_module(M), not is_critical_module(M)].

is_critical_module(Module) ->
    lists:member(Module, ?CRITICAL_MODULES).

is_Astral_module(Module) ->
    ModuleStr = atom_to_list(Module),
    lists:prefix("Astral_", ModuleStr) orelse
