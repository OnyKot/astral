...
-module(astral_gateway_env).

-export([load/0, get/1, get_optional/1, get_map/0, patch/1, update/1]).

-define(APP, astral_gateway).
-define(CONFIG_TERM_KEY, {astral_gateway, runtime_config}).
...
build_config() ->
    #{
        ...
        fcm_api_url => env_binary("FCM_API_URL", fcm_api_url, <<"https://fcm.googleapis.com/fcm/send">>),
        ...
        vapid_email => env_binary("VAPID_EMAIL", vapid_email, <<"support@Astral.app">>),
...
-spec env_optional_string_compat(string(), string(), atom()) -> string() | undefined.
env_optional_string_compat(PrimaryEnvVar, FallbackEnvVar, AppKey) when is_atom(AppKey) ->
    case os:getenv(PrimaryEnvVar) of
        false ->
            case os:getenv(FallbackEnvVar) of
                false ->
                    app_env_optional_string(AppKey);
                Value ->
                    Value
