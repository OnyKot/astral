...
init(PresenceData) ->
    ...
    MusicActivity = maps:get(music_activity, PresenceData, null),
    FriendIds =
        case IsBot of
            true -> [];
            false -> FriendIds0
    ...
    State = #{
        ...
        music_activity => normalize_state_music_activity(MusicActivity),
        status => Status,
...
handle_cast({presence_update, Request}, State) ->
    ...
    {FinalRequest, StateWithMusicActivity} = maybe_handle_music_activity(
        UpdatedRequest, StateWithCustomStatus
    ),
    Result = presence_session:handle_presence_update(FinalRequest, StateWithMusicActivity),
    publish_global_if_needed(Result);
...
build_presence_external(State) ->
    ...
    MusicActivity = maps:get(<<"music_activity">>, Payload, null),
    CurrentExternal = #{
...
build_presence_payload(State) ->
    ...
    presence_payload:build(UserData, Status, Mobile, Afk, CustomStatus, MusicActivity).
...
maybe_handle_music_activity(Request, State) ->
    case request_map_find(music_activity, <<"music_activity">>, Request) of
        error ->
            {Request, State};
        {ok, MusicActivity} ->
            Normalized = normalize_state_music_activity(MusicActivity),
            PreviousMusicActivity = maps:get(music_activity, State, null),
            case PreviousMusicActivity =:= Normalized of
                true ->
                    {Request, State};
                false ->
                    {Request, maps:put(music_activity, Normalized, State)}
            end
    end.
...
normalize_state_music_activity(null) ->
    null;
normalize_state_music_activity(Map) when is_map(Map) ->
    Map;
normalize_state_music_activity(_) ->
    null.

request_map_find(AtomKey, BinaryKey, Request) ->
    case maps:find(AtomKey, Request) of
        {ok, Value} ->
            {ok, Value};
        error ->
            maps:find(BinaryKey, Request)
    end.
