...
init([]) ->
    ...
    ApiHost = astral_gateway_env:get(api_host),
    ApiCanaryHost = astral_gateway_env:get(api_canary_host),
    {ok, #{
        sessions => #{},
        api_host => ApiHost,
...
handle_identify_request(
    ...
    case fetch_rpc_data(Request, PeerIP, RpcClient) of
        {ok, Data} ->
            ...
            PresenceMusicActivity = get_presence_music_activity(Presence),
            CustomStatus =
            ...
            SessionData = #{
                ...
                music_activity => PresenceMusicActivity,
                version => Version,
...
check_identify_rate_limit(Attempts) ->
    case astral_gateway_env:get(identify_rate_limit_enabled) of
        true ->
...
-spec get_presence_music_activity(term()) -> map() | null.
get_presence_music_activity(Presence) ->
    case Presence of
        null -> null;
        Map when is_map(Map) ->
            case map_utils:get_safe(Map, <<"music_activity">>, null) of
                MusicActivity when is_map(MusicActivity) -> MusicActivity;
                _ -> null
            end;
        _ -> null
    end.
