...
update_state(Event, EventData, State) ->
    ...
    UpdatedState0 = maps:put(data, UpdatedData, StateWithUpdatedUser),

    %% Voice-channel and voice-member cleanup. Previously these events
    %% only touched the guild's `data` / `sessions` / `presences` maps —
    %% `voice_states` was left stale, leaving the user as a ghost tile in
    %% LiveKit rooms until DTLS timeout. The helpers below force-disconnect
    %% affected participants via RPC and drop their voice states.
    UpdatedState = cleanup_voice_on_event(Event, EventData, UpdatedState0),

    handle_post_update(Event, StateWithUpdatedUser, UpdatedState).

cleanup_voice_on_event(channel_delete, EventData, State) ->
    case maps:get(<<"id">>, EventData, undefined) of
        undefined -> State;
        ChannelIdBin ->
            ChannelId = utils:binary_to_integer_safe(ChannelIdBin),
            case ChannelId of
                0 -> State;
                _ -> guild_voice_disconnect:cleanup_channel_voice_states(ChannelId, State)
            end
    end;
cleanup_voice_on_event(guild_member_remove, EventData, State) ->
    UserId = extract_user_id(EventData),
    case UserId of
        0 -> State;
        _ -> guild_voice_disconnect:cleanup_user_voice_states(UserId, State)
    end;
cleanup_voice_on_event(_Event, _EventData, State) ->
    State.
