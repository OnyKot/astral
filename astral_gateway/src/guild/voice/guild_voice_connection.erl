...
-include_lib("astral_gateway/include/voice_state.hrl").
    ...
    case maps:get(connection_id, Context) of
        undefined ->
            guild_voice_disconnect:disconnect_voice_user(
                #{user_id => maps:get(user_id, Context), connection_id => null}, State
            );
        _ ->
...
handle_update_connection(Context, ChannelIdValue, Member, Channel, VoiceStates, State) ->
    ...
    case maps:get(ConnectionId, VoiceStates, undefined) of
        undefined ->
            %% Stale or unknown connection_id from the client. Treat this as
            %% a fresh join attempt against the requested channel instead of
            %% surfacing an opaque error — the user clicked a voice channel
            %% and expects to join it. Falling through to the new-connection
            %% path makes the gateway request a new LiveKit token.
            handle_new_connection(Context, Member, Channel, VoiceStates, State);
        ExistingVoiceState ->
            ...
            case IsChannelChange of
                true ->
                    %% IMPORTANT: when the user is switching to a *different*
                    %% voice channel we MUST tear down the existing LiveKit
                    %% participant and allocate a brand-new connection. The
                    %% old in-place rewrite of `channel_id` left the LiveKit
                    %% room A connection alive while the gateway pretended
                    %% the user was in room B — voice and UI then diverged
                    %% and "switching channels" appeared to fail until the
                    %% user manually disconnected first. Sequence:
                    %%   1. force-disconnect old participant from old room
                    %%      (also cleans up gateway voice_state[OldConnId])
                    %%   2. fall through to handle_new_connection which
                    %%      requests a new token and registers a new
                    %%      pending voice_state for the target channel
                    UserId = maps:get(user_id, Context),
                    SessionId = maps:get(session_id, Context),
                    {reply, _DiscReply, StateAfterDisconnect} =
                        guild_voice_disconnect:handle_voice_disconnect(
                            ConnectionId,
                            SessionId,
                            UserId,
                    ...
                    NewVoiceStates = voice_state_utils:voice_states(StateAfterDisconnect),
                    handle_new_connection(
                        Context, Member, Channel, NewVoiceStates, StateAfterDisconnect
                    );
                false ->
                    %% Same channel — this is a legitimate in-place update
                    %% for mute / deaf / video / stream / viewer_stream_key.
                    %% No new token required, no LiveKit room change.
...
handle_new_connection(Context, Member, Channel, VoiceStates, State) ->
    ...
    VoiceStatesForLimit = voice_state_utils:drop_voice_states(
        user_voice_states(UserId, VoiceStates), VoiceStates
    ),

    PermCheck = guild_voice_permissions:check_voice_permissions_and_limits(
        UserId, ChannelIdValue, Channel, VoiceStatesForLimit, State, false
    ),
...
user_voice_states(UserId, VoiceStates) ->
    voice_state_utils:filter_voice_states(VoiceStates, fun(_, VoiceState) ->
        voice_state_utils:voice_state_user_id(VoiceState) =:= UserId
    end).

disconnect_stale_user_voice_connections(UserId, State) ->
    VoiceStates = voice_state_utils:voice_states(State),
    StaleVoiceStates = user_voice_states(UserId, VoiceStates),
    PendingConnections = pending_voice_connections(State),
    NewPendingConnections = maps:filter(
        fun(_ConnectionId, PendingData) ->
            pending_get_integer(PendingData, user_id) =/= UserId
        end,
        PendingConnections
    ),
    StateWithoutPending = maps:put(pending_voice_connections, NewPendingConnections, State),

    case maps:size(StaleVoiceStates) of
        0 ->
            StateWithoutPending;
        StaleCount ->
            logger:info(
                "[guild_voice_connection] Replacing ~p stale voice connection(s) for user_id=~p",
                [StaleCount, UserId]
            ),
            GuildId = map_utils:get_integer(State, id, undefined),
            maps:foreach(
                fun(ConnectionId, VoiceState) ->
                    force_disconnect_stale_connection(GuildId, ConnectionId, VoiceState)
                end,
                StaleVoiceStates
            ),
            NewVoiceStates = voice_state_utils:drop_voice_states(StaleVoiceStates, VoiceStates),
            NewState = maps:put(voice_states, NewVoiceStates, StateWithoutPending),
            voice_state_utils:broadcast_disconnects(StaleVoiceStates, NewState),
            NewState
    end.

force_disconnect_stale_connection(undefined, _ConnectionId, _VoiceState) ->
    ok;
force_disconnect_stale_connection(GuildId, ConnectionId, VoiceState) ->
    ChannelId = voice_state_utils:voice_state_channel_id(VoiceState),
    UserId = voice_state_utils:voice_state_user_id(VoiceState),
    case {ChannelId, UserId} of
        {undefined, _} ->
            ok;
        {_, undefined} ->
            ok;
        _ ->
            spawn(fun() ->
                _ = guild_voice_disconnect:force_disconnect_participant(
        ...
        {ok, GuildId, GuildIdBin} ->
            ...
            case request_voice_token(GuildId, ChannelIdValue, UserId, VoicePermissions) of
                {ok, TokenData} ->
                    ...
                    StateWithSingleUserConnection =
                        disconnect_stale_user_voice_connections(UserId, State),
                    VoiceStates = voice_state_utils:voice_states(StateWithSingleUserConnection),
                    NewVoiceStates = maps:put(ConnectionId, VoiceState, VoiceStates),
                    StateWithVoiceStates = maps:put(
                        voice_states, NewVoiceStates, StateWithSingleUserConnection
                    ),
