...
handle_dm_disconnect(undefined, UserId, _VoiceStates, State) ->
    disconnect_voice_user(UserId, State);
handle_dm_disconnect(null, UserId, _VoiceStates, State) ->
    disconnect_voice_user(UserId, State);
handle_dm_disconnect(ConnectionId, _UserId, VoiceStates, State) ->
    case maps:get(ConnectionId, VoiceStates, undefined) of
        ...
        OldVoiceState ->
            force_disconnect_dm_connection(ConnectionId, OldVoiceState),
...
handle_dm_token_success(
    ...
    StateWithSingleUserConnection = disconnect_existing_dm_user_connections(UserId, State),
    VoiceStates = maps:get(dm_voice_states, StateWithSingleUserConnection, #{}),
    NewVoiceStates = maps:put(ConnectionId, VoiceState, VoiceStates),
    NewState = maps:put(dm_voice_states, NewVoiceStates, StateWithSingleUserConnection),
...
disconnect_existing_dm_user_connections(UserId, State) ->
    VoiceStates = maps:get(dm_voice_states, State, #{}),
    UserVoiceStates = maps:filter(
        fun(_ConnectionId, VoiceState) ->
            map_utils:get_integer(VoiceState, <<"user_id">>, undefined) =:= UserId
        end,
        VoiceStates
    ),
    case maps:size(UserVoiceStates) of
        ...
        StaleCount ->
            logger:info(
                "[dm_voice] Replacing ~p stale DM voice connection(s) for user_id=~p",
                [StaleCount, UserId]
            ),
            NewVoiceStates = maps:fold(
                fun(ConnectionId, VoiceState, Acc) ->
                    force_disconnect_dm_connection(ConnectionId, VoiceState),
            ...
            NewState = maps:put(dm_voice_states, NewVoiceStates, State),
            broadcast_dm_disconnects(UserVoiceStates, NewState),
            NewState
    end.
    ...
    case {ChannelId, UserId} of
        ...
        _ ->
            Req = voice_utils:build_force_disconnect_rpc_request(
                null, ChannelId, UserId, ConnectionId
            ),
            spawn(fun() ->
                _ = rpc_client:call(Req),
                ok
            end),
            ok
    end.

broadcast_dm_disconnects(VoiceStates, State) ->
    maps:foreach(
        fun(ConnectionId, VoiceState) ->
            ...
            case validation:validate_snowflake(<<"channel_id">>, ChannelId) of
                {ok, ChannelIdInt} ->
                    broadcast_voice_state_update(ChannelIdInt, DisconnectVoiceState, State);
                {error, _, Reason} ->
                    logger:warning("[dm_voice] Invalid channel_id in stale voice state: ~p", [
                        Reason
                    ]),
                    ok
            end
...
disconnect_voice_user(UserId, State) ->
    ...
    case maps:size(UserVoiceStates) of
        ...
        _ ->
            ...
            maps:foreach(
                fun(_ConnectionId, VoiceState) ->
                    force_disconnect_dm_connection(_ConnectionId, VoiceState),
                    ChannelId = maps:get(<<"channel_id">>, VoiceState, null),
                    DisconnectVoiceState = maps:put(
                        <<"channel_id">>,
                        null,
