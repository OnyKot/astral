...
-export([cleanup_channel_voice_states/2]).
-export([cleanup_user_voice_states/2]).
                ...
                _ ->
                    ...
                    maps:foreach(
                        fun(ConnectionId, VoiceState) ->
                            force_disconnect_from_bulk(GuildId, ConnectionId, VoiceState)
                        end,
                        UserVoiceStates
                    ),
                ...
                VoiceState ->
                    case voice_state_utils:voice_state_user_id(VoiceState) of
                        ...
                        VoiceStateUserId when VoiceStateUserId =:= UserId ->
                            GuildId = maps:get(id, State, undefined),
                            force_disconnect_from_bulk(GuildId, SpecificConnection, VoiceState),
...
disconnect_all_voice_users_in_channel(#{channel_id := ChannelId}, State) ->
    ...
    case maps:size(ChannelVoiceStates) of
        ...
        Count ->
            %% Before dropping the local state, force-disconnect every
            %% participant from LiveKit. Previously this path only
            %% broadcast VOICE_STATE_UPDATE and relied on the client to
            %% tear down its own LiveKit session, leaving ghost tiles
            %% for the full DTLS timeout if the client was slow or dead.
...
%% Fire-and-forget RPC force-disconnect for bulk-disconnect / channel-delete
%% / guild-member-remove paths. Non-fatal on failure (at worst we fall back
%% to the pre-fix behavior of waiting for LiveKit DTLS timeout).
force_disconnect_from_bulk(undefined, _ConnectionId, _VoiceState) ->
    ok;
force_disconnect_from_bulk(GuildId, ConnectionId, VoiceState) ->
    ChannelIdVal = voice_state_utils:voice_state_channel_id(VoiceState),
    UserId = voice_state_utils:voice_state_user_id(VoiceState),
    case {ChannelIdVal, UserId} of
        {undefined, _} ->
            ok;
        {_, undefined} ->
            ok;
        _ ->
            Req = voice_utils:build_force_disconnect_rpc_request(
                GuildId, ChannelIdVal, UserId, ConnectionId
            ),
            spawn(fun() ->
                case rpc_client:call(Req) of
                    ...
                    {error, Reason} ->
                        logger:warning(
                            "[guild_voice_disconnect] bulk force-disconnect RPC failed (non-fatal): ~p",
                            [Reason]
                        )
                end
            end),
...
%% Called from guild_state when a voice channel is deleted. Finds every
%% voice_state bound to the deleted channel, fires a fire-and-forget
%% force_disconnect RPC per participant, drops the states, and broadcasts
%% a null-channel VOICE_STATE_UPDATE so clients stop rendering the tile.
-spec cleanup_channel_voice_states(integer(), guild_state()) -> guild_state().
cleanup_channel_voice_states(ChannelId, State) ->
    VoiceStates = voice_state_utils:voice_states(State),
    ChannelVoiceStates = voice_state_utils:filter_voice_states(VoiceStates, fun(_, V) ->
        voice_state_utils:voice_state_channel_id(V) =:= ChannelId
    end),
...
%% Called from guild_state when a member is removed (leave/kick/ban).
%% Same pattern as cleanup_channel_voice_states but filters by user id —
%% lets us disconnect a member from every voice channel in the guild they
%% currently hold a session in.
-spec cleanup_user_voice_states(integer(), guild_state()) -> guild_state().
cleanup_user_voice_states(UserId, State) ->
    VoiceStates = voice_state_utils:voice_states(State),
    UserVoiceStates = voice_state_utils:filter_voice_states(VoiceStates, fun(_, V) ->
        voice_state_utils:voice_state_user_id(V) =:= UserId
    end),
