...
import {extractURLs, idnaEncodeURL, isAstralAppExcludedURL, isValidURL} from './UnfurlerUtils';
...
describe('UnfurlerUtils', () => {
	...
	describe('isAstralAppExcludedURL', () => {
		it('should return true for invite shortlink domain', () => {
			expect(isAstralAppExcludedURL('https://asrtal.ru/abc123')).toBe(true);
			expect(isAstralAppExcludedURL('https://asrtal.ru/')).toBe(true);
			expect(isAstralAppExcludedURL('http://asrtal.ru/test')).toBe(true);
		});

		it('should return true for gift shortlink domain', () => {
			expect(isAstralAppExcludedURL('https://asrtal.ru/xyz789')).toBe(true);
			expect(isAstralAppExcludedURL('https://asrtal.ru/')).toBe(true);
			expect(isAstralAppExcludedURL('http://asrtal.ru/test')).toBe(true);
		});
		...
		it('should return true for all web app URLs', () => {
			expect(isAstralAppExcludedURL('https://asrtal.ru/')).toBe(true);
			expect(isAstralAppExcludedURL('https://asrtal.ru/login')).toBe(true);
			expect(isAstralAppExcludedURL('https://asrtal.ru/channels/@me')).toBe(true);
			expect(isAstralAppExcludedURL('https://asrtal.ru/any/path')).toBe(true);
		});

		it('should return false for marketing site URLs', () => {
			expect(isAstralAppExcludedURL('https://asrtal.ru/')).toBe(false);
			expect(isAstralAppExcludedURL('https://asrtal.ru/about')).toBe(false);
			expect(isAstralAppExcludedURL('https://asrtal.ru/blog')).toBe(false);
			expect(isAstralAppExcludedURL('https://asrtal.ru/docs')).toBe(false);
		});

		it('should treat marketing channel paths as Astral app URLs', () => {
			expect(isAstralAppExcludedURL('https://asrtal.ru/channels/@me')).toBe(true);
			expect(isAstralAppExcludedURL('https://asrtal.ru/channels/general')).toBe(true);
		});

		it('should return false for different hosts', () => {
			expect(isAstralAppExcludedURL('https://example.com/login')).toBe(false);
			expect(isAstralAppExcludedURL('https://other.com/invite/abc')).toBe(false);
		});

		it('should handle URLs with query strings and fragments', () => {
			expect(isAstralAppExcludedURL('https://asrtal.ru/login?redirect=/home')).toBe(true);
			expect(isAstralAppExcludedURL('https://asrtal.ru/abc?ref=email')).toBe(true);
			expect(isAstralAppExcludedURL('https://asrtal.ru/xyz#promo')).toBe(true);
		});

		it('should handle invalid URLs gracefully', () => {
			expect(isAstralAppExcludedURL('not-a-url')).toBe(false);
			expect(isAstralAppExcludedURL('')).toBe(false);
		});
	});
	...
	describe('extractURLs', () => {
		...
		it('should filter out invite URLs', () => {
			const text = 'Visit https://example.com but not https://asrtal.ru/test123';
			const result = extractURLs(text);

			expect(result).toContain('https://example.com/');
			expect(result).not.toContain('https://asrtal.ru/test123');
		});
		...
		it('should handle complex mixed content', () => {
			const text = `
				...
				Don't forget https://asrtal.ru/abc123 (invite link).
			...
			expect(result).not.toContain('https://asrtal.ru/abc123');
		});

		it('should filter out all web app URLs', () => {
			const text = `
				Visit https://example.com and https://asrtal.ru/login
				Check out https://asrtal.ru/channels/@me/123
				Also see https://asrtal.ru/about which is allowed
			`;
			const result = extractURLs(text);

			expect(result).toContain('https://example.com/');
			expect(result).toContain('https://asrtal.ru/about');
			expect(result).not.toContain('https://asrtal.ru/login');
			expect(result).not.toContain('https://asrtal.ru/channels/@me/123');
		});

		it('should filter invite and gift shortlinks', () => {
			const excludedUrls = ['https://asrtal.ru/code', 'https://asrtal.ru/code', 'https://asrtal.ru/any/path'];
