...
describe('SnowflakeUtils', () => {
	describe('getSnowflake', () => {
		...
		it('should handle ASTRAL_EPOCH correctly', () => {
			const snowflake = getSnowflake(ASTRAL_EPOCH);
	...
	describe('extractTimestamp', () => {
		...
		it('should handle zero snowflake', () => {
			...
			expect(extractedTimestamp).toBe(ASTRAL_EPOCH);
		});
	...
	describe('roundtrip conversion', () => {
		it('should maintain timestamp integrity through conversion', () => {
			const timestamps = [ASTRAL_EPOCH, ASTRAL_EPOCH + 1000, Date.now(), Date.now() + 86400000];
