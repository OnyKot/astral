...
export class ChannelOperationsService {
	...
	async updateChannelPositionsByList(params: {
		...
		try {
			...
			for (const update of updates) {
				if (!viewableChannelIds.has(update.channelId)) {
					throw InputValidationError.create('id', 'Channel not found');
				}
				if (update.parentId && !viewableChannelIds.has(update.parentId)) {
					throw InputValidationError.create('parent_id', 'Invalid parent channel');
				}
				if (update.precedingSiblingId && !viewableChannelIds.has(update.precedingSiblingId)) {
					throw InputValidationError.create('preceding_sibling_id', 'Invalid preceding sibling channel');
				}
			}

			for (const update of updates) {
				if (update.precedingSiblingId !== undefined) {
					await this.applyExplicitReorderUpdate({
						guildId,
						userId,
						update,
						requestCache,
	...
	private async applyExplicitReorderUpdate(params: {
		guildId: GuildID;
		userId: UserID;
		update: {
			channelId: ChannelID;
		...
		await this.executeChannelReorder({
			...
			operation: {
				...
				precedingSiblingId: update.precedingSiblingId ?? null,
			},
			requestCache,
		});
	...
	private async executeChannelReorder(params: {
		...
		insertIndex = ChannelHelpers.normalizeGuildChannelInsertIndex({
			remainingChannels,
			insertIndex,
			targetChannel,
			desiredParentId,
