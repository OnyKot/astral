...
export class GuildRoleService {
	...
	private async updateRolePositionsByList(params: {
		...
		const currentIndexByRoleId = new Map(currentOrder.map((role, index) => [role.id, index]));
		...
		const targetOrder = [...currentOrder].sort((a, b) => {
			...
			return (currentIndexByRoleId.get(a.id) ?? 0) - (currentIndexByRoleId.get(b.id) ?? 0);
		});
	...
	private async updateRolePositionsLocked(params: {
		...
		if (!customOrder && insertIndex < originalIndex) {
		...
		const finalOrder = customOrder
			? (() => {
					...
					const normalized = customOrder.filter((roleId) => {
						if (roleId === everyoneRoleId || seen.has(roleId) || !roleMap.has(roleId)) {
							return false;
						}
						seen.add(roleId);
						return true;
					...
					for (const role of sortedRoles) {
						if (role.id !== everyoneRoleId && !seen.has(role.id)) {
