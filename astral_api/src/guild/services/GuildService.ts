...
import {AuditLogActionType, GuildFeatures, GuildOperations, JoinSourceTypes, Permissions} from '~/Constants';
import type {
...
import type {GuildJoinRequestRepository} from '../repositories/GuildJoinRequestRepository';
...
import {GuildDiscoveryService} from './GuildDiscoveryService';
import {GuildMemberService} from './GuildMemberService';
import {GuildModerationService} from './GuildModerationService';
import type {GuildDiscoveryCategoryId} from './data/GuildOperationsService';
...
interface GuildJoinRequestResponse {
	guild_id: string;
	user_id: string;
...
const mapGuildJoinRequestToResponse = (row: GuildJoinRequestRow): GuildJoinRequestResponse => ({
...
export class GuildService {
	...
	public readonly discoveryApplications: GuildDiscoveryService;
	private readonly guildRepository: IGuildRepository;
	private readonly guildJoinRequestRepository: GuildJoinRequestRepository;
	...
	constructor(
		...
		discoveryApplicationRepository: GuildDiscoveryApplicationRepository,
		guildJoinRequestRepository: GuildJoinRequestRepository,
		grokService: IGrokService,
	) {
		this.gatewayService = gatewayService;
		this.guildRepository = guildRepository;
		this.guildJoinRequestRepository = guildJoinRequestRepository;
		...
		this.data = new GuildDataService(
			...
			discoveryApplicationRepository,
			grokService,
		);
		...
		this.discoveryApplications = new GuildDiscoveryService(
			guildRepository,
			discoveryApplicationRepository,
			gatewayService,
		);
	...
	async getGuildDiscovery(params: {
		...
		limit: number;
		offset: number;
		sortBy: 'relevance' | 'memberCount' | 'createdAt' | 'trending';
		sortOrder: 'asc' | 'desc';
		category: GuildDiscoveryCategoryId;
	}): Promise<GuildDiscoveryResponse> {
		return this.data.getGuildDiscovery(params);
	}

	async joinDiscoverableGuild(params: {
		userId: UserID;
		guildId: GuildID;
		...
		if (!guild.features.has(GuildFeatures.DISCOVERY_APPROVED) || guild.features.has(GuildFeatures.DISCOVERY_DISABLED)) {
	...
	async submitDiscoveryJoinRequest(params: {
		userId: UserID;
		guildId: GuildID;
		message: string | null;
		...
		if (!guild.features.has(GuildFeatures.INVITES_DISABLED)) {
			throw InputValidationError.create('guild_id', 'This guild is open. Join it directly from Discovery.');
		}
		...
		await this.checkUserBanStatus({userId, guildId});

		const existingMember = await this.guildRepository.getMember(guildId, userId);
		if (existingMember) {
			throw InputValidationError.create('guild_id', 'You are already a member of this guild');
		}

		const existingRequest = await this.guildJoinRequestRepository.findByGuildAndUser(guildId, userId);
		if (existingRequest?.status === 'pending') {
			return mapGuildJoinRequestToResponse(existingRequest);
		}

		const row: GuildJoinRequestRow = {
			guild_id: guildId,
			user_id: userId,
			status: 'pending',
		...
		await this.guildJoinRequestRepository.upsert(row);
		return mapGuildJoinRequestToResponse(row);
	}

	async listGuildJoinRequests(params: {
		userId: UserID;
		guildId: GuildID;
		status?: GuildJoinRequestStatus;
	}): Promise<{requests: Array<GuildJoinRequestResponse>}> {
		const {userId, guildId, status} = params;
		await this.requireManageGuild(userId, guildId);
		const rows = await this.guildJoinRequestRepository.listByGuild(guildId, status);
		return {
			requests: rows.map(mapGuildJoinRequestToResponse),
		};
	}

	async reviewGuildJoinRequest(params: {
		userId: UserID;
		guildId: GuildID;
		targetId: UserID;
		approved: boolean;
		...
		const {userId, guildId, targetId, approved, reviewNote, requestCache} = params;
		await this.requireManageGuild(userId, guildId);

		const existingRequest = await this.guildJoinRequestRepository.findByGuildAndUser(guildId, targetId);
		if (!existingRequest || existingRequest.status !== 'pending') {
			throw InputValidationError.create('user_id', 'No pending join request exists for this user');
		}
		...
		const status: GuildJoinRequestStatus = approved ? 'approved' : 'rejected';
		await this.guildJoinRequestRepository.setStatus({
			guildId,
			userId: targetId,
			status,
			reviewerId: userId,
		...
		return {
			...mapGuildJoinRequestToResponse(existingRequest),
			status,
			reviewed_by: userId.toString(),
	...
	async updateDiscoveryVisibilityFeature(
		params: {userId: UserID; guildId: GuildID; enabled: boolean; requestCache: RequestCache},
		auditLogReason?: string | null,
	): Promise<GuildResponse> {
		const {userId, guildId, enabled, requestCache} = params;
		...
		const featuresSet = new Set(currentGuild.features);
		if (enabled) {
			featuresSet.delete(GuildFeatures.DISCOVERY_DISABLED);
		} else {
			featuresSet.add(GuildFeatures.DISCOVERY_DISABLED);
		}
		const featuresPayload = Array.from(featuresSet);
