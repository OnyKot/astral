...
import {AstralAPIError} from '~/Errors';
...
export class AuthService implements IAuthService {
	...
	async resendVerificationEmail(user: User, request: Request): Promise<void> {
		return this.emailService.resendVerificationEmail(user, request);
	}
	...
	async recreateAuthSessionAndTerminateOthers(params: RecreateAuthSessionParams): Promise<void> {
		return this.sessionService.recreateAuthSessionAndTerminateOthers(params);
	}
	...
	async getUserSession(_requestCache: RequestCache, token: string): Promise<AuthSession> {
		...
		if (!session) {
			throw new AstralAPIError({code: APIErrorCodes.UNAUTHORIZED, message: 'Invalid session token', status: 401});
		}
		return session;
	}
	...
	async resendIpAuthorization(ticket: string, request: Request): Promise<{retryAfter?: number}> {
		return this.loginService.resendIpAuthorization(ticket, request);
	}
