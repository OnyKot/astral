...
import {createStringType, PhoneNumberType, SudoVerificationSchema, z} from '~/Schema';
...
export const AuthController = (app: HonoApp) => {
	...
	app.post(
		...
		async (ctx) => {
			...
			await ctx.get('authService').sendPhoneVerificationCode(phone, null);
	...
	app.post(
		...
		async (ctx) => {
			...
			const phoneToken = await ctx.get('authService').verifyPhoneCode(phone, code, null);
	...
	app.post(
		...
		async (ctx) => {
			...
			await ctx.get('authService').resendVerificationEmail(user, ctx.req.raw);
			return ctx.body(null, 204);
		},
	);
	...
	app.post(
		...
		async (ctx) => {
			const {ticket} = ctx.req.valid('json');
			await ctx.get('authService').resendIpAuthorization(ticket, ctx.req.raw);
			return ctx.body(null, 204);
		},
	);
	...
	app.post('/auth/handoff/initiate', RateLimitMiddleware(RateLimitConfigs.AUTH_HANDOFF_INITIATE), async (ctx) => {
		...
		return ctx.json({
			...
			control_token: result.controlToken,
		});
	});
	...
	app.get(
		...
		async (ctx) => {
			...
			const controlToken = ctx.req.header('X-Astral-Handoff-Control');
			const result = await ctx.get('desktopHandoffService').getHandoffStatus(code, controlToken);
			return ctx.json({
	...
	app.delete(
		...
		async (ctx) => {
			const {code} = ctx.req.valid('param');
			const controlToken = ctx.req.header('X-Astral-Handoff-Control');
			await ctx.get('desktopHandoffService').cancelHandoff(code, controlToken);
			return ctx.body(null, 204);
		},
	);
};
