...
export class AuthLoginService {
	...
	async completeIpAuthorization(token: string): Promise<{token: string; user_id: string; ticket: string}> {
		...
		if (payload.platform) {
			headers['x-Astral-platform'] = payload.platform;
		}

		const syntheticRequest = new Request(`${Config.endpoints.apiPublic}/auth/ip-authorization`, {
			headers,
			method: 'POST',
		});
	...
	> {
		...
		if (!hasMfa && !isAppStoreReviewer) {
			...
			if (!isIpAuthorized) {
				...
				const platform = request.headers.get('x-Astral-platform');
