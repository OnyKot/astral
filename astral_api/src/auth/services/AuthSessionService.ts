...
interface RecreateAuthSessionParams {
	user: User;
...
const encodeSessionIdHash = (hash: Uint8Array): string => Buffer.from(hash).toString('base64url');
...
export class AuthSessionService {
	...
	async createAuthSession({user, request}: CreateAuthSessionParams): Promise<[token: string, AuthSession]> {
		...
		const platformHeader = request.headers.get('x-Astral-platform')?.trim().toLowerCase() ?? null;
	...
	async recreateAuthSessionAndTerminateOthers({
		user,
		oldAuthSession: _oldAuthSession,
		request,
	}: RecreateAuthSessionParams): Promise<void> {
		const authSessions = await this.repository.listAuthSessions(user.id);
		const hashesToTerminate = authSessions.map((session) => session.sessionIdHash);
		const sessionIdHashesToTerminate = authSessions.map((session) => encodeSessionIdHash(session.sessionIdHash));

		if (hashesToTerminate.length > 0) {
			await this.repository.deleteAuthSessions(user.id, hashesToTerminate);

			await this.gatewayService.terminateSession({
				userId: user.id,
				sessionIdHashes: sessionIdHashesToTerminate,
			});
		}

		await this.createAuthSession({user, request});
	}
