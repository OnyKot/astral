...
export class AuthPhoneService {
	...
	async sendPhoneVerificationCode(phone: string, userId: UserID | null): Promise<void> {
		...
		Logger.info(
			{userId: userId?.toString() ?? null, phone: this.maskPhone(phone)},
	...
	async addPhoneToAccount(userId: UserID, phoneToken: string): Promise<void> {
		...
		Logger.info({userId: userId.toString(), phone: this.maskPhone(tokenData.phone)}, 'Adding phone to account');
	...
	async removePhoneFromAccount(userId: UserID): Promise<void> {
		...
		if (Config.sms.enabled) {
			throw new AccessDeniedError('Phone number is required for all accounts');
		}
