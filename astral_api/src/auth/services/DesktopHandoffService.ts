...
function generateHandoffControlToken(): string {
...
function isValidHandoffCode(code: string): boolean {
	return NORMALIZED_CODE_REGEX.test(code);
}

function controlTokenMatches(expected: string, provided?: string): boolean {
	if (!provided) {
		return false;
	}
...
export class DesktopHandoffService {
	...
	async initiateHandoff(userAgent?: string): Promise<{code: string; expiresAt: Date; controlToken: string}> {
		const code = generateHandoffCode();
		const normalizedCode = normalizeHandoffCode(code);
		const controlToken = generateHandoffControlToken();
		...
		return {code, expiresAt, controlToken};
	}
	...
	async cancelHandoff(code: string, controlToken?: string): Promise<void> {
		const normalizedCode = normalizeHandoffCode(code);
		if (!isValidHandoffCode(normalizedCode)) {
			return;
		}
