...
interface ResolvedRegistrationPhone {
	phone: string;
	token: string;
}
...
export class AuthRegistrationService {
	...
	async register({
		...
		const resolvedPhone = await this.resolveRegistrationPhone(data.phone_token ?? null);
		const registrationInviteCode = (data.invite_code ?? Config.instance.autoJoinInviteCode ?? '').trim() || null;
		...
		if (Config.instance.requireInviteCodeForRegistration && !registrationInviteCode) {
			throw InputValidationError.create('invite_code', 'Invite code is required');
		}

		if (registrationInviteCode && this.inviteService) {
			try {
				await this.inviteService.getInvite(createInviteCode(registrationInviteCode));
			} catch {
				throw InputValidationError.create('invite_code', 'Invalid invite code');
			}
		}
		...
		if (resolvedPhone) {
			const phoneTaken = await this.repository.findByPhone(resolvedPhone.phone);
			if (phoneTaken) throw InputValidationError.create('phone_token', 'Phone already in use');
		}
		...
		let flags = Config.nodeEnv === 'development' && Config.dev.grantStaffOnRegistration ? UserFlags.STAFF : 0n;
		...
		if (resolvedPhone) {
			await this.repository.deletePhoneToken(createPhoneVerificationToken(resolvedPhone.token));
		}
		...
		if (rawEmail) await this.maybeSendVerificationEmail({user, email: rawEmail, request});
	...
	private async maybeSendVerificationEmail(params: {user: User; email: string; request: Request}): Promise<void> {
	...
	private async resolveRegistrationPhone(phoneToken: string | null): Promise<ResolvedRegistrationPhone | null> {
		if (!phoneToken) {
			return null;
		}

		const token = createPhoneVerificationToken(phoneToken);
		const tokenData = await this.repository.getPhoneToken(token);
		if (!tokenData || tokenData.user_id !== null) {
			throw InputValidationError.create('phone_token', 'Invalid or expired phone verification');
		}
	...
	private async buildRegistrationMetadataContext(params: {
		...
		const AstralTag = `${user.username}#${user.discriminator.toString()}`;
	...
	private async sendRegistrationWebhook(
		...
		const embedFields = [
			...
			{name: 'AstralTag', value: context.AstralTag, inline: true},
