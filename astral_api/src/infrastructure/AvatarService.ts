...
export class AvatarService {
	...
	async uploadAvatar(params: {
		...
		const previousHashStripped = previousKey ? stripAnimationPrefix(previousKey) : null;

		await this.storageService.uploadAvatar({prefix, key: `${fullKeyPath}/${imageHashShort}`, body: imageBuffer});

		// Avoid deleting the object we just uploaded when hash hasn't changed.
		if (previousKey && previousHashStripped !== imageHashShort) {
			await this.storageService.deleteAvatar({prefix, key: `${fullKeyPath}/${stripAnimationPrefix(previousKey)}`});
		}
