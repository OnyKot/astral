...
describe('EmailI18nService', () => {
	...
	describe('Template Rendering', () => {
		describe('passwordReset', () => {
			const variables = {
				username: 'testuser',
				resetUrl: 'https://asrtal.ru/reset/abc123',
			};

			it('should render in en-US', () => {
				const result = service.getTemplate('passwordReset', 'en-US', variables);
				expect(result.subject).toBe('Reset your Astral password');
				expect(result.body).toContain('Hello testuser');
				expect(result.body).toContain('https://asrtal.ru/reset/abc123');
		...
		describe('emailVerification', () => {
			const variables = {
				username: 'newuser',
				verifyUrl: 'https://asrtal.ru/verify/xyz789',
			};

			it('should render in en-US', () => {
				const result = service.getTemplate('emailVerification', 'en-US', variables);
				expect(result.subject).toBe('Verify your Astral email address');
				expect(result.body).toContain('Hello newuser');
				expect(result.body).toContain('https://asrtal.ru/verify/xyz789');
		...
		describe('ipAuthorization', () => {
			const variables = {
				username: 'secureuser',
				authUrl: 'https://asrtal.ru/auth/ip123',
			...
			it('should render in en-US', () => {
				...
				expect(result.body).toContain('https://asrtal.ru/auth/ip123');
			});
		...
		describe('passkeyMigrationNotice', () => {
			const variables = {
				username: 'passkeyuser',
				loginUrl: 'https://astraof.com/login',
				securityUrl: 'https://astraof.com/channels/@me',
				legacyDomain: 'asrtal.ru',
				newDomain: 'astraof.com',
			};

			it('should render in en-US', () => {
				const result = service.getTemplate('passkeyMigrationNotice', 'en-US', variables);
				expect(result.subject).toContain('astraof.com');
				expect(result.body).toContain('passkeyuser');
				expect(result.body).toContain('https://astraof.com/login');
				expect(result.body).toContain('asrtal.ru');
			});
		});

		describe('passkeyResetNotice', () => {
			const variables = {
				username: 'passkeyuser',
				loginUrl: 'https://astraof.com/login',
				securityUrl: 'https://astraof.com/channels/@me',
				forgotUrl: 'https://astraof.com/forgot',
				newDomain: 'astraof.com',
			};

			it('should render in en-US', () => {
				const result = service.getTemplate('passkeyResetNotice', 'en-US', variables);
				expect(result.subject).toContain('removed');
				expect(result.body).toContain('passkeyuser');
				expect(result.body).toContain('https://astraof.com/forgot');
				expect(result.body).toContain('astraof.com');
			});
		});
		...
		describe('accountTempBanned', () => {
			...
			it('should render in en-US with plural hours', () => {
				const variables = {
					...
					durationHours: 24,
					bannedUntil,
					termsUrl: 'https://asrtal.ru/terms',
					guidelinesUrl: 'https://asrtal.ru/guidelines',
				};

				const result = service.getTemplate('accountTempBanned', 'en-US', variables);
				expect(result.subject).toBe('Your Astral account has been temporarily suspended');
		...
		describe('accountScheduledDeletion', () => {
			...
			it('should render in en-US', () => {
				...
				expect(result.subject).toBe('Your Astral account is scheduled for deletion');
				expect(result.body).toContain('Hello deleteduser');
				expect(result.body).toContain('Repeated violations');
				expect(result.body).toContain('appeals@astraof.com');
			});
		...
		describe('selfDeletionScheduled', () => {
			...
			it('should render in en-US', () => {
				...
				expect(result.subject).toBe('Your Astral account deletion has been scheduled');
		...
		describe('inactivityWarning', () => {
			...
			it('should render in en-US', () => {
				const variables = {
					...
					deletionDate,
					lastActiveDate,
					loginUrl: 'https://asrtal.ru/login',
				};

				const result = service.getTemplate('inactivityWarning', 'en-US', variables);
				expect(result.subject).toBe('Your Astral account will be deleted due to inactivity');
				expect(result.body).toContain('Hello inactiveuser');
				expect(result.body).toContain('over 2 years');
				expect(result.body).toContain('https://asrtal.ru/login');
			});
		...
		describe('harvestCompleted', () => {
			...
			it('should render in en-US', () => {
				const variables = {
					username: 'datauser',
					downloadUrl: 'https://asrtal.ru/download/abc123',
					totalMessages: 12345,
					fileSizeMB: 456,
					expiresAt,
				};
				...
				expect(result.subject).toBe('Your Astral Data Export is Ready');
				expect(result.body).toContain('Hello datauser');
				expect(result.body).toContain('12,345');
				expect(result.body).toContain('456 MB');
				expect(result.body).toContain('https://asrtal.ru/download/abc123');
			});
		...
		describe('unbanNotification', () => {
			it('should render in en-US', () => {
				...
				expect(result.subject).toBe('Your Astral account suspension has been lifted');
		...
		describe('giftChargebackNotification', () => {
			it('should render in en-US', () => {
				...
				expect(result.subject).toBe('Your Astral Premium gift has been revoked');
		...
		describe('reportResolved', () => {
			it('should render in en-US', () => {
				...
				expect(result.subject).toBe('Your Astral report has been reviewed');
		...
		describe('registrationApproved', () => {
			it('should render in en-US', () => {
				const variables = {
					...
					channelsUrl: 'https://asrtal.ru/channels',
				};

				const result = service.getTemplate('registrationApproved', 'en-US', variables);
				expect(result.subject).toBe('Your Astral registration has been approved');
				expect(result.body).toContain('Hello newapproveduser');
				expect(result.body).toContain('Great news');
				expect(result.body).toContain('https://asrtal.ru/channels');
			});
	...
	describe('Locale handling', () => {
		it('should fall back to en-US for unsupported locale', () => {
			const variables = {
				username: 'testuser',
				resetUrl: 'https://asrtal.ru/reset',
			};
	...
	describe('All templates coverage', () => {
		...
		it('should have tests for all 20 templates', () => {
