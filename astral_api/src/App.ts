...
import {initializeAdminBootstrap} from '~/admin/init';
...
import {createLegalHoldPreviewResponse, LegalHoldMiddleware} from '~/middleware/LegalHoldMiddleware';
...
import {ApplicationCommandController} from '~/oauth/ApplicationCommandController';
...
import type {ApplicationCommandService} from './oauth/ApplicationCommandService';
...
import type {CloudPaymentsService} from './payments/cloudpayments/CloudPaymentsService';
import type {IntellectMoneyService} from './payments/intellectmoney/IntellectMoneyService';
import type {TBankService} from './payments/tbank/TBankService';
...
export interface HonoEnv {
	Variables: {
		...
		intellectmoneyService?: IntellectMoneyService;
...
if (Config.nodeEnv === 'production') {
	routes.use('*', async (ctx, next) => {
		...
		if (ctx.req.method !== 'GET' && isKnownAppHost(host) && !isServerToServerWebhookPath(path)) {
			/*
			 * Bots and external integrations send Bearer/Bot tokens but
			 * typically don't send an Origin header at all — node-fetch
			 * / undici / curl, etc. Only skip the browser origin gate
			 * when the Authorization header uses a supported token
			 * scheme; unsupported schemes still must pass Origin checks
			 * to prevent cookie-authenticated CSRF.
			 */
			const authorization = ctx.req.header('authorization')?.trim();
			const isTokenAuth =
				(authorization?.startsWith('Bearer ') || authorization?.startsWith('Bot ')) &&
				authorization.split(/\s+/).length === 2;
			if (isTokenAuth) {
				await next();
				return;
			}
			...
			if (!origin || !isKnownAppOrigin(origin)) {
...
routes.use(LegalHoldMiddleware);
...
if (Config.cloudpayments.enabled) {
	CloudPaymentsController(routes);
}
...
await initializeAdminBootstrap();
...
Logger.info(
	{
		...
		music_search_enabled: Config.musicSearch.enabled,
		music_search_upstreams: Config.musicSearch.upstreams.length,
		music_typesense_url: Config.musicSearch.typesense.url,
		music_typesense_api_key_set: !!Config.musicSearch.typesense.apiKey,
	},
...
Logger.info({port: Config.port}, `Astral API listening on http://0.0.0.0:${Config.port}`);

/*
 * Global safety net — catch unhandled errors that would otherwise
 * crash the Node.js process. The API should NEVER go down because of
 * a single Cassandra timeout, Redis hiccup, or unhandled promise.
 *
 * Strategy:
 *   - Log the error with full context via Sentry + Logger
 *   - Do NOT exit the process — keep serving requests
 *   - Individual requests that triggered the error will already have
 *     received a 500 via the Hono error handler
 */
process.on('uncaughtException', (error) => {
	Logger.error({error}, 'Uncaught exception — process NOT exiting');
	Sentry.captureException(error);
});

process.on('unhandledRejection', (reason) => {
	Logger.error({error: reason}, 'Unhandled promise rejection — process NOT exiting');
