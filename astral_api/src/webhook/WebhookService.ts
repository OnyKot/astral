...
export class WebhookService {
	...
	async createWebhook(
		...
		const webhook = await this.repository.create({
			...
			interactionUrl: data.interaction_url ?? null,
		});
	...
	async updateWebhook(
		...
		const updatedWebhook = await this.repository.update(webhookId, {
			...
			interactionUrl: updatedData.interactionUrl,
		});
	...
	private async updateWebhookData({
		...
		const interactionUrl =
			data.interaction_url !== undefined ? data.interaction_url : webhook.interactionUrl;
		return {name: name!, avatarHash, channelId, interactionUrl};
	}
