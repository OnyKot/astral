...
import type {StripeReferralService} from './StripeReferralService';
...
export class StripeCheckoutService {
	...
	async createCheckoutSession({userId, priceId, isGift = false, referralCode}: CreateCheckoutSessionParams): Promise<string> {
		if (!this.stripe) {
		...
		const referrerUserId =
			!isGift && this.referralService ? await this.referralService.resolveReferrerUserId(userId, referralCode) : null;
		...
		const checkoutParams: Stripe.Checkout.SessionCreateParams = {
			...
			metadata: {
				...
				referralCode: referrerUserId ? referralCode ?? '' : '',
	...
	getPriceIds(countryCode?: string): {
		...
		const requestedCurrency = getCurrency(countryCode);
