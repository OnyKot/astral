...
export class VoiceDataInitializer {
	...
	private async createDefaultRegions(
		...
		const livekitEndpoint =
			Config.voice.url ||
			(() => {
				const apiPublicUrl = new URL(Config.endpoints.apiPublic);
				const protocol = apiPublicUrl.protocol.slice(0, -1) === 'https' ? 'wss' : 'ws';
				// Keep explicit port for local development (e.g. localhost:8088) so LiveKit can be reached behind Caddy.
				return `${protocol}://${apiPublicUrl.host}/livekit`;
			})();
