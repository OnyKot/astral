...
import {getCachedChannel} from '~/channel/ChannelCacheHelpers';
import {mapChannelToResponse, mapMessageToResponse} from '~/channel/ChannelModel';
import type {IChannelRepository} from '~/channel/IChannelRepository';
import {RateLimitError, UnauthorizedError, UnknownGuildError, UnknownUserError} from '~/Errors';
...
import {ASTRAL_EPOCH} from '~/constants/Core';
...
export class RpcService {
	...
	constructor(
		...
		private grokService?: IGrokService,
	) {
		this.customStatusValidator = new CustomStatusValidator(this.userRepository, this.guildRepository, this.packService, this.grokService!);
	}
	...
	async handleRpcRequest({request, requestCache}: HandleRpcRequestParams): Promise<RpcResponse> {
		...
		const rpcType = request.type;
		try {
			const result = await this.dispatchRpcRequest({request, requestCache});
			getMetricsService().histogram({
				name: 'rpc.latency_ms',
				dimensions: {type: rpcType, result: 'ok'},
			...
			getMetricsService().histogram({
				name: 'rpc.latency_ms',
				dimensions: {type: rpcType, result: 'error'},
				valueMs: performance.now() - startedAt,
			});
			throw err;
		}
	...
	private async dispatchRpcRequest({request, requestCache}: HandleRpcRequestParams): Promise<RpcResponse> {
		switch (request.type) {
			case 'session':
				return {
					type: 'session',
	...
	private async handleSessionRequest({
		...
		const [privateChannels, relationships] = await Promise.all([
			(async () =>
				(
					await Promise.all(
						userData.privateChannels.map(async (channel) => {
							try {
								return await mapChannel(channel);
							} catch (error) {
								...
								Logger.warn(
									...
									'Skipping private channel with missing user during RPC session payload build',
								);
								return null;
							}
						}),
			...
			(async () =>
				(
					await Promise.all(
						userData.relationships.map(async (relationship) => {
							try {
								return await mapRelationship(relationship);
							} catch (error) {
								if (!isUnknownUserFailure(error)) {
									throw error;
								}
								// Negative-cached in UserCacheService for 60s; this branch
								// is expected (deleted friend in a cached relationship list).
								// Used to log at warn with full stack on every session
								// payload build — was the loudest line in prod logs.
								return null;
							}
						}),
					)
				).flatMap((relationship) => (relationship ? [relationship] : [])))(),
		]);
	...
	private async handleCallEnded(params: {
		...
		const [message, channel] = await Promise.all([
			...
			getCachedChannel(channelId, this.channelRepository, requestCache),
		]);
			...
			Logger.warn(
				{
					...
					reason: !message ? 'missing_message' : 'missing_channel',
				},
				'call_ended: cannot persist — message or channel vanished',
			);
			getMetricsService().counter({
				name: 'voice_call_ended_total',
				dimensions: {result: 'fail', reason: !message ? 'missing_message' : 'missing_channel'},
			});
			return;
		}
		...
		if (message.type !== MessageTypes.CALL) {
			Logger.warn(
				{channelId: channelId.toString(), messageId: messageId.toString(), messageType: message.type},
				'call_ended: message is not a CALL type — ignoring',
			);
			getMetricsService().counter({
				name: 'voice_call_ended_total',
				dimensions: {result: 'fail', reason: 'wrong_message_type'},
			});
			return;
		}

		// Best-effort call duration for observability. Message id is an
		// Astral snowflake — the high bits encode the millisecond offset
		// from ASTRAL_EPOCH, which is ~when the CALL system message was
		// inserted (first participant joined). Close enough for a histogram.
		const messageBornMs = Number((BigInt(message.id) >> 22n)) + ASTRAL_EPOCH;
		const durationMs = endedTimestamp.getTime() - messageBornMs;
		...
		if (!updatedMessage || !this.mediaService) {
			getMetricsService().counter({
				name: 'voice_call_ended_total',
				dimensions: {result: 'fail', reason: 'upsert_failed_or_no_media'},
			});
			return;
		}
		...
		getMetricsService().counter({
			name: 'voice_call_ended_total',
			dimensions: {result: 'ok', channel_type: String(channel.type)},
		});
		getMetricsService().histogram({
			name: 'voice_call_duration_ms',
			dimensions: {channel_type: String(channel.type)},
			valueMs: Math.max(0, durationMs),
		});
		Logger.info(
			{
				channelId: channelId.toString(),
			...
			'call_ended: persisted and broadcasting',
		);
	...
	private async getDmChannel(params: {
		...
		const channel = await getCachedChannel(channelId, this.channelRepository, requestCache);
		if (!channel) {
			return null;
		}
