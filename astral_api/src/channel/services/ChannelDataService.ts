...
export class ChannelDataService {
	...
	async getChannel({userId, channelId}: {userId: UserID; channelId: ChannelID}): Promise<Channel> {
		// Latency histogram on the raw channel fetch — the highest-traffic
		// read path after message.send. No p95/p99 signal before this.
		const startedAt = Date.now();
		try {
			const channel = await this.channelOperationsService.getChannel({userId, channelId});
			getMetricsService().histogram({
				name: 'channel.fetch.latency_ms',
	...
	async getChannelAuthenticated({
		...
		const startedAt = Date.now();
		try {
			const result = await this.channelAuthService.getChannelAuthenticated({userId, channelId});
			getMetricsService().histogram({
				name: 'channel.fetch_authenticated.latency_ms',
				valueMs: Date.now() - startedAt,
			});
			return result;
		} catch (error) {
