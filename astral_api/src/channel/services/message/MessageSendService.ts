...
import {moderateTextAsync} from '~/infrastructure/ContentModerationHelper';
...
import type {Channel, Message, User, Webhook} from '~/Models';
...
export class MessageSendService {
	...
	async sendMessage({
		...
		try {
			...
			// Fire-and-forget moderation: runs after send so the user
			// doesn't wait for the Grok roundtrip. If flagged, the
			// message is deleted asynchronously.
			if (data.content && !user.isBot) {
				moderateTextAsync(this.deps.grokService, data.content, {
					onFlagged: async () => {
						await this.deps.deleteMessageAfterModeration({channel, message});
					},
				});
			}

			getMetricsService().counter({name: 'message.send'});
			// Latency histogram for alerting — `message.send` is the
			// critical user-facing write path, we were flying blind on
			// p95/p99 before this.
			getMetricsService().histogram({
				name: 'message.send.latency_ms',
				valueMs: Date.now() - startedAt,
			});
