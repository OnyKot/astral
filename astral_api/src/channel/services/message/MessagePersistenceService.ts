...
import {type MessageComponents, serializeComponents} from '~/channel/ComponentTypes';
...
interface CreateMessageParams {
	...
	allowEmbeds?: boolean;
	/*
	 * Discord-style action rows / buttons / selects. Currently only the
	 * webhook execute path passes this in (regular user sends ignore it).
	 * Persisted as a JSON string in the messages.components column.
	 */
...
export class MessagePersistenceService {
	...
	async createMessage(params: CreateMessageParams): Promise<Message> {
		...
		const messageRowData = {
			...
			has_reaction: false,
			version: 1,
			components: serializeComponents(params.components),
		};
