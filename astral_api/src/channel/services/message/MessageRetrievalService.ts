...
import {MissingPermissionsError, UnknownMessageError} from '~/Errors';
...
export class MessageRetrievalService {
	...
	constructor(
		...
		this.internalMessageSearchService = new InternalMessageSearchService(
			this.channelRepository.messages,
			this.userRepository,
		);
	}
	...
	private getMessageSearchService() {
		try {
			...
			Logger.warn({error}, '[MessageRetrievalService] Message search backend unavailable, using internal fallback');
			return null;
		}
	}
	...
	async searchMessages({
		...
		if (searchService && !channelIndexedAt) {
			await this.searchService.triggerChannelIndexing(channelId);
			Logger.warn(
				{channelId: channelId.toString()},
				'[MessageRetrievalService] Channel not indexed yet, using internal search fallback',
			);
		}
		...
		const result = searchService
			? await searchService
					...
					.catch((error) => {
						Logger.warn({error, channelId: channelId.toString()}, '[MessageRetrievalService] External search failed');
						return null;
					})
			: null;
		const finalResult =
		...
		const messageEntries = finalResult.hits.map((hit) => ({
