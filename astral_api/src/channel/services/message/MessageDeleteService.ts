...
export class MessageDeleteService {
	...
	private async deleteResolvedMessage({
		channel,
		message,
		auditLogUserId,
	}: {
		...
		if (message.pinnedTimestamp) {
			await this.deps.channelRepository.messageInteractions.removeChannelPin(channel.id, messageId);
		}
		...
		if (channel.guildId && auditLogUserId) {
			await this.guildAuditLogService
				.createBuilder(channel.guildId, auditLogUserId)
	...
	async deleteMessage({
		...
		try {
			...
			await this.deleteResolvedMessage({channel, message, auditLogUserId: userId});
	...
	async deleteMessageAfterModeration({channel, message}: {channel: Channel; message: Message}): Promise<void> {
		try {
			await this.deleteResolvedMessage({channel, message});
			getMetricsService().counter({name: 'message.moderation.delete'});
		} catch (error) {
			getMetricsService().counter({name: 'message.moderation.delete.error'});
			throw error;
		}
	}
