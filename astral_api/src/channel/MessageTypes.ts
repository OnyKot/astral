...
import {MessageComponentsSchema} from '~/channel/ComponentTypes';
...
const BaseMessageResponse = z.object({
	...
	components: MessageComponentsSchema.nullish(),
});
...
const MessageRequestSchema = z
	.object({
		...
		tts: z.boolean().optional(),
		/*
		 * Discord-style message components. Currently the only senders that
		 * actually use this are webhooks (bots) — regular user messages
		 * will always have null here. The user-facing send route ignores
		 * the field even if you pass it. The webhook execute route reads
		 * it and persists it.
		 */
		components: MessageComponentsSchema.nullish(),
	})
