...
const mapMessageEmbedToResponse = ({embed, mediaService}: MapMessageEmbedToResponseParams): MessageEmbedResponse => ({
	...
	// Non-null assertions on nested fields have bitten us before: a
	// Cassandra row with a partial embed (e.g. image persisted without
	// url after a botched migration) would crash the whole message
	// mapping. Fall back to sane defaults so one broken row doesn't
	// take down a whole channel history request.
	...
	provider: embed.provider && embed.provider.name
		? {
		...
		: undefined,
	video: embed.video && embed.video.url
		? {
				url: embed.video.url,
...
export async function mapMessageToResponse({
	...
	if (message.authorId) {
		...
		// Previously threw, which propagated up as a 500 and — for rows
		// caught by the channel-history builder in particular — killed
		// the request that the gateway session was awaiting to finish
		// bootstrap, ultimately causing the session process to die and
		// the client to surface the "Reset app data" prompt. A malformed
		// row should blank the author slot, not bring down the pipeline.
		// The caller (MessageController) already filters out null rows.
		author = {
	...
	if (message.mentionedUserIds.size > 0) {
		...
		// A @-mention of a since-deleted user no longer crashes the
		// whole message render — the mention just disappears from the
		// response. The message content itself still carries the raw
		// <@id> token, so UI fallback is well-defined.
		response.mentions = mentionedUserIds
			.map((userId) => mentionedUserPartials.get(userId))
	...
	if (message.components && message.components.length > 0) {
		response.components = message.components;
	}
