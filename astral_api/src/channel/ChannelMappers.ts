...
async function addDMRecipients(
	...
	if (
		...
		if (recipientIds.length > 0) {
			...
			// Drop recipients whose user vanished between ID-list capture
			// and resolve (deleted account, cache-stale pointer). Same
			// defensive pattern as the guild mappers — a stale recipient
			// in a DM channel should not crash the channel payload.
			response.recipients = recipientIds
				.map((userId) => userPartials.get(userId))
