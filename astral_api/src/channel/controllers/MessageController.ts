...
import {InputValidationError, UnclaimedAccountRestrictedError, UnknownUserError} from '~/Errors';
...
export const MessageController = (app: HonoApp) => {
	...
	app.get(
		...
		async (ctx) => {
			...
			// Warm the per-request user cache in one batch instead of
			// re-entering UserCacheService from inside every mapMessageToResponse.
			// Without this, a 50-message page from 5 authors fires 50 cache
			// reads (redis round-trip each) sequentially inside the Promise.all;
			// after batching the repeats become Map-hits on requestCache.userPartials.
			...
			if (authorIds.length > 0) {
				await ctx.get('userCacheService')
					.getUserPartialResponses(authorIds, requestCache)
					.catch(() => {
						// Non-fatal — individual per-message lookups will retry
						// and surface their own UnknownUserError handling.
					});
			}

			// Batch-fetch referenced messages (replies) so mapMessageToResponse
			// doesn't re-enter the repo for each reply inside the Promise.all.
			// Reference targets can live in *any* channel (cross-channel reply),
			// so we key by "<channelId>:<messageId>" and pass a callback that
			// just consults the pre-built Map.
				...
				await Promise.all(
					Array.from(referenceTargets.entries()).map(async ([k, ref]) => {
						try {
							...
							referencedMessagesMap.set(k, null);
						}
					}),
				);
			}
			...
			const mappedMessages = (
				await Promise.all(
					messages.map(async (message) => {
						try {
							return await mapMessageToResponse({
								message,
								...
								getReferencedMessage: async (channelId, messageId) => {
									const k = referenceKey(channelId, messageId);
									if (referencedMessagesMap.has(k)) {
										return referencedMessagesMap.get(k) ?? null;
									}
									// Fall back to direct fetch for references we didn't
									// pre-collect (e.g. nested reference-of-reference).
									return ctx.get('channelRepository').getMessage(channelId, messageId);
								},
							});
						} catch (error) {
							...
							Logger.warn(
								...
								'Skipping message with missing user during channel history response build',
							);
							return null;
						}
					}),
