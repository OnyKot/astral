...
const buildCodeChallengeFromVerifier = (verifier: string, method: 'S256' | 'plain'): string => {
	if (method === 'plain') {
		return verifier;
	}
...
export class OAuth2Service {
	...
	private validatePkceAuthorizationParams(params: {
		codeChallenge?: string;
		codeChallengeMethod?: 'S256' | 'plain';
	}): {codeChallenge: string | null; codeChallengeMethod: 'S256' | 'plain' | null} {
		if (params.codeChallengeMethod && !params.codeChallenge) {
			throw new InvalidRequestError('code_challenge is required when code_challenge_method is provided');
		}
		...
		if (!PKCE_CHALLENGE_REGEX.test(params.codeChallenge)) {
			throw new InvalidRequestError('Invalid code_challenge');
		}

		return {
			codeChallenge: params.codeChallenge,
			codeChallengeMethod: params.codeChallengeMethod ?? 'plain',
		};
	}

	private verifyPkceForAuthorizationCode(params: {
		...
		if (!params.codeVerifier || !PKCE_VERIFIER_REGEX.test(params.codeVerifier)) {
			throw new InvalidGrantError('Missing or invalid code_verifier');
		}

		const challengeMethod = params.codeChallengeMethod ?? 'plain';
		const calculatedChallenge = buildCodeChallengeFromVerifier(params.codeVerifier, challengeMethod);

		if (!secureStringEquals(calculatedChallenge, params.codeChallenge)) {
	...
	async authorizeAndConsent(params: {
		...
		const pkce = this.validatePkceAuthorizationParams({
			codeChallenge: params.codeChallenge,
	...
	async tokenExchange(params: {
		...
		if (params.grantType === 'authorization_code') {
			...
			this.verifyPkceForAuthorizationCode({
