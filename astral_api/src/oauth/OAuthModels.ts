...
const PKCECodeChallenge = createStringType(43, 128).refine(
	(value) => PKCE_CHALLENGE_REGEX.test(value),
	'Invalid PKCE code_challenge format',
);

const PKCECodeVerifier = createStringType(43, 128).refine(
	(value) => PKCE_VERIFIER_REGEX.test(value),
	'Invalid PKCE code_verifier format',
);

const PKCECodeChallengeMethod = z.enum(['S256', 'plain']);
...
export const AuthorizeRequest = z.object({
	...
	code_challenge: PKCECodeChallenge.optional(),
	code_challenge_method: PKCECodeChallengeMethod.optional(),
	prompt: z.enum(['consent', 'none']).optional(),
	guild_id: Int64Type.optional(),
	permissions: z.string().optional(),
	disable_guild_select: z.enum(['true', 'false']).optional(),
}).superRefine((value, ctx) => {
	if (value.code_challenge_method && !value.code_challenge) {
		ctx.addIssue({
			code: 'custom',
			message: 'code_challenge is required when code_challenge_method is provided',
			path: ['code_challenge'],
		});
	}
});
...
export const TokenRequest = z.discriminatedUnion('grant_type', [
	z.object({
		...
		code_verifier: PKCECodeVerifier.optional(),
