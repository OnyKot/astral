...
function getMusicOAuth2Config(): OAuth2Config | null {
	Logger.info(
		{
			MUSIC_OAUTH2_AUTO_CREATE: Config.musicOauth2.autoCreate,
			CLIENT_ID_SET: !!Config.musicOauth2.clientId,
			CLIENT_SECRET_SET: !!Config.musicOauth2.clientSecret,
		},
		'getMusicOAuth2Config check',
	);

	if (!Config.musicOauth2.autoCreate) {
		return null;
	}

	const clientIdStr = Config.musicOauth2.clientId;
	const clientSecret = Config.musicOauth2.clientSecret;

	if (!clientIdStr || !clientSecret) {
		Logger.info(
			'Skipping music OAuth2 client auto-create; set MUSIC_OAUTH2_CLIENT_ID and MUSIC_OAUTH2_CLIENT_SECRET to enable.',
		);
		return null;
	}

	const parseResult = OAuth2ConfigSchema.safeParse({
		clientIdStr,
		clientSecret,
		redirectUri: Config.musicOauth2.redirectUri,
	});
	if (!parseResult.success) {
		Logger.error({errors: parseResult.error.issues}, 'Invalid music OAuth2 configuration');
		return null;
	}
...
async function upsertOAuth2Client(
	...
	params: {
		config: OAuth2Config;
		name: string;
		ownerUserId: ReturnType<typeof createUserID>;
		logLabel: string;
	},
): Promise<void> {
	const {config, name, ownerUserId, logLabel} = params;
	Logger.info({clientId: config.clientIdStr, redirectUri: config.redirectUri}, `Upserting ${logLabel} OAuth2 client...`);
	const applicationId = createApplicationID(BigInt(config.clientIdStr));
	const existing = await repo.getApplication(applicationId);
	Logger.info({existing: !!existing}, `Checked for existing ${logLabel} application`);
	...
	if (existing) {
		...
		Logger.info(
			...
			`Updated ${logLabel} OAuth2 application`,
		);
		return;
	}
	...
	await repo.upsertApplication(row);
	Logger.info({application_id: applicationId.toString()}, `Created ${logLabel} OAuth2 application`);
}
...
export async function initializeOAuth(): Promise<void> {
	try {
		...
		if (adminCfg) {
			await upsertOAuth2Client(repo, {
				config: adminCfg,
				name: 'Astral Admin',
				ownerUserId: createUserID(-1n),
				logLabel: 'admin',
			});
		}

		const musicCfg = getMusicOAuth2Config();
		Logger.info({musicCfg: !!musicCfg}, 'Music OAuth2 config loaded');
		if (musicCfg) {
			await upsertOAuth2Client(repo, {
				config: musicCfg,
				name: 'AstraMusic',
				ownerUserId: createUserID(-2n),
				logLabel: 'music',
			});
		}
