...
export class AdminUserSecurityService {
	...
	async sendPasskeyMigrationEmail(
		...
		const {userRepository, emailService, auditService} = this.deps;
		const userId = createUserID(data.user_id);
		const user = await userRepository.findUnique(userId);
		if (!user) {
			throw new UnknownUserError();
		...
		if (credentials.length === 0) {
			throw InputValidationError.create('user_id', 'User does not have passkeys');
		}

		await emailService.sendPasskeyMigrationNotice(user.email, user.username, user.locale);

		await auditService.createAuditLog({
			adminUserId,
			targetType: 'user',
			targetId: BigInt(userId),
			action: 'send_passkey_migration_email',
			auditLogReason,
			metadata: new Map([
				['email', user.email],
				['credential_count', credentials.length.toString()],
			]),
		});
	}
	...
	async resetPasskeys(data: ResetPasskeysRequest, adminUserId: UserID, auditLogReason: string | null) {
		...
		await userRepository.deleteAllWebAuthnCredentials(userId);
		...
		let emailSent = false;
		if (user.email) {
			emailSent = await emailService.sendPasskeyResetNotice(user.email, user.username, user.locale);
		}
		...
		await auditService.createAuditLog({
			...
			auditLogReason,
			metadata: new Map([
				['removed_credentials', credentials.length.toString()],
