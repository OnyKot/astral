...
import {ALL_FEATURE_FLAGS, FEATURE_FLAG_GLOBAL_GUILD_ID, FeatureFlags} from '~/constants/FeatureFlags';
	...
	app.post(
		...
		Validator(
			...
			z.object({
				...
				guild_ids: z.union([z.string(), z.array(z.string())]).optional(),
				enabled_for_all: z.boolean().optional(),
			}),
		),
		async (ctx) => {
			const {flag, guild_ids, enabled_for_all} = ctx.req.valid('json');
			const featureFlagService = ctx.get('featureFlagService');

			const rawGuildIds = guild_ids ?? [];
			const guildIdArray = (Array.isArray(rawGuildIds) ? rawGuildIds : rawGuildIds.split(','))
				.map((id) => id.trim())
				.filter((id) => id.length > 0);
			const guildIdSet = enabled_for_all ? new Set([FEATURE_FLAG_GLOBAL_GUILD_ID]) : new Set(guildIdArray);
