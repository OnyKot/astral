...
export const GuildAdminController = (app: HonoApp) => {
	...
	app.get(
		'/admin/guilds/discovery/applications',
		RateLimitMiddleware(AdminRateLimitConfigs.ADMIN_LOOKUP),
		requireAdminACL(AdminACLs.GUILD_DISCOVERY_REVIEW),
		Validator(
			'query',
			z.object({
	...
	app.get(
		'/admin/guilds/:guild_id/discovery/application',
		RateLimitMiddleware(AdminRateLimitConfigs.ADMIN_LOOKUP),
		requireAdminACL(AdminACLs.GUILD_DISCOVERY_REVIEW),
		Validator('param', z.object({guild_id: Int64Type})),
		async (ctx) => {
			const guildId = createGuildID(ctx.req.valid('param').guild_id);
			const guildService = ctx.get('guildService');
			return ctx.json(await guildService.discoveryApplications.getApplicationAsAdmin(guildId));
		},
	);
	...
	app.post(
		...
		RateLimitMiddleware(RateLimitConfigs.ADMIN_GUILD_MODIFY),
		requireAdminACL(AdminACLs.GUILD_DISCOVERY_REVIEW),
		Validator('json', z.object({guild_id: Int64Type, review_note: z.string().max(500).nullish()})),
		async (ctx) => {
			const adminUserId = ctx.get('adminUserId');
			const body = ctx.req.valid('json');
			const guildId = createGuildID(body.guild_id);
			const guildService = ctx.get('guildService');
			return ctx.json(
				await guildService.discoveryApplications.approveApplication({
					adminId: adminUserId,
					guildId,
	...
	app.post(
		...
		async (ctx) => {
			...
			const guildService = ctx.get('guildService');
			return ctx.json(
				await guildService.discoveryApplications.rejectApplication({
					adminId: adminUserId,
					guildId,
					reviewNote: body.review_note ?? null,
				}),
