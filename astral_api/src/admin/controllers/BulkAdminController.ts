...
export const BulkAdminController = (app: HonoApp) => {
	...
	app.post(
		'/admin/bulk/send-email-broadcast',
		RateLimitMiddleware(RateLimitConfigs.ADMIN_BULK_OPERATION),
		requireAdminACL(AdminACLs.BULK_SEND_EMAIL_BROADCAST),
		Validator('json', BulkSendEmailBroadcastRequest),
		async (ctx) => {
			const adminService = ctx.get('adminService');
			const adminUserId = ctx.get('adminUserId');
			const auditLogReason = ctx.get('auditLogReason');
			return ctx.json(await adminService.bulkSendEmailBroadcast(ctx.req.valid('json'), adminUserId, auditLogReason));
		},
	);
