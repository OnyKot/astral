...
export const SendPasskeyMigrationEmailRequest = z.object({
	user_id: Int64Type,
});

export type SendPasskeyMigrationEmailRequest = z.infer<typeof SendPasskeyMigrationEmailRequest>;

export const ResetPasskeysRequest = z.object({
	user_id: Int64Type,
});

export type ResetPasskeysRequest = z.infer<typeof ResetPasskeysRequest>;
...
export const BulkSendEmailBroadcastRequest = z.object({
	subject: createStringType(1, 200),
	body: createStringType(1, 20000),
	broadcast_key: createStringType(1, 100)
		.refine((value) => /^[a-z0-9._:-]+$/.test(value), 'Invalid broadcast key format')
		.optional(),
	category: createStringType(1, 64)
		.refine((value) => /^[a-z0-9._:-]+$/.test(value), 'Invalid broadcast category format')
		.optional(),
	attach_tracking_headers: z.boolean().default(true),
});

export type BulkSendEmailBroadcastRequest = z.infer<typeof BulkSendEmailBroadcastRequest>;
