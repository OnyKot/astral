...
const FETCH_PENDING_VERIFICATION_QUERY = PendingVerifications.selectCql({
...
export class AdminRepository implements IAdminRepository {
	...
	async removePendingVerification(userId: UserID): Promise<void> {
		const pending = await fetchOne<PendingVerificationRow>(FETCH_PENDING_VERIFICATION_QUERY, {user_id: userId});
		if (!pending) {
			return;
		}
