...
const PushSubscribeWebRequest = z.object({
	type: z.literal('webpush').optional(),
	endpoint: URLType,
	keys: z.object({
...
const PushSubscribeFcmRequest = z.object({
	type: z.literal('fcm'),
	token: createStringType(16, 4096),
	user_agent: createStringType(1, 1024).optional(),
});

const PushSubscribeRequest = z.union([PushSubscribeWebRequest, PushSubscribeFcmRequest]);

const buildUserProfilePayload = async (
	ctx: Context<HonoEnv>,
	params: {
		currentUserId: ReturnType<typeof createUserID>;
		targetUserId: ReturnType<typeof createUserID>;
		guildId?: ReturnType<typeof createGuildID>;
...
export const UserAccountController = (app: HonoApp) => {
	...
	app.get(
		...
		async (ctx) => {
			...
			return ctx.json(
				await buildUserProfilePayload(ctx, {
					currentUserId,
					targetUserId,
					guildId,
	...
	app.post(
		...
		Validator(
			...
			z.object({
				...
				guild_id: Int64Type.optional(),
				with_mutual_friends: z.boolean().optional().default(true),
				with_mutual_guilds: z.boolean().optional().default(true),
			}),
		),
		async (ctx) => {
			const {user_id, channel_id, guild_id, with_mutual_friends, with_mutual_guilds} = ctx.req.valid('json');
			const currentUserId = ctx.get('user').id;
			const targetUserId = createUserID(user_id);
			const channelId = channel_id != null ? createChannelID(channel_id) : undefined;
			let guildId = guild_id != null ? createGuildID(guild_id) : undefined;
			...
			if (channelId) {
				...
				if (guildId && channel.guildId && guildId !== channel.guildId) {
					throw InputValidationError.create('guild_id', 'Guild does not match the provided channel');
				}
			...
			const payload = await buildUserProfilePayload(ctx, {
				currentUserId,
				targetUserId,
				guildId,
				withMutualFriends: with_mutual_friends,
			...
			const path = channelId
				? `/channel/${channelId.toString()}/user/${targetUserId.toString()}`
				: `/user/${targetUserId.toString()}`;
			const requestUrl = new URL(ctx.req.url);

			return ctx.json({
				path,
				url: `${requestUrl.origin}${path}`,
				channel_id: channelId?.toString() ?? null,
				guild_id: guildId?.toString() ?? null,
				...payload,
			});
		},
	);
	...
	app.post(
		...
		async (ctx) => {
			...
			const subscription =
				body.type === 'fcm'
					? await ctx.get('userService').registerPushSubscription({
							userId: ctx.get('user').id,
							provider: 'fcm',
							token: body.token,
