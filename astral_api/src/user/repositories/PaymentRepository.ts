...
const FETCH_PAYMENTS_BY_REFERRER_QUERY = PaymentsByReferrer.selectCql({
	where: PaymentsByReferrer.where.eq('referrer_user_id'),
});
...
export class PaymentRepository {
	...
	async updatePayment(data: Partial<PaymentRow> & {checkout_session_id: string}): Promise<{applied: boolean}> {
		...
		const result = await executeVersionedUpdate(
			...
			(current) => {
				...
				addField('status');
				addField('gift_code');
				addField('referral_code');
				addField('referrer_user_id');
				addField('referral_reward_cents');
	...
	private async updatePaymentIndexes(data: Partial<PaymentRow> & {checkout_session_id: string}): Promise<void> {
		...
		const payment = await this.getPaymentByCheckoutSession(data.checkout_session_id);
		if (payment?.referrerUserId && payment.completedAt) {
			batch.addPrepared(
				PaymentsByReferrer.upsertAll({
					referrer_user_id: payment.referrerUserId,
	...
	async findPaymentRefsByReferrerUserId(referrerUserId: UserID): Promise<Array<PaymentByReferrerRow>> {
		return fetchMany<PaymentByReferrerRow>(FETCH_PAYMENTS_BY_REFERRER_QUERY, {
