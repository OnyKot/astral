...
export class UserAuthService {
	...
	async verifyEmail(token: string): Promise<boolean> {
		...
		const tokenEmail = normalizeEmail(emailToken.email);
	...
	async resendVerificationEmail(user: User): Promise<boolean> {
		...
		await this.emailService.sendEmailVerification(recipientEmail, user.username, verificationToken, user.locale);
		return true;
	}
