...
type RegisterPushSubscriptionParams =
	| {
			userId: UserID;
			provider: 'webpush';
			endpoint: string;
...
export class UserContentService {
	...
	async registerPushSubscription(params: RegisterPushSubscriptionParams): Promise<PushSubscription> {
		const {userId, userAgent} = params;

		const endpoint = params.provider === 'fcm' ? `fcm://${params.token.trim()}` : params.endpoint;
		const keys =
			params.provider === 'fcm'
				? {
						p256dh: '__fcm__',
