...
import {PUBLIC_USER_FLAGS, UserFlags} from '~/Constants';
import type {
	BetaCode,
...
import {getRequiredActionNames} from './UserRequiredActions';
import type {
	BetaCodeResponse,
...
export const mapUserToPrivateResponse = (user: User): UserPrivateResponse => {
	const isPremium = user.isPremium();
	const requiredActions = getRequiredActionNames(user);
