...
export class ReportService {
	...
	constructor(
		...
		private grokService: IGrokService | null = null,
	) {
	...
	async reportMessage(
		...
		if (this.grokService?.isEnabled() && message.content) {
			this.grokService
				.analyzeReport(category, message.content, additionalInfo)
				.catch(() => {});
		}
	...
	private async findUserByTag(tag: string): Promise<User> {
		const parsed = this.parseAstralTag(tag);
		if (!parsed) {
			throw new InvalidDsaReportTargetError();
		}
	...
	private parseAstralTag(tag: string): {username: string; discriminator: number} | null {
		const trimmed = tag.trim();
		const match = /^(.+)#(\d{1,4})$/.exec(trimmed);
		if (!match) return null;
		const discriminator = Number.parseInt(match[2], 10);
		if (discriminator < 1 || discriminator > 9999) return null;
		return {
			username: match[1],
			discriminator,
		};
