...
const ASTRAL_TAG_REGEX = /^([^#]{1,32})#([0-9]{4})$/;

const ASTRAL_TAG_TYPE = z
	.string()
	.min(3)
	.max(37)
	.refine((value) => ASTRAL_TAG_REGEX.test(value), 'Astral tag must be in the format username#1234');
...
const DsaReportBase = z.object({
	...
	reporter_Astral_tag: z.optional(ASTRAL_TAG_TYPE),
});
...
const DsaReportMessage = DsaReportBase.extend({
	...
	reported_user_tag: z.optional(ASTRAL_TAG_TYPE),
});
...
const DsaReportUser = DsaReportBase.extend({
	...
	user_tag: z.optional(ASTRAL_TAG_TYPE),
