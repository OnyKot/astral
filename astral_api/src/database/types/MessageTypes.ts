...
export interface MessageRow {
	...
	version: number;
	/*
	 * Discord-style Interactions API: action rows containing buttons /
	 * select menus / future component types. Stored as a JSON string
	 * (Cassandra `text`) so the schema doesn't need a migration every
	 * time Discord ships a new component type. Always parsed/serialised
	 * at the persistence boundary; the rest of the app sees it as a
	 * structured Array<MessageActionRow>. Null = no components.
	 */
	components: Nullish<string>;
}
