...
async function executeQuery<T = Record<string, unknown>, P extends CassandraParams = CassandraParams>(
	...
	// Always measure — the histogram is the only way we get per-query
	// p50/p95/p99 signal on the hottest layer of the service. Cost is
	...
	try {
		...
		getMetricsService().histogram({
			name: 'cassandra.query.latency_ms',
			dimensions: {op: queryType, result: 'ok'},
			valueMs: durationMs,
		});
		...
		getMetricsService().histogram({
			name: 'cassandra.query.latency_ms',
			dimensions: {op: queryType, result: 'error'},
			valueMs: durationMs,
		});
		getMetricsService().counter({
			name: 'cassandra.query.error',
...
export async function executeConditional<P extends CassandraParams = CassandraParams>(
	...
	try {
		...
		/*
		 * Cassandra timeout errors (WriteTimeout 0x1100 / ReadTimeout
		 * 0x1200) during LWT/CAS queries mean the coordinator couldn't
		 * confirm the write within the timeout window. The write MAY
		 * have been applied — we genuinely don't know. Returning
		 * applied=false lets the caller (typically executeVersionedUpdate)
		 * retry, which is safe because CAS is idempotent.
		 *
		 * Previously we re-threw here, which propagated as an unhandled
		 * rejection and crashed the entire Node.js process on a single
		 * Cassandra hiccup.
		 */
		const isTimeoutError =
			err instanceof Error &&
			'code' in err &&
			(((err as {code: number}).code === 0x1100) || ((err as {code: number}).code === 0x1200));

		if (isTimeoutError) {
			Logger.warn(
				{error: errorMessage, query: cql},
				'Cassandra conditional query timed out — treating as not-applied (will retry if applicable)',
			);
			return {applied: false, rows: []};
		}
