...
export const SearchController = (app: HonoApp) => {
	app.post(
		...
		async (ctx) => {
			...
			if (Config.grok.searchEnhancementEnabled && searchParams.content) {
				const grokService = ctx.get('grokService') as IGrokService;
				if (grokService.isEnabled()) {
					try {
						const expansion = await grokService.expandSearchQuery(searchParams.content);
						searchParams.content = expansion.expandedQuery;
					} catch {}
				}
			}
