...
export class WataProductRegistry {
	private products = new Map<string, WataProduct>();

	constructor() {
		const prices = Config.wata.prices;
		if (!prices) return;
	...
	private register(product: WataProduct): void {
	...
	getProduct(priceId: string): WataProduct | null {
