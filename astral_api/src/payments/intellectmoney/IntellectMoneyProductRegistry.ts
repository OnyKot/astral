...
export interface IntellectMoneyProduct {
	priceId: string;
	type: ProductType;
	premiumType: 1 | 2;
...
export class IntellectMoneyProductRegistry {
	private products = new Map<string, IntellectMoneyProduct>();

	constructor() {
		const prices = Config.intellectmoney.prices;
		if (!prices) return;
	...
	private register(product: IntellectMoneyProduct): void {
	...
	getProduct(priceId: string): IntellectMoneyProduct | null {
