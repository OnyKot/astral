...
import {getEffectiveRequiredActionFlags} from '~/user/UserRequiredActions';
...
export const LoginRequired = createMiddleware<HonoEnv>(async (ctx, next) => {
	...
	const requiredActionFlags = getEffectiveRequiredActionFlags(user);
	if (requiredActionFlags !== 0) {
		throw new AccountSuspiciousActivityError(requiredActionFlags);
	}
