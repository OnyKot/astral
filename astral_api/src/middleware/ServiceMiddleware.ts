...
import {GuildDiscoveryApplicationRepository} from '~/guild/repositories/GuildDiscoveryApplicationRepository';
import {GuildJoinRequestRepository} from '~/guild/repositories/GuildJoinRequestRepository';
...
import {DisabledGrokService, GrokService} from '~/infrastructure/GrokService';
...
import {ApplicationCommandService} from '~/oauth/ApplicationCommandService';
...
import {ApplicationCommandRepository} from '~/oauth/repositories/ApplicationCommandRepository';
...
export const ServiceMiddleware = createMiddleware<HonoEnv>(async (ctx, next) => {
	...
	const grokService = Config.grok.enabled
		? new GrokService({
				enabled: true,
	...
	const guildDiscoveryApplicationRepository = new GuildDiscoveryApplicationRepository();
	const guildJoinRequestRepository = new GuildJoinRequestRepository();
	...
	const guildService = new GuildService(
		...
		webhookRepository,
		guildAuditLogService,
		guildDiscoveryApplicationRepository,
	...
	let cloudPaymentsService: CloudPaymentsService | null = null;
	...
	let intellectMoneyService: IntellectMoneyService | null = null;
	if (Config.intellectmoney.enabled) {
		intellectMoneyService = new IntellectMoneyService(
			userRepository,
			cacheService,
			gatewayService,
			guildRepository,
	...
	const applicationCommandService = new ApplicationCommandService({
	...
	ctx.set('applicationCommandService', applicationCommandService);
	...
	ctx.set('grokService', grokService);
	...
	if (cloudPaymentsService) {
		ctx.set('cloudpaymentsService', cloudPaymentsService);
	}
	if (intellectMoneyService) {
		ctx.set('intellectmoneyService', intellectMoneyService);
	}
	if (wataService) {
	...
	ctx.set('interactionService', interactionService);
