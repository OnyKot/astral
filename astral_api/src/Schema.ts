...
export const DiscriminatorType = z
	...
	.superRefine((value, ctx) => {
		...
		if (num < 1 || num > 9999) {
			ctx.addIssue({
				code: 'custom',
				message: 'Discriminator must be between 1 and 9999',
			});
			return z.NEVER;
		}
	})
...
const ASTRAL_TAG_REGEX = /^[a-zA-Z0-9_]+$/;
...
export const UsernameType = z
	...
	.refine(
		(value) => ASTRAL_TAG_REGEX.test(value),
	...
	.refine((value) => {
		const lowerValue = value.toLowerCase();
		return !lowerValue.includes('Astral') && !lowerValue.includes('system message');
	}, 'Username cannot contain "Astral" or "system message"');
