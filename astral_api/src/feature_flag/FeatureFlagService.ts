...
export class FeatureFlagService {
	...
	isFeatureEnabled(flag: FeatureFlag, guildId: string): boolean {
		...
		return guildIds?.has(FEATURE_FLAG_GLOBAL_GUILD_ID) || guildIds?.has(guildId) || false;
	}
	...
	async isFeatureEnabledForUser(
		...
		if (this.isFeatureEnabledGlobally(flag)) {
			return true;
		}
	...
	isFeatureEnabledGlobally(flag: FeatureFlag): boolean {
		return this.inMemoryCache.get(flag)?.has(FEATURE_FLAG_GLOBAL_GUILD_ID) ?? false;
	}
