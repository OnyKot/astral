...
const PROXY_INITIATOR_HEADER = 'x-Astral-proxy-initiator';
...
const refererMatchesAllowedOrigin = (referer?: string): boolean => {
	...
	return TRUSTED_APP_URLS.some((allowed) => referer.startsWith(allowed));
};
...
const setCorsHeaders = (res: http.ServerResponse, origin?: string) => {
	...
	res.setHeader(
		...
		'Authorization,Content-Type,Accept,Origin,X-Requested-With,X-Astral-Proxy-Initiator',
	);
	res.setHeader('Access-Control-Expose-Headers', 'x-Astral-sudo-mode-jwt,x-request-id,content-type');
};
...
const rejectIfDisallowedPage = (
	...
	if (originCandidate && !isAllowedOrigin(originCandidate)) {
		res.writeHead(403);
		res.end();
		return true;
	}
	...
	if (refererCandidate && !refererMatchesAllowedOrigin(refererCandidate)) {
		res.writeHead(403);
		res.end();
		return true;
	}
...
const pipeRequest = (
	...
	const upstreamReq = agent.request(requestOptions, (upstreamRes) => {
		...
		headers['Access-Control-Allow-Origin'] = req.headers.origin ?? req.headers.referer ?? STABLE_APP_URL;
		headers['Access-Control-Allow-Credentials'] = 'true';
		headers['Access-Control-Expose-Headers'] = 'x-Astral-sudo-mode-jwt,x-request-id,content-type';
		headers.Vary = 'Origin';
