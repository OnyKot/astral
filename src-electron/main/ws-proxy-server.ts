...
import {TRUSTED_APP_ORIGINS} from '../common/constants.js';
...
const normalizeTargetWsUrl = (url: string | null): string | null => {
	if (!url) return null;
	try {
...
const handleUpgrade = (request: http.IncomingMessage, socket: import('stream').Duplex, head: Buffer): void => {
	...
	const normalizedTargetUrl = normalizeTargetWsUrl(targetUrl);
	...
	wss?.handleUpgrade(request, socket, head, (clientSocket: WebSocket) => {
		wss?.emit('connection', clientSocket, request, normalizedTargetUrl);
	});
};
