...
import {getApiProxyUrl} from './api-proxy-server.js';
import {getMediaProxyToken, getMediaProxyUrl} from './media-proxy-server.js';
import {getWindowsNowPlayingSnapshot} from './windows-now-playing.js';
...
function createNativePasskeyProvider(): PasskeyProvider {
	const nativeAddon = loadNativeWebAuthnAddon();
	if (!nativeAddon) {
		return {
			isSupported: async () => false,
			authenticate: async () => {
				throw new Error('Passkeys are unavailable in this build');
			},
			register: async () => {
				throw new Error('Passkeys are unavailable in this build');
			},
...
function loadNativeWebAuthnAddon(): NativeWebAuthnAddon | null {
	try {
		const nativeAddon = requireModule('@electron-webauthn/native') as {
			isSupported?: unknown;
			get?: unknown;
			create?: unknown;
		};
		if (
			typeof nativeAddon.isSupported === 'function' &&
			typeof nativeAddon.get === 'function' &&
			typeof nativeAddon.create === 'function'
		) {
			return {
				isSupported: nativeAddon.isSupported as NativeWebAuthnAddon['isSupported'],
				get: nativeAddon.get as NativeWebAuthnAddon['get'],
				create: nativeAddon.create as NativeWebAuthnAddon['create'],
			};
		}
		console.warn('Failed to initialize @electron-webauthn/native: invalid addon shape');
		return null;
	} catch (error) {
		console.warn('Failed to initialize @electron-webauthn/native:', error);
		return null;
	}
}
...
export function registerIpcHandlers(): void {
	...
	ipcMain.handle('get-desktop-now-playing', async () => {
		try {
			return await getWindowsNowPlayingSnapshot();
		} catch (error) {
			console.warn('Failed to read Windows now playing snapshot:', error);
			return null;
		}
	});
	...
	ipcMain.handle('get-ws-proxy-url', (): string | null => getWsProxyUrl());
	ipcMain.on('get-ws-proxy-url', (event) => {
		event.returnValue = getWsProxyUrl();
	});
	ipcMain.handle('get-api-proxy-url', (): string | null => getApiProxyUrl());
	ipcMain.on('get-api-proxy-url', (event) => {
		event.returnValue = getApiProxyUrl();
	});
	ipcMain.handle('get-media-proxy-url', (): string | null => getMediaProxyUrl());
	ipcMain.on('get-media-proxy-url', (event) => {
		event.returnValue = getMediaProxyUrl();
	});
