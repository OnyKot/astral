...
import {Agent, errors, request, setGlobalDispatcher} from 'undici';
import {ASTRAL_USER_AGENT} from '~/Constants';
import type {ErrorType} from '~/lib/MediaTypes';
import * as metrics from '~/lib/MetricsClient';

/**
 * Per-request fetch times were tailing at ~10.5 s for external images
 * (undici's default connectTimeout of 10 s firing on slow/unreachable
 * hosts). A 10 s wait just to start the TCP handshake for a thumbnail
 * means the user has long abandoned the UI. We tighten both the
 * connection and header timeouts — the total-request timeout is a
 * separate AbortController below.
 */
setGlobalDispatcher(
	new Agent({
...
class HttpClient {
	// 8 s total request budget for external media. Anything slower than
	// this is effectively a broken thumbnail UX — users abandon long
	// before the old 30 s budget expired, and the request kept a worker
	// slot busy until then. Matches the upstream p99 budget we can
	// afford on an image-heavy feed.
	private static readonly DEFAULT_TIMEOUT = 8_000;
	// Only the standard web ports are reachable through the proxy. Allowing
	// arbitrary ports lets an attacker probe internal services on high ports
	// (e.g. http://public-host:6379/ -> Redis) even when the host itself
	...
	private static isDisallowedIPAddress(ip: string): boolean {
		...
		return (
			...
			(a === 255 && b === 255 && parts[2] === 255 && parts[3] === 255) // broadcast
		);
	}
	...
	private static async resolveSafeAddress(hostname: string): Promise<string> {
		...
		if (directIpVersion) {
			if (HttpClient.isDisallowedIPAddress(hostname)) {
				throw new HttpError('Unsafe URL destination', 400, undefined, true, 'forbidden');
			}
			return hostname;
		}
	...
	private static async assertSafeDestination(urlString: string): Promise<{pinnedIp: string}> {
		...
		if (hostname === 'localhost') throw new HttpError('Unsafe URL destination', 400, undefined, true, 'forbidden');
	...
	private static pinnedLookup(pinnedIp: string): LookupFunction {
		return (_hostname, options, callback) => {
			...
			// surfaces as "Invalid IP address: undefined" and fails every proxied fetch.
	...
	// consumed by the caller. Inherits the global agent's timeout budget.
	private static pinnedDispatcher(pinnedIp: string): Agent {
		return new Agent({
			connect: {timeout: 4_000, lookup: HttpClient.pinnedLookup(pinnedIp)},
			headersTimeout: 5_000,
			bodyTimeout: 10_000,
		});
	}
