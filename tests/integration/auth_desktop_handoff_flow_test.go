...
func TestAuthDesktopHandoffFlow(t *testing.T) {
	...
	if second.ControlToken == "" {
		t.Fatalf("expected second handoff control token")
	}

	cancelURL := fmt.Sprintf("/auth/handoff/%s", second.Code)
	resp, err = client.deleteWithHeaders(cancelURL, "", map[string]string{
		"X-Astral-Handoff-Control": second.ControlToken,
	})
	if err != nil {
	...
	resp, err = client.getWithHeaders(fmt.Sprintf("/auth/handoff/%s/status", second.Code), map[string]string{
		"X-Astral-Handoff-Control": second.ControlToken,
	})
	if err != nil {
