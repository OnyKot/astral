...
const disableDevServerClient = process.env.ASTRAL_DISABLE_DEV_SERVER_CLIENT === '1';
const enableDevRefresh = isDevelopment && !disableDevServerClient;
...
function getPublicEnvVar(name) {
    const value = process.env[name] ?? (isDevelopment ? DEV_ENV_DEFAULTS[name] : undefined);
    ...
    return {
        ...
        module: {
            rules: [{
                    ...
                    parser: {
                        /*
                         * Enable dynamic imports so React.lazy() and
                         * `import('./Foo')` actually produce separate
                         * chunks instead of being inlined into main.
                         * This was the primary reason the production
                         * main bundle was ~14MB.
                         */
                        dynamicImport: true,
                    },
                    use: {
        ...
        plugins: [
            ...
            new DefinePlugin({
                ...
                'import.meta.env.PUBLIC_EMOJI_STYLE': getPublicEnvVar('PUBLIC_EMOJI_STYLE'),
                'import.meta.env.PUBLIC_EMOJI_STYLE_CDN': getPublicEnvVar('PUBLIC_EMOJI_STYLE_CDN'),
                'import.meta.env.PUBLIC_TELEGRAM_EMOJI_STYLE_CDN': getPublicEnvVar('PUBLIC_TELEGRAM_EMOJI_STYLE_CDN'),
                'import.meta.env.PUBLIC_VK_EMOJI_STYLE_CDN': getPublicEnvVar('PUBLIC_VK_EMOJI_STYLE_CDN'),
                'import.meta.env.PUBLIC_EMOJI_STYLE_FILE_EXT': getPublicEnvVar('PUBLIC_EMOJI_STYLE_FILE_EXT'),
                'import.meta.env.PUBLIC_FIRST_PARTY_HOSTS': getPublicEnvVar('PUBLIC_FIRST_PARTY_HOSTS'),
                'import.meta.env.PUBLIC_FIRST_PARTY_HOST_SUFFIXES': getPublicEnvVar('PUBLIC_FIRST_PARTY_HOST_SUFFIXES'),
            }),

            enableDevRefresh && new ReactRefreshPlugin(),
        ].filter(Boolean),
        ...
        optimization: {
            ...
            runtimeChunk: false,
            /*
             * Removed `chunkSplit: false` — it was overriding the
             * splitChunks cache groups above and forcing every vendor
             * library back into the main bundle. With this gone, the
             * cacheGroups (livekit, framer-motion, mobx, sentry, etc.)
        ...
        devServer: isDevelopment ? {
            ...
            /*
             * Keep browser dev same-origin and avoid CORS preflight failures
             * when the app runs on 127.0.0.1:3000 but API is on astraof.com.
             */
            proxy: [{
                    ...
                    onProxyReq: (proxyReq, req) => {
                        ...
                        if (req.headers['x-captcha-token']) {
                            console.log('[Proxy] Forwarding X-Captcha-Token:', req.headers['x-captcha-token']);
                        }
                        if (req.headers['x-captcha-type']) {
                            console.log('[Proxy] Forwarding X-Captcha-Type:', req.headers['x-captcha-type']);
                        }
                    },
                },
                {
